...
LFHTEMP DataGrid<C,nbdim> DataGrid<C, nbdim>::selectedSlices(unsigned int which_dim, Vector<unsigned int> coor){
    ...
    Tuple<unsigned int, nbdim> dasize;
    unsigned int i;
    unsigned int totsize =1;
    for(i=0;i<which_dim;i++) {dasize[i] = dims[i]; totsize*=dims[i];}
...
LFHTEMP void DataGrid<C, nbdim>::leftHouseHolderMultiply_lame(const double * const vec, const int lenght,bool hint){
...
LFHTEMP template<class O> DataGrid<O,nbdim> DataGrid<C, nbdim>::applyFunctionOnPixels(std::function<void (const C*, O*)> func, uint32_t newpixsize) const{DataGrid<O,nbdim> fout;
	Tuple<uint32_t, nbdim> coor; coor = dims; if (newpixsize != 0) coor[0] = newpixsize;
	fout.setSizes(coor);
	class Task{ public: Task& operator<<=(Task& o){return *this;}
		void operator()(AtomicIterator<decltype(declval<const DataGrid<C,nbdim> >().getPixelIterator())> &pixels, std::function<void (const C*, O*)> &func, DataGrid<O,nbdim>& target){
			if (auto pite = pixels()) do{func(*pite, target(pite()));
			}while(pite++);
		}
		double operator()(std::function<void (const C*, O*)> &func, DataGrid<O,nbdim>& target){return 0.0;}
	};
	foreach[this->getPixelIterator()].template run<Task>(func,fout);
return fout;}
...
LFHTEMP C DataGrid<C, nbdim>::operator()(const typename Tuple<unsigned int,nbdim>::TUPLE_TYPE& p ) const{
    int pos=0;
    if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + Tuple<unsigned int,nbdim>::accessTupleType(p,i)) * dims[i-1];
    if (nbdim>=8) pos = (pos + Tuple<unsigned int,nbdim>::accessTupleType(p,7)) * dims[6];
    if (nbdim>=7) pos = (pos + Tuple<unsigned int,nbdim>::accessTupleType(p,6)) * dims[5];
    ...
    if (nbdim>=2) pos = (pos + Tuple<unsigned int,nbdim>::accessTupleType(p,1)) * dims[0];
return(this->data[pos + Tuple<unsigned int,nbdim>::accessTupleType(p,0)]);}
LFHTEMP C DataGrid<C, nbdim>::operator()(const typename Tuple<int,nbdim>::TUPLE_TYPE& p) const{
	int pos=0;
	if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + Tuple<int,nbdim>::accessTupleType(p,i)) * dims[i-1];
	if (nbdim>=8) pos = (pos + Tuple<int,nbdim>::accessTupleType(p,7)) * dims[6];
	if (nbdim>=7) pos = (pos + Tuple<int,nbdim>::accessTupleType(p,6)) * dims[5];
	...
	if (nbdim>=2) pos = (pos + Tuple<int,nbdim>::accessTupleType(p,1)) * dims[0];
return(this->data[pos + Tuple<int,nbdim>::accessTupleType(p,0)]);}
LFHTEMP const C* DataGrid<C, nbdim>::operator()(const typename Tuple<unsigned int,nbdim-1>::TUPLE_TYPE& p ) const{
    int pos=0;
    if (nbdim>=9) for(int i = nbdim-1;i>7;i--) pos = (pos + *(p() + i)) * dims[i-2];
	if (nbdim>=8) pos = (pos + *(p() + 6)) * dims[6];
	if (nbdim>=7) pos = (pos + *(p() + 5)) * dims[5];
	if (nbdim>=6) pos = (pos + *(p() + 4)) * dims[4];
	if (nbdim>=5) pos = (pos + *(p() + 3)) * dims[3];
	if (nbdim>=4) pos = (pos + *(p() + 2)) * dims[2];
	if (nbdim>=3) pos = (pos + *(p() + 1)) * dims[1];
	if (nbdim>=2) pos = (pos + *(p())) * dims[0];
return this->data + pos ;}
LFHTEMP const C* DataGrid<C, nbdim>::operator()(const typename Tuple<int,nbdim-1>::TUPLE_TYPE& p) const{
	int pos=0;
    if (nbdim>=9) for(int i = nbdim-1;i>7;i--) pos = (pos + *(p() + i)) * dims[i-2];
	if (nbdim>=8) pos = (pos + *(p() + 6)) * dims[6];
	if (nbdim>=7) pos = (pos + *(p() + 5)) * dims[5];
	...
	if (nbdim>=2) pos = (pos + *(p())) * dims[0];
return this->data + pos;}
LFHTEMP C* DataGrid<C, nbdim>::operator()(const typename Tuple<unsigned int,nbdim-1>::TUPLE_TYPE& p ){
    int pos=0;
    if (nbdim>=9) for(int i = nbdim-1;i>7;i--) pos = (pos + *(p() + i)) * dims[i-2];
	if (nbdim>=8) pos = (pos + *(p() + 6)) * dims[6];
	if (nbdim>=7) pos = (pos + *(p() + 5)) * dims[5];
	...
	if (nbdim>=2) pos = (pos + *(p())) * dims[0];
return this->data + pos ;}
LFHTEMP C* DataGrid<C, nbdim>::operator()(const typename Tuple<int,nbdim-1>::TUPLE_TYPE& p){
	int pos=0;
    if (nbdim>=9) for(int i = nbdim-1;i>7;i--) pos = (pos + *(p() + i)) * dims[i-2];
	if (nbdim>=8) pos = (pos + *(p() + 6)) * dims[6];
	if (nbdim>=7) pos = (pos + *(p() + 5)) * dims[5];
...
LFHTEMP typename MT_IFTYPE<(int)(nbdim-1), DataGrid<C,nbdim-1>,C>::TYPE DataGrid<C, nbdim>::operator()(unsigned int a) const{
    typename MT_IFTYPE<(int)(nbdim-1), DataGrid<C,nbdim-1>,C>::TYPE f_out;
...
return f_out;}
LFHTEMP typename MT_IFTYPE<(int)(nbdim-2), DataGrid<C,nbdim-2>,C>::TYPE DataGrid<C, nbdim>::operator()(unsigned int a,unsigned int b) const{
    typename MT_IFTYPE<(int)(nbdim-2), DataGrid<C,nbdim-2>,C>::TYPE f_out;
...
return f_out;}
LFHTEMP typename MT_IFTYPE<(int)(nbdim-3), DataGrid<C,nbdim-3>,C>::TYPE DataGrid<C, nbdim>::operator()(unsigned int a,unsigned int b,unsigned int c) const{
    typename MT_IFTYPE<(int)(nbdim-3), DataGrid<C,nbdim-3>,C>::TYPE f_out;
...
LFHTEMP const DataGrid<C,nbdim> & DataGrid<C, nbdim>::blur(const std::function<double(const Tuple<double,nbdim>&)> &blurfunc){
...
LFHTEMP const DataGrid<C,nbdim> & DataGrid<C, nbdim>::blur_zeroborder(const std::function<double(const Tuple<double,nbdim>&)> &blurfunc){
...
LFHTEMP DataGrid<C,nbdim> & DataGrid<C, nbdim>::blur_gauss_crude(const TMatrix<double, nbdim, nbdim> &blur){ // used to be gauss.ihvar, -half the precision matrix
    unsigned int coor[nbdim];
    unsigned int dir,altdir;
    ...
    for(loop =0;loop < nbdim; loop++){
     ...
     tmp = 1.0f-2.0f*blur.data[loop * (nbdim+1)];
//		 tmp = 1.0f+2.0f;
...
LFHTEMP DataGrid<unsigned int,nbdim> DataGrid<C, nbdim>::makeWatershedSortIndex(unsigned int &nbpix,  bool min_gradient_bassin, bool tree_skip_save) const{ // no_cheese inclues lone surrounded pixels
  ...
  //  bool has_cheese_up = false;
	...
	//	printf("%i\n", merges.getSize());
        ...
        stat[i][0] = toins.d;
        if (loc_merges[l].isEmpty()) foreach.terminate("empty?\n");
  ...
  //      printf("aliveb! %i\n",stat[stat[i][1]][2]); fflush(stdout);
		...
		// MARK AS DONE!
        if (stat[i][0]  == stat[i][1]) foreach.terminate("got equal...\n");
