 ...
 *
 *
 * This file contain class template for basic linear algebra types, such as:
 *   Complex / Quaternion
 *   Matrix and Trianglix (symmetric matrices)
 *
 *
 *
 *
	...
	void flushWarnings(){
		...
		if (lastw < 10){
			switch((LFHwarnings)lastw){
				...
				case LFH_ERROR_CANT_ALLOCATE_MEMORY:  LFH_exit("Could allocate memory! (critical error)\n");
...
void NBmeanvar_logprob_raw(double* fout6, const uint32_t x, const double mean, const double var); // argument domain: 0  < mean
void NBmeanvar_logprob(double* fout6, const uint32_t x, const double mean, const double excess_var); // argument domain: 0  < mean
void NBmeanvar_logprob_old(double* fout6, const uint32_t x, const double mean, const double excess_var); // argument domain: 0  < mean
...
class DataGrid{ //,: public ConstGrid<C,nbdim>  public TypedFunctor<C,const typename Tuple<unsigned int,nbdim>::TUPLE_TYPE& >{
	...
	typedef typename MT_IFTYPE<(int)(nbdim-1), Tuple<unsigned int, nbdim> , unsigned int >::TYPE ITERATOR_TYPE;
	...
	template<int ISCONST> class PixelIterator{
	public:
		Tuple<uint32_t, nbdim-1> coor;
        typename MetaType<C,ISCONST>::IS_CONST_PTR cur;
        typename MetaType<C,ISCONST>::IS_CONST_PTR maxval;
        typename MetaType<DataGrid<C,nbdim>,ISCONST>::IS_CONST_REF target;
        PixelIterator(typename MetaType<DataGrid<C,nbdim>,ISCONST>::IS_CONST_REF trg): target(trg){}
        operator bool (){cur = target.data; maxval = cur + target.totsize(); coor.toZero(); return (target.totsize()!=0u);}
        bool operator++(int){uint32_t i; cur+= target.dims[0]; for(i=1;i<nbdim;i++) {if ((++coor[i-1]) < target.dims[i]) break; coor[i-1]=0;} return (i < nbdim);}
        Tuple<uint32_t, nbdim-1> operator()()const{return coor;}
        typename MetaType<C,ISCONST>::IS_CONST_REF operator[](int offset){return cur[offset];}
        typename MetaType<C,ISCONST>::IS_CONST_REF operator[](unsigned int offset){return cur[offset];}
	...
	public:
        ...
        C* maxval;
        DataGrid<C,nbdim>& target;
        PixelIteratorRange(DataGrid<C,nbdim>& trg, C* _start, C* _maxval): target(trg), start(_start),maxval(_maxval){}
        operator bool (){cur = start; maxval = cur + target.totsize(); coor.toZero(); return (target.totsize()!=0u);}
        bool operator++(int){uint32_t i; cur += target.dims[0]; for(i=1;i<nbdim;i++) {if ((++coor[i-1]) < target.dims[i]) break; coor[i-1]=0;} return (i < nbdim);}
        Tuple<uint32_t, nbdim-1> operator()()const{return coor;}
        C& operator[](int offset){return cur[offset];}
        C* operator*(){return cur;}
        void show(FILE* f = stdout, int level =0){if (maxval == start) fprintf(f,"Empty pixel iterator!\n"); else fprintf(f,"Pixel iterator with %i color channels and %i pixels\n", target.dims[0], ((uint32_t)(maxval - start))/target.dims[0] );}
	};

	operator RemoteMemory<const C,nbdim>()const{RemoteMemory<const C,nbdim> fout; fout.remote_data = data; fout.dims = dims; fout.offsets[0] = 1; for(uint32_t i=1;i<nbdim;i++) fout.offsets[i] = dims[i - 1] * fout.offsets[i-1]; return fout;}
	operator RemoteMemory<C,nbdim>(){RemoteMemory<C,nbdim> fout; fout.remote_data = data; fout.dims = dims; fout.offsets[0] = 1; for(uint32_t i=1;i<nbdim;i++) fout.offsets[i] = dims[i - 1] * fout.offsets[i-1]; return fout;}

	// dereferencing datagrid gives an mutable handle, which does not own the data. (no resize / memory free)
	RemoteMemory<const C,nbdim> operator*()const{RemoteMemory<const C,nbdim> fout; fout.remote_data = data; fout.dims = dims; fout.offsets[0] = 1; for(uint32_t i=1;i<nbdim;i++) fout.offsets[i] = dims[i - 1] * fout.offsets[i-1]; return fout;}
	RemoteMemory<C,nbdim> operator*(){RemoteMemory<C,nbdim> fout; fout.remote_data = data; fout.dims = dims; fout.offsets[0] = 1; for(uint32_t i=1;i<nbdim;i++) fout.offsets[i] = dims[i - 1] * fout.offsets[i-1]; return fout;}

    DataGrid<C,nbdim>::Iterator getIterator(){return Iterator(*this);}
    DataGrid<C,nbdim>::PixelIterator<0> getPixelIterator(){return PixelIterator<0>(*this);}
    DataGrid<C,nbdim>::PixelIterator<1> getPixelIterator()const{return PixelIterator<1>(*this);}
    DataGrid<C,nbdim>::PixelIteratorRange getPixelIteratorOnRow(unsigned int row){return PixelIteratorRange(*this, data + (dims[0] * row) , data + (dims[0] * (row+1)));}
    RemoteMemory<const C,nbdim-1> getPixelsOnRow(unsigned int row) const{RemoteMemory<const C,nbdim-1> fout; fout.remote_data = data + row * dims[1] * dims[0]; fout.dims[1] = dims[1]; fout.dims[0] = dims[0]; fout.offsets[1] = dims[0]; fout.offsets[0] = 1; return fout;}
	...
	typename MT_IFTYPE<(int)(nbdim-1), DataGrid<C,nbdim-1>,C>::TYPE operator()(unsigned int) const;
	typename MT_IFTYPE<(int)(nbdim-2), DataGrid<C,nbdim-2>,C>::TYPE operator()(unsigned int,unsigned int) const;
	typename MT_IFTYPE<(int)(nbdim-3), DataGrid<C,nbdim-3>,C>::TYPE operator()(unsigned int,unsigned int,unsigned int) const;
	...
	C* operator()(const typename Tuple<unsigned int,nbdim-1>::TUPLE_TYPE & );
	C* operator()(const typename Tuple<int,nbdim-1>::TUPLE_TYPE&);
	const C* operator()(const typename Tuple<unsigned int,nbdim-1>::TUPLE_TYPE & )const;
	const C* operator()(const typename Tuple<int,nbdim-1>::TUPLE_TYPE&)const;
...
//	Tuple<unsigned int,nbdim> getSizes() const;
	...
	const DataGrid<C,nbdim>& blur(const std::function<double(const Tuple<double,nbdim>&)> &);
	const DataGrid<C,nbdim>& blur_zeroborder(const std::function<double(const Tuple<double,nbdim>&)> &);
	DataGrid<C,nbdim>& blur_gauss_crude(const TMatrix<double, nbdim, nbdim> &);

	DataGrid<C,nbdim>& blur_crude_dir(unsigned int dir, double wind);

	template<class O> DataGrid<O,nbdim> applyFunctionOnPixels(std::function<void (const C*, O*)> func, uint32_t newsize = 0)const;
...
public:
    ...
    double pnorm() const;
    double norm() const{return(sqrt(this->pnorm()));}
...
class RBGAPix{
	...
	RBGAPix(){}
	template<class C> RBGAPix(const Tuple<C,4>& other){rgba[0] = (unsigned char) other[0]; rgba[1] = (unsigned char) other[1]; rgba[2] = (unsigned char) other[2]; rgba[3] = (unsigned char) other[3]; }

    RBGAPix operator+(const RBGAPix& other)const{RBGAPix fout; unsigned alp[2]; alp[0] = ((unsigned int)rgba[3]); alp[1] = ((unsigned int)other.rgba[3]);
		if ((alp[0] + alp[1]) != 0){
        fout.rgba[0] = (unsigned char)((alp[0] * rgba[0] + alp[1] * other.rgba[0]) / (alp[0] + alp[1]));
        fout.rgba[1] = (unsigned char)((alp[0] * rgba[1] + alp[1] * other.rgba[1]) / (alp[0] + alp[1]));
        fout.rgba[2] = (unsigned char)((alp[0] * rgba[2] + alp[1] * other.rgba[2]) / (alp[0] + alp[1]));
        fout.rgba[3] = (unsigned char) ((alp[0] + alp[1]) >>1);
		}else fout.rgba[3] = 0;
        return fout;
    }

	RBGAPix& operator+=(const RBGAPix& other){RBGAPix fout; unsigned alp[2]; alp[0] = ((unsigned int)rgba[3]); alp[1] = ((unsigned int)other.rgba[3]);
		if ((alp[0] + alp[1]) != 0){
        rgba[0] = (unsigned char)((alp[0] * rgba[0] + alp[1] * other.rgba[0]) / (alp[0] + alp[1]));
        rgba[1] = (unsigned char)((alp[0] * rgba[1] + alp[1] * other.rgba[1]) / (alp[0] + alp[1]));
        rgba[2] = (unsigned char)((alp[0] * rgba[2] + alp[1] * other.rgba[2]) / (alp[0] + alp[1]));
        rgba[3] = (unsigned char) ((alp[0] + alp[1]) >>1);
		}else rgba[3] = 0;
        return *this;
    }
	...
	RBGAPix& operator<<=(const RBGAPix& other){RBGAPix fout; unsigned alp[2]; alp[0] = ((unsigned int)rgba[3]); alp[1] = ((unsigned int)other.rgba[3]);
		if ((alp[0] + alp[1]) != 0){
			rgba[0] = (unsigned char)(((255 - alp[1]) * rgba[0] + alp[1] * other.rgba[0]) / 255);
			rgba[1] = (unsigned char)(((255 - alp[1]) * rgba[1] + alp[1] * other.rgba[1]) / 255);
			rgba[2] = (unsigned char)(((255 - alp[1]) * rgba[2] + alp[1] * other.rgba[2]) / 255);
			rgba[3] = 255 - (unsigned char)((255-alp[0] * 255-alp[1])/ 255);
		}else rgba[3] = 0;
        return *this;
    }

	Tuple<double,4> operator*(const double fact)const{Tuple<double,4> fout; fout[0] = fact * rgba[0];fout[1] = fact * rgba[1];fout[2] = fact * rgba[2];fout[3] = fact * rgba[3];return fout;}

	unsigned char& operator[](unsigned int i){return rgba[i];}
	unsigned char operator[](unsigned int i) const {return rgba[i];}
};
...
public:
	...
	template <class C> bool put(const DataGrid<C,3u>& f_out, bool updateRange =false);
	template <class C, unsigned int SIZE, Tuple_flag Cflag> bool put(const DataGrid<Tuple<C, SIZE, Cflag>,2u>& f_out, bool updateRange =false);

	template <class C, class TYPE> bool put(const DataGrid<C,3u>& f_out, TYPE min, TYPE max, bool updateRange =false);
	template <class C, class TYPE> bool put(const DataGrid<C,2u>& f_out, TYPE min, TYPE max, bool updateRange =false);
	template <class C, class TYPE, unsigned int SIZE, Tuple_flag Cflag> bool put(const DataGrid<Tuple<C,SIZE, Cflag>,2u>& f_out, TYPE min, TYPE max, bool updateRange =false);
	...
	void putMeanVarImage(const DataGrid<double, 3u> &image, double* mean_stddev = NULL);
...
class GaussElem< Tuple<C, SIZE>, 0u >{ // COMMUTATIVE!
	...
	typedef std::integral_constant<bool, ExCo< Tuple<C, SIZE> >::IsPOD::value && ExCo< Trianglix<C, SIZE> >::IsPOD::value > IsPOD;
	...
	GaussElem< Tuple<C, SIZE>, 0u > setSize(unsigned int size){mean.setSize(size); cov.setSize(size); return *this; }
 //   void show(FILE* f = stdout, int level =0)const;
    ...
    GaussElem< Tuple<C, SIZE>, 0 > & operator=(const GaussElem< Tuple<C, SIZE>,0u > &other){mean = other.mean;cov = other.cov;w = other.w;w2 = other.w2; determinant = other.determinant;return(*this);}


	GaussElem< Tuple<C, SIZE>, 0u >& operator+=(const GaussElem< Tuple<C, SIZE>,0u > &other){w+=other.w; w2+=other.w2; mean += other.mean; cov += other.cov; ExOp::toZero(determinant); return(*this);}
    GaussElem< Tuple<C, SIZE>, 0u > operator+(const GaussElem< Tuple<C, SIZE>, 0u > &other)const {return (GaussElem< Tuple<C, SIZE>, 0u >(mean +other.mean,  cov + other.cov, w+other.w, w2+other.w2));}

	// free mean addition
	GaussElem< Tuple<C, SIZE>, 0 >& addGaussElem_free_mean(const GaussElem< Tuple<C, SIZE>, 0u > &other);
    GaussElem< Tuple<C, SIZE>, 0 >& addSample(const Tuple<C, 0u> &o, double _w=1.0f);
    GaussElem< Tuple<C, SIZE>, 0 >& addSample(const C* o, double _w=1.0f);
	...
	GaussElem< Tuple<C, SIZE>, 0u >& operator*=(const DBL_Weight& inc_w){w *= inc_w;w2 *= inc_w*inc_w; mean *= inc_w; cov *= inc_w; return (*this);}
	GaussElem< Tuple<C, SIZE>, 0u > operator*(const DBL_Weight& inc_w) const {return GaussElem< Tuple<C, SIZE>, 0u >(mean * inc_w, cov * inc_w, w * inc_w,w2 * inc_w*inc_w);}
	GaussElem< Tuple<C, SIZE>, 0u >& operator/=(const DBL_Weight& inc_w){w /= inc_w;w2 /= inc_w*inc_w; mean /= inc_w; cov /= inc_w; return (*this);}
	GaussElem< Tuple<C, SIZE>, 0u > operator/(const DBL_Weight& inc_w) const {return GaussElem< Tuple<C, SIZE>, 0u >(mean / inc_w, cov / inc_w, w / inc_w,w2 / (inc_w*inc_w));}
...
LFH_GOLD double likelihoodratio_dist(const GaussElem< Tuple<C, SIZE>, 0u > &other, bool has_weight = true, bool has_covariance = true) const;
    double bhattacharryya_dist(const GaussElem< Tuple<C, SIZE>, 0u > &other, bool has_weight = true, bool has_covariance = true) const; // weighted does not work
...
public:
	...
	GaussElem< Tuple<C, 0u>, 0u >& setSize(unsigned int size){mean.setSize(size); cov.setSize(size); return *this;}
    ...
    GaussElem< Tuple<C, 0u>, 0 > & operator=(const GaussElem< Tuple<C, 0u>,0u > &other){mean = other.mean;cov = other.cov;w = other.w;w2 = other.w2; determinant = other.determinant;return(*this);}
	...
	GaussElem< Tuple<C, 0u>, 0u >& operator+=(const GaussElem< Tuple<C, 0u>,0u > &other){
...
//		printf("time to crash done!\n"); fflush(stdout);
    ...
    GaussElem< Tuple<C, 0u>, 0u > operator+(const GaussElem< Tuple<C, 0u>, 0u > &other)const {return (GaussElem< Tuple<C, 0u>, 0u >(mean +other.mean,  cov + other.cov, w+other.w, w2+other.w2));}
	// free mean addition
	GaussElem< Tuple<C, 0u>, 0 >& addGaussElem_free_mean(const GaussElem< Tuple<C, 0u>, 0u > &other);
	...
	GaussElem< Tuple<C, 0u>, 0 >& operator-=(const Tuple<C, 0u>& shift){unsigned int i,j,k; for(j=0,k=0;j<mean.getSize();j++) for(i=0;i<=j;i++,k++) cov.data[k] += (((ExOp::mkTrju(shift[i]) * shift[j]) * w) - ExOp::mkTrju(shift[i]) * mean[j]- ExOp::mkTrju(mean[i]) * shift[j]); mean -= shift * w; return(*this);}
    GaussElem< Tuple<C, 0u>, 0 >& addSample(const Tuple<C, 0u> &o, double _w=1.0f);
    GaussElem< Tuple<C, 0u>, 0 >& addSample(const C* o, double _w=1.0f);

	// REVERT SCOPE!
	GaussElem< Tuple<C, 0u>, 0u >& operator-=(const GaussElem< Tuple<C, 0u>,0u > &other){w-=other.w; w2-=other.w2; mean -= other.mean; cov -= other.cov; ExOp::toZero(determinant); return(*this);}
	...
	GaussElem< Tuple<C, 0u>, 0u >& operator*=(const DBL_Weight& inc_w){w *= inc_w;w2 *= inc_w*inc_w; mean *= inc_w; cov *= inc_w; return (*this);}
	GaussElem< Tuple<C, 0u>, 0u > operator*(const DBL_Weight& inc_w) const {return GaussElem< Tuple<C, 0u>, 0u >(mean * inc_w, cov * inc_w, w * inc_w,w2 * inc_w*inc_w);}
	GaussElem< Tuple<C, 0u>, 0u >& operator/=(const DBL_Weight& inc_w){w /= inc_w;w2 /= inc_w*inc_w; mean /= inc_w; cov /= inc_w; return (*this);}
	GaussElem< Tuple<C, 0u>, 0u > operator/(const DBL_Weight& inc_w) const {return GaussElem< Tuple<C, 0u>, 0u >(mean / inc_w, cov / inc_w, w / inc_w,w2 /(inc_w*inc_w));}
    ...
    double likelihoodratio_dist_underbound(const GaussElem< Tuple<C, 0u>, 0u > &other, unsigned int nb_dims, bool has_weight = true, bool has_covariance = true, C const * const this_det = NULL, C const * const  other_det = NULL) const;

	LFH_GOLD double likelihoodratio_dist(const GaussElem< Tuple<C, 0u>, 0u > &other, bool has_weight = true, bool has_covariance = true) const;
    double bhattacharryya_dist(const GaussElem< Tuple<C, 0u>, 0u > &other, bool has_weight = true, bool has_covariance = true) const;
    ...
    Trianglix<C, 0u> getKernelDensity_Silverman(double scale_factor)const;
...
public:
	double value;
	double buffered; // during learning, cumulates gradient, outside of learning, remembers curvature
	operator double () const{ return value;}
};

// X_{g,i} ~ NB(M = X_i M_g Y_i + s_i Mbg f_i, v = X_i V_g Y_i + s_i Vbg f_i)
// dLL(X_i, M_g, Y_i,s_i, Mbg, f_i, V , Vbg) / dM_g
// dLL(X_i, M_g, Y_i,s_i, Mbg, f_i, V , Vbg)

// FA(g) M V Mbg Vbg
// FA(i) X_i Y_i F_i


// Scope holding buffers for learning, has no clue what the types are, assume it perfectly match the operation inbound
class MemoryLimitedOptimizationScope{
public:
	Tuple<double> scope;
	Tuple<double> hhfact;
	uint32_t step, size;
	double lastvalue; // pow(0.5, 45.0)
	MemoryLimitedOptimizationScope& init(uint32_t _history = 8, double _expdiff = pow(0.5, 15.0)){step =0; hhfact.setSize(_history<<1); scope.setSize(0); lastvalue = _expdiff;return *this;} // *must be different to initial guess, and both are within the search space domain*
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true> bool updateAscent(double error, I guess, J derivative); // return max relative change
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true> bool checkDerivative(double error, I guess, J derivative);
	void forceGuessInRange(double& curval, uint32_t index, double abs_max);
	void forceGuessPositive(double& curval, uint32_t index);

};


class AdativeLearningParameters{
public:
	double best_value;
	double cur_value;
	double alpha;
	double beta, beta2;
	int step;
	AdativeLearningParameters()=default;
	int operator()()const {return step;}
	operator bool(){step = 0; beta = 0.9; beta2 = 0.999; alpha = 0.01;return true;}
	bool operator++(int){step++;return step < 70;}
};

class AdativeLearningParametersMK2{
public:
	int step;
	double beta, beta2;
	double alpha;
...
// Lightweight class
class AdaptiveLearningScope{ // uses adam for learning OR memory limited LBFS update
public:
	Tuple<double> vars; // has length 3 for ADAM, and 4 > for LBFS update
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
	double updateVariable(const AdativeLearningParameters &params, I guess, J derivative);
	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true>
	//void writeBest(I guess);
	AdaptiveLearningScope& toMemfree(){vars.toMemfree(); return *this;}


	bool makeGuessStrictlyPositive(double& curval, uint32_t index); // return if got altered

	ERRCODE save(FILE*f) const{return vars.save(f);}
    ERRCODE load(FILE*f){return vars.load(f);}
};
...
public:
	...
	void init(double _expdiff = pow(0.5, 45.0)); // expected score diference (as a fraction of score)
	...
	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true> CurvatureSearchScope& initWithSecondGuess(I sec_guess); // *must be different to initial guess, and both are within the search space domain*
	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true> CurvatureSearchScope& initWithSecondDerivative(I sec_derivative); // *must be different to initial guess, and both are within the search space domain*

	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
		bool checkDerivativeCrude(double value, I guess, J derivative, SparseMatrix<double>* addout = NULL, int rowout=0); // replace updateAscent for debugging purposes
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
		bool checkDerivative(double value, I guess, J derivative); // replace updateAscent for debugging purposes

	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true>
		bool updateGotNAN(I guess); // update guess, sets derivative to zero for convinience
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
		bool updateAscent(double value, I guess, J derivative, SparseMatrix<double>* addout = NULL, int rowout=0); // update guess, sets derivative to zero for convinience
	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
		bool updateAscentPositiveDomain(double value, I guess, J derivative, SparseMatrix<double>* addout = NULL, int rowout=0); // update guess, sets derivative to zero for convinience
//	template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true, ENABLEIF_WRITE_ITERATOR_ANYKEY(J) =true>
//		bool updateAscentPositiveDomainMask(double value, I guess, J derivative, myHashmap<uint32_t>); // update guess, sets derivative to zero for convinience

	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true> CurvatureSearchScope& initInvCurv(I inv_curvature);
	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true> void wrInvCurv(I inv_curvature) const;
	template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I) =true> void wrFinalGuess(I guess_to_write) const;

//	void init(unsigned int in_size, LearnableParameter* guess);

	// void initMK2(unsigned int in_size, const double * dderiv); // uses d2F/dx2 only!
    unsigned int  getSize()const{return size;}

    double makeGuessPositive(uint32_t index);
    void makeGuessInRange(double& curval, uint32_t index, double abs_max);
    void makeGuessStrictlyPositive(double& curval, uint32_t index);
	...
	bool updateAscent(const double &value, double * guess,  double const * const deriv, SparseMatrix<double>* addout = NULL, int rowout=0); // returns a safe bound on the parameter changes;
...
//	double updateAscentMK2(const double &value, double * guess,  double const * const deriv,  double const * const dbl_deriv); // returns a safe bound on the parameter changes;
	...
	uint32_t getNbStalledSteps() const{return shrinkcount;}
	void wrFinalGuess(double* guess)const;
	void wrDDeriv(double* dderiv)const; // save diago for future fuzzy search
...
public:
    ...
    SerialStore(SerialStore&&) = delete;
    SerialStore& operator=(const SerialStore&) = delete;
    SerialStore& operator=(SerialStore&&) = delete;
	...
	SerialStore<IDTYPE>& setNoOverwriteOnDestruction(){if (path) {delete[](path); path = NULL;} return *this;}
...
public:
	...
	ERRCODE cluster_likelihood(const TMatrix< double > &data, double prior_scale = 0.125); // assumes the number of dimensions is low < 16

	class Task_cluster_likelihood_bruteforce; // uses 0 or 1 dim to reduce searchspace
	ERRCODE cluster_likelihood_bruteforce(const SparseMatrix< double > &data, TMatrix<double> &proj, bool use_one = false);
...
template<class C> class Hello{
	public:
    C operator[](int hehe) { return C(); }
};

template< >
class Hello<double>{
	public:
	double operator[](int hehe) { return 0.0; }
};
...
class Forest<NODE,nbrel>::TaskClustering : public Event<uint32_t>{
    ...
    template<class TB> void run(TB& foreach);
};*/
