...
LFH_typename_check_dep(KEYITERATOR_TYPE, void);
LFH_typename_check_dep(KEYITERATORMKR_TYPE, void);
...
LFH_typename_check_dep(IsPOD, std::false_type );
...
public:
    ...
    // Item iterator class type indicator for the Key, (Data can be any type, but Const/non-const for is marks the Key instead)
	typedef typename isdef_KEYITERATOR_TYPE<sizeof(doexist_KEYITERATOR_TYPE<A>(0)),A>::TYPE KEYITERATOR_TYPE;
	typedef typename isdef_KEYITERATORMKR_TYPE<sizeof(doexist_KEYITERATORMKR_TYPE<A>(0)),A>::TYPE KEYITERATORMKR_TYPE;
    ...
    typedef typename isdef_IsPOD<sizeof(doexist_IsPOD<A>(0)),A>::TYPE IsPOD;
    ...
    typedef Logarithm<A> LOGARITHM_TYPE;
    ...
    //static const bool IsPOD = Exlisten_load<A, ERRCODE (A::*)(const uint8_t* &data, const uint8_t * const endpos)>::ans;
    static const bool NeedsAddLink = IS_OWNED::value; // containers needs to update addresses in link registers


    template<class B> static inline A& toConversion(A& a, const B& b){return a = (A)b;} // tries implicit conversion if no explicit exists...

    static bool isValid(const A& val){return true;}
    static A& toInvalid(A& val){return val;}
    ...
    static const A& show(const A& val, FILE* out = stdout, int level = 0){val.show(out,level); return val;}
    ...
    static Magscaled<A> mkMagscaled(const A &a){Magscaled<A> fout; fout.value = a; fout.exponent = 0.0f; return fout;}

    static ERRCODE save(const A& what, FILE *f) {return (fwrite(&what,sizeof(A),1,f) == 1) ? 0 : 1;}
    static ERRCODE load(A& what, FILE *f){return (1 == fread(&what,sizeof(A),1,f)) ? 0 : 1;}
...
/*
template<class A>
class ExCo<Alias<A>, 0u>{
public:
	typedef std::integral_constant<bool, ExCo<A>::IsPOD::value > IsPOD;
	typedef ExCo< Alias<A> > SAFETYPE;

	int32_t mkHashValue(const Alias<A>& a);
	Alias<A> mkZero();
};*/
...
public:
    ...
    typedef typename StConstList<sizeof(A)>::INTEGER_CLASS INDEX_TYPE;
	typedef std::true_type IsPOD;
    static const A& show(const A& what, FILE*f, int level){fprintf(f, "enum%i\n", (int32_t)what); return what;}
...
public:
    ...
    static A* & toMemfree(A* & a){return (a =NULL);} // delete(a); naked pointers does not free (could be void* or redundant pointers)
	...
	static ERRCODE load(A * const & what, FILE *f);
	static A * const & show(A * const & what, FILE *f,int level);
};
...
template<class A>
class ExCo< std::function<unsigned int (A)> , 0>{
public:
	typedef ExCo< std::function<unsigned int(A)> , 0> SAFETYPE;
	typedef std::false_type IsPOD;
	static std::function<unsigned int(A)>& toMemmove(ExCo<std::function<unsigned int(A)> >& a, ExCo<std::function<unsigned int(A)> >& b){a = b; b =std::function<unsigned int(A)>(); return a;}
	static inline bool isGT(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return true;}
	static inline bool isGE(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return false;}
	static inline bool isLT(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return true;}
	static inline bool isLE(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return false;}
	static inline bool isEQ(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return false;}
	static inline bool isNQ(const std::function<unsigned int (A)> &a, const std::function<unsigned int (A)> &b){return true;}
};
...
public:
	...
	static ERRCODE save(const char* const & what, FILE *f) {if (what != NULL) fwrite(what,1,strlen(what)+1,f); else {int tmp=0; fwrite(&tmp,1,1,f);}return 0;}
	static ERRCODE load(char* & what, FILE *f) {ERRCODE fout=0; char buffer[65536]; char* tmp = buffer; do {fread(tmp,sizeof(unsigned char), 1,f); } while(*(tmp++) != '\0'); what = new char[(unsigned int)(tmp - buffer)]; memcpy(what, buffer, sizeof(char) * (unsigned int)(tmp - buffer)); return fout;}
	int32_t mkHashValue(const char* s) {int32_t h = 0; for ( ; *s; ++s) h = 5*h + *s; return h;}
	static const char* const & show(const char* const & what, FILE *f,int level) { if (level == 0) fprintf(f,"%s\n",what); else fprintf(f,"%s",what);return what;}
};
template< >
class ExCo<const char*>{
public:
//typedef YESNO<false> IsPOD;
	typedef std::false_type IsPOD;
	static const bool NeedsAddLink = false;
	...
	static const char* & toMemmove(const char*& a, const char* & b){a = b; b =NULL; return a;}
    static const char* & toMemswap(const char*& a, const char* & b){const char* tmp = b; b = a; return a = tmp;}
	int32_t mkHashValue(const char* s) {uint32_t h = 0; for ( ; *s; ++s) h = 5*h + *s; return h;}
...
//        typedef typename ExCo<A>::SAFETYPE[SIZE] TYPE;
	...
	//	typedef typename ExCo<A>::DERIVATIVE_TYPE DERIVATIVE_TYPE[SIZE];
		...
		static const A (&show(const A (&a)[SIZE], FILE* out=stdout, int level=0)) [SIZE];

		static bool isValid(const A (&a)[SIZE]);
		...
		int32_t mkHashValue(const A (&a)[SIZE]);

		inline static ERRCODE save(const A (&a)[SIZE], FILE *f);
		inline static ERRCODE load(A (&a)[SIZE], FILE *f);
	};


template< class R, class A>
class ExCo< R (*)(A), 0 >{ // function that returns R with arg A
    public:
	typedef std::false_type IsPOD;
...
public:
	typedef std::integral_constant<bool, ExCo<A>::IsPOD::value && ExCo<B>::IsPOD::value > IsPOD;
...
//	typedef pair< typename ExCo<A>::MAGNITUDE_TYPE, typename ExCo<B>::MAGNITUDE_TYPE> MAGNITUDE_TYPE;
	...
	static const pair<A,B>& show(const pair<A,B> &a, FILE* out=stdout, int level=0);
	static string type_tostring(const pair<A,B> &a);
	static ERRCODE save(const pair<A,B> &a, FILE*f);
	static ERRCODE load(pair<A,B> &a, FILE*f);
};
...
//	typedef typename ExCo<A>::MAGNITUDE_TYPE MAGNITUDE_TYPE;
	...
	static inline const A & show(const A &a, FILE* out=stdout, int level=0);
...
//	typedef short MAGNITUDE_TYPE;
	...
	static double mkRand(){double a = (((unsigned int)rand()) ^ ((unsigned int)(rand() << 12)) ^ ((unsigned int)(rand() << 24))) * pow(0.5f, 32); return a;  };
	inline static double mkAngle(const double &a){return std::signbit(a) ? M_PI : 0.0;}
    static Magscaled<double> mkMagscaled(const double &a){Magscaled<double> fout; int expval; fout.value = std::frexp(a, &expval); fout.exponent = (M_LN2 * expval); return fout;}

	inline static double mkLog(const double& what){return log(what);}
	inline static double mkLogarithm(const double& what){return log(what);}
	...
	static bool isValid(const double& a){return isfinite(a);}
	static bool isNegative(const double& a){return signbit(a);}
	...
	static const double & show(const double &val, FILE* out=stdout, int level=0){fprintf(out, "%e", val); if (level == 0) fprintf(out, "\n");return(val); }
	...
	static ERRCODE load(double & what, FILE *f) {return fread(&what,sizeof(double), 1,f) == 1 ? 0 : 1;}
...
//	typedef short MAGNITUDE_TYPE;
	...
	inline static float& toMaximum(float &a){a = FLT_MAX; return a;}
	inline static float& toMinimum(float &a){a = -FLT_MAX; return a;}
   ...
   // static float toAbsoft(const float &a, const double& k){float fout = fabs(a); return (fout > k * 16) ? fout : a / k  }
	...
	inline static double mkAngle(const float& what){return std::signbit(what) ? M_PI : 0.0;}
    static Magscaled<float> mkMagscaled(const float &a){Magscaled<float> fout; int expval; fout.value = std::frexp(a, &expval); fout.exponent = ((float)M_LN2) * expval; return fout;}
	inline static float mkLog(const float& what){return log(what);}
	inline static float mkLogarithm(const float& what){return log(what);}
	...
	static bool isValid(const float& a){return isfinite(a);}
	static bool isNegative(const float& a){return signbit(a);}
    ...
    static void show(const double &val, char* &ptr, int level=0){ptr += sprintf(ptr, "%e", val); if (level == 0) *(ptr++) = '\n';}
	static const float& show(const float &val, FILE* out=stdout, int level=0){fprintf(out, "%e", val); if (level == 0) fprintf(out, "\n"); return(val);}
	...
	static ERRCODE load(float & what, FILE *f) {return fread(&what,sizeof(float), 1,f) == 1 ? 0 : 1;}
...
//	typedef unsigned char MAGNITUDE_TYPE;
    ...
    static bool isNegative(const bool &a){return false;}
	...
	static const bool & show(const bool &val, FILE* out=stdout, int level=0){fprintf(out, "%c", val ? 'T' : 'F'); if (level == 0) fprintf(out, "\n"); return val;}
	...
	static ERRCODE load(bool& what, FILE *f) {return fread(&what,sizeof(char), 1,f) == 1 ? 0 : 1;}

};


template< > class ExCo<char>{
	public:
//		typedef unsigned char MAGNITUDE_TYPE;
		typedef void DERIVATIVE_TYPE;
    ...
    typedef char TYPE;
        ...
        static bool isNegative(const char &a){return (a & 0x80) != 0;}
		...
		static char mkRand(){char a = (rand() & 255) ; return a;}
        inline static double mkAngle(const char& a){const double factor = ldexp(M_PI, -7);  return factor * a;}
		...
		static const char& show(const char& val, FILE* out=stdout, int level=0){fprintf(out, "%c", val); if (level == 0) fprintf(out, "\n"); return val;}
		...
		static ERRCODE load(char& what, FILE *f) {return fread(&what,sizeof(char), 1,f) == 1 ? 0 : 1;}
    ...
    typedef uint8_t TYPE;
        ...
        static bool isNegative(const uint8_t &a){return false;}
		...
		static uint8_t mkRand(){uint8_t a =  (rand() & 255) ;return a;};
        inline static double mkAngle(const uint8_t & a){const double factor = ldexp(M_PI, -7);  return factor * a;}
        ...
        static const uint8_t & show(const uint8_t &val, FILE* out=stdout, int level=0){fprintf(out, "%i", val); if (level == 0) fprintf(out, "\n"); return val;}
		...
		static ERRCODE load( uint8_t& what, FILE *f) {return fread(&what,sizeof(uint8_t), 1,f) == 1 ? 0 : 1;}
		...
		static int32_t mkHashValue(const uint8_t& a){return a;}
...
//		typedef unsigned char MAGNITUDE_TYPE;
        ...
        static bool isNegative(const int16_t &a){return (a & 0x8000) != 0;}
        static bool isZero(const int16_t &a){return a == 0;}
        static bool isOne(const int16_t &a){return a == 1;}
		static int16_t& toZero(int16_t & a){return a = 0;}
		static int16_t& toOne(int16_t & a){return a = 1;}
		static int16_t& toRand(int16_t & a ){ a = rand() ^ (rand() << 12);return a;};
		static int16_t mkRand(){int16_t a = rand() ^ (rand() << 12);return a;};
        inline static double mkAngle(const int16_t & a){const double factor = ldexp(M_PI, -15);  return factor * a;}

		static int16_t& toMinimum(int16_t& a) {a=-32768;return a;}
		static int16_t& toMaximum(int16_t& a) {a=32767;return a;}
        static void epsilon(int16_t & a){a = 1;}
		static void delta(int16_t & a){a = 1;}

        static double sign(const int16_t& val){return(0.0f);}
		...
		static const int16_t & show(const int16_t &val, FILE* out=stdout, int level=0){fprintf(out, "%i", val); if (level == 0) fprintf(out, "\n"); return val;}
        static string type_tostring(const short & a){return string("short");}


		static ERRCODE save(const int16_t& what, FILE *f) { return (fwrite(&what,sizeof(int16_t), 1,f) == 1) ? 0 : 1;}
		static ERRCODE load(int16_t& what, FILE *f) {return fread(&what,sizeof(int16_t), 1,f) == 1 ? 0 : 1;}

		static void bitreverse(int16_t& a){
			a = ((a << 8 ) & 0xFF00) | ((a >> 8)& 0x00FF);
			a = ((a << 4 ) & 0xF0F0) | ((a >> 4)& 0x0F0F);
			a = ((a << 2 ) & 0xCCCC) | ((a >> 2)& 0x3333);
			a = ((a << 1 ) & 0xAAAA) | ((a >> 1)& 0x5555);
		}
		static void bytereverse(int16_t& a){
			a = ((a << 8 ) & 0xFF00) | ((a >> 8)& 0x00FF);
		}
		static int32_t mkMagnitude(const int16_t& a){
				int32_t f;
				int16_t r;
			...
			return f;}
		static int32_t mkHashValue(const int16_t& a){return a;}
		template<unsigned int SIZE> static unsigned int getHyperDirection(const int16_t (&a)[SIZE], const int16_t (&b)[SIZE]){ // direction for hyper-ordering of arrays
			uint16_t xorb,xorc;
			*(uint16_t*)(&xorb) = a[0] ^ b[0]; // xor and make unsigned
			unsigned int best =0;
			for(unsigned int cur=1;cur<SIZE;cur++) {
				*(uint16_t*)(&xorc) = a[cur] ^ b[cur]; // xor and make unsigned
				if ((xorb <= xorc)&&((xorb & xorc) <= (xorb ^ xorc))) {best = cur; xorb = xorc;}
			}
		return(best);}
		inline static int16_t& toRightFlood(int16_t& a){a |= a << 8;a |= a << 4; a |= a << 2; return a |= a << 1;}
		inline static int16_t& toLeftFlood(int16_t& a){a |= a >> 8;a |= a >> 4; a |= a >> 2; return a |= a >> 1;}
		inline static int16_t& toBitInverse(int16_t& a){return a ^= 0xFFFF;}
	};


template< > class ExCo<unsigned short>{
	public:
        ...
        static bool isNegative(const unsigned short &a){return false;}
		...
		static unsigned short mkRand(){unsigned short a = rand() ^ (rand() << 12);return a;}
        inline static double mkAngle(const uint16_t & a){const double factor = ldexp(M_PI, -15);  return factor * a;}
		...
		static const uint16_t& show(const uint16_t &val, FILE* out=stdout, int level=0){fprintf(out, "%u", val); if (level == 0) fprintf(out, "\n"); return val;}
		...
		static ERRCODE load( unsigned short& what, FILE *f) {return fread(&what,sizeof(unsigned short), 1,f) == 1 ? 0 : 1;}
		...
		static int32_t mkHashValue(const uint16_t& a){return a;}
...
template< > class ExCo<int32_t>{
	public:
        ...
        static bool isNegative(const int32_t &a){return (a & 0x80000000) != 0;}
		...
		static int32_t mkRand(){int32_t a = rand() ^ (rand() << 12) ^ (rand() << 24); return a;};
        inline static double mkAngle(const int32_t & a){const double factor = ldexp(M_PI, -31);  return factor * a;}

		static double sign(const int32_t& val){return(val >= 0 ? 1.0f : -1.0f);}
		static const int32_t & show(const int32_t &val, FILE* out=stdout, int32_t level=0){fprintf(out, "%i", val); if (level == 0) fprintf(out, "\n"); return val;}
		...
		static ERRCODE load(int32_t& what, FILE *f) {return fread(&what,sizeof(int32_t), 1,f) == 1 ? 0 : 1;}
		...
		static int32_t mkHashValue(const int32_t& a){return a;}
...
public:
	...
	static bool isNegative(const uint32_t &a){return false;}
	...
	static uint32_t mkRand(){uint32_t a = rand() ^ (rand() << 12) ^ (rand() << 24);return a;}
	inline static double mkAngle(const uint32_t & a){const double factor = ldexp(M_PI, -31);  return factor * a;}
	...
	static const uint32_t& show(const uint32_t&val, FILE* out=stdout, int level=0){fprintf(out, "%u", val); if (level == 0) fprintf(out, "\n"); return val;}
	...
	static int32_t mkMagnitude(const uint32_t& a){
		...
		return f;}
	static int32_t mkHashValue(const uint32_t& a){return a;}
...
template class ExCo<uint32_t>;
	...
	public:
        ...
        static bool isNegative(const int64_t &a){return (a & 0x8000000000000000) != 0;}
    ...
    #ifdef __MINGW32__
		static const int64_t & show(const int64_t &val, FILE* out=stdout, int64_t level=0){fprintf(out, "%" PRId64 "i", val); if (level == 0) fprintf(out, "\n"); return val;}
    #else
		static const int64_t & show(const int64_t &val, FILE* out=stdout, int64_t level=0){fprintf(out, "%lli", (long long)val); if (level == 0) fprintf(out, "\n");return val; }
    #endif
		...
		static ERRCODE load(int64_t& what, FILE *f) {return fread(&what,sizeof(int64_t), 1,f) == 1 ? 0 : 1;}
		...
		static int32_t mkHashValue(const uint64_t& a){return a ^ (a >> 32);}
...
typedef void DERIVATIVE_TYPE;
	...
	static bool isNegative(const uint64_t &a){return false;}
...
#ifdef __MINGW32__
	static const uint64_t& show(const uint64_t &val, FILE* out=stdout, int level=0){fprintf(out, "%" PRIu64 "", val); if (level == 0) fprintf(out, "\n"); return val; }
#else
	static const uint64_t& show(const uint64_t &val, FILE* out=stdout, int64_t level=0){fprintf(out, "%llu", (unsigned long long)val); if (level == 0) fprintf(out, "\n");return val; }
#endif
...
template< > class ExCo<std::string >{
public:
	typedef string TYPE;
	typedef std::false_type IsPOD;
	static const bool NeedsAddLink = false;
	...
	static const string& show(const string& what, FILE *f,int level) {
		if (what.length() == 0){
	...
	return what;}
	static int32_t mkHashValue(const string& __s){   int32_t h = 0;
		const char* tmp = __s.c_str();
		for ( ; *tmp; ++tmp)
		h = 5*h + *tmp;
	return h;}
	static string getIndex(const string& w){return w;}
	static string& getIndex(string& w){return w;}

	static ERRCODE save(const string& w, uint8_t * &chunk, const uint8_t * const endpos){if ((uint32_t)(endpos - chunk) <= w.length()) return 1; memcpy(chunk, w.c_str(), w.length()+1); chunk += w.length()+1; return 0;}
	static ERRCODE load(string& w, const uint8_t * &chunk, const uint8_t * const endpos){const uint8_t* t = chunk; while(t != endpos) if ((*t) == 0) break; if (t == endpos) return 1; w = string((const char*)chunk); chunk = t+1; return 0;}
	...
	static ERRCODE load( string& what, FILE *f) { char buffer[65536]; char* tmp = buffer; do {if (fread(tmp,sizeof(unsigned char), 1,f) != 1) return 1; } while(*(tmp++) != '\0'); what = string(buffer);  return 0;}
};
	...
	class ExCo<SETCMP_enum>{
        public:
		static const SETCMP_enum& show(const SETCMP_enum& what, FILE *f,int level) {
            switch(what){
...
template<class A> class ExCo<std::function<void(A)>, 0u >{
	public:
	...
	static const std::function<void(A)>& show(const std::function<void(A)>& what, FILE *f,int level) {
		fprintf(f,"functor\n");
	return what;}
};

template<class O, class A>
class ExCo<std::function<O(A)>, 0u >{
	public:
	typedef A INDEX_TYPE;
	typedef O INNER_TYPE;
	typedef ExCo<std::function<O(A)>, 0u > SAFETYPE;
	typedef std::false_type IsPOD;
	static std::function<O(A)>& toZero(std::function<O(A)>& what){return what;}
	static std::function<O(A)>& toMemmove(std::function<O(A)>& what, std::function<O(A)>& from){return what = from;}
	static const std::function<O(A)>&  show(const std::function<O(A)>& what, FILE *f,int level) {
		fprintf(f,"functor\n");
	return what;}
};
...
LFHTEMP bool ExCo<A[SIZE],0>::isValid(const A (&a)[SIZE]){ uint32_t i;
    for(i=0;i<SIZE;i++) if (!ExOp::isValid(a[i])) break;
return i == SIZE;}
...
LFHTEMP const A (&ExCo<A[SIZE],0>::show(const A (&a)[SIZE], FILE* f_out, int l))[SIZE]{
	switch(l){
		case 0:
...
return fout;}
LFHTEMP ERRCODE ExCo<A[SIZE],0>::load(A (&a)[SIZE], FILE *f){ ERRCODE fout=0;
	for(unsigned int i=0;i<SIZE;i++) fout |= ExOp::load(a[i], f);
return fout;}
LFHTEMP int32_t ExCo<A[SIZE],0>::mkHashValue(const A (&a)[SIZE]){
	int32_t fout = ExOp::mkHashValue(a[0]);
	for(unsigned int i=1;i<SIZE;i++) fout = fout * 5 + ExOp::mkHashValue(a[i]);
return fout;}
...
LFHTEMP A * const & ExCo<A*,0>::show(A * const & what, FILE *f,int level) {ExOp::show(*what,f, level);return what;}
LFHTEMP ERRCODE ExCo<A*,0>::save(A * & what, FILE *f) {return ExOp::save(*what,f);}
LFHTEMP ERRCODE ExCo<A*,0>::load(A * const & what, FILE *f) {return ExOp::load(*what,f);}
...
LFHTEMP const A & ExCo<const A,0>::show(const A &a, FILE* fout, int level){ExCo<A>::show(a,fout,level);return a;}
LFHTEMP bool ExCo<const A,0>::isValid(const A& a){return(ExCo<A>::isValid(a));}

//LFHTEMP int32_t ExCo<Alias<A>,0>::mkHashValue(const Alias<A>& a){return ExCo<A>::mkHashValue(a.value);}
//LFHTEMP Alias<A> ExCo<Alias<A>,0>::mkZero(){return Alias<A>(ExCo<A>::mkZero());}
...
LFHTEMP const pair<A,B> & ExCo<pair<A,B>,0>::show(const pair<A,B> &a, FILE* f_out, int l){
		switch(l){
		case 0:
...
LFHTEMP ERRCODE ExCo<pair<A,B>,0>::load(pair<A,B> &a, FILE*f){return ExOp::load(a.first,f) | ExOp::load(a.second,f);}
...
return *this;}
LFHTEMP Magscaled<C> Magscaled<C>::operator+(const Magscaled<C> &other)const{  Magscaled<C> fout;
    if (exponent > other.exponent) {
        fout.value = value + other.value * exp(other.exponent - exponent);
        fout.exponent = exponent;
    }else{
        fout.value = value * exp(exponent - other.exponent) + other.value;
        fout.exponent = other.exponent;
    }
return fout;}
LFHTEMP Magscaled<C> Magscaled<C>::operator-(const Magscaled<C> &other)const{  Magscaled<C> fout;
    if (exponent > other.exponent) {
...
LFHTEMP const Magscaled<C>& Magscaled<C>::show(FILE*f, int level)const{fprintf(f,"%e * e^%e%c", value, exponent, (level ==0) ? '\n' : '\t');return *this;}


#undef LFHTEMP
#define LFHTEMP template<class C, ANNOTATED_TYPE_enum A>
LFHTEMP int32_t Annotype<C,A>::mkHashValue()const{return ExCo<C>::mkHashValue(value);}
LFHTEMP const Annotype<C,A>& Annotype<C,A>::show(FILE *f, int level) const{
	fprintf(f,"[alias "); ExOp::show(value, f,level+1);
