...
void myexit_command(const char *file, int line, int errocode){
	if (errocode) fprintf(stderr,"ErrorCode %i outputed from %s[%d] at t=%i:\n", errocode, file, line, (uint32_t)clock());
	else fprintf(stderr,"Process successfully ended from %s[%d] at t=%i:\n", file, line, (uint32_t)clock());
...
void myexit_command(const char *file, int line, const char *__format, ...) {
	fprintf(stderr,"Exiting from %s[%d] at t=%i:\n", file, line, (uint32_t)clock());
    va_list __va; va_start(__va, __format);
    std::vfprintf(stderr, __format, __va);
    va_end(__va);
    fflush(stderr);
	#ifndef USE_EMSCRITEN
		throw std::exception();
...
// forced class instantiations

Anything::Anything(uint8_t* _ptr, uint32_t _anytype): ptr(_ptr), anytype(_anytype){}
AnythingConst::AnythingConst(const uint8_t* _ptr, uint32_t _anytype): ptr(_ptr), anytype(_anytype){}
...
LFH_GOLD double hypot(const double a, const double b, const double c) {
    int expons[3];
    double da = frexp(a, expons); da *= da; if (da == 0.0) expons[0] = -1000;
    double db = frexp(b, expons + 1); db *= db; if (db == 0.0) expons[1] = -1000;
    double dc = frexp(c, expons + 2); dc *= dc; if (dc == 0.0) expons[2] = -1000;
    int dif[2];
    if ((dif[0] = expons[0] - expons[1]) >= 0){
        if ((dif[1] = expons[2] - expons[0]) >= 0){
            da = ldexp(da, -dif[1] << 1);
            db = ldexp(db, (-dif[1] - dif[0]) << 1);
            return ldexp(sqrt(da + db + dc), expons[2]);
        }else{
            dc = ldexp(dc, dif[1] << 1);
            db = ldexp(db, -dif[0] << 1);
            return ldexp(sqrt(da + db + dc), expons[0]);
        }
    }else{
        if ((dif[1] = expons[2] - expons[1]) >= 0){
            da = ldexp(da, (-dif[1] + dif[0]) << 1);
            db = ldexp(db, -dif[1] << 1);
            return ldexp(sqrt(da + db + dc), expons[2]);
        }else{
            da = ldexp(da, dif[0] << 1);
            dc = ldexp(dc, dif[1] << 1);
            return ldexp(sqrt(da + db + dc), expons[1]);
        }
    }
}
LFH_GOLD double hypot_unsafe(const double a, const double b, const double c) {return sqrt(a*a + b*b + c*c);}
...
int safestrlen(const char *str, int maxlength){
	...
	for(;maxlength != 0; maxlength--){
		if (*(cur++) == '\0') return ((int)(cur - str))-1;
	}
return (int)(cur - str);}

double Zscore_to_LogQuartile(const double z){ // to CHECK
	if (z > 14.0){
		return  exp( -0.5 * z*z) * ((1.0/ z*z) - 1.0) / (sqrt(M_2PI) * z);
	}else if (z > -14.0){
		return log(erfc(-z /sqrt(2.0))) - log(2.0);
	}else{
		return -0.5 * z*z + log((1.0 - (1.0/ z*z)) / (sqrt(M_2PI) * z));
	}
}

double logQuantile_to_ZSscore(double logQuantile){
	return logitPval_to_Zscore(logQuantile - log_1_me_x(logQuantile));
}
double quantile_to_ZSscore(double quantile){
	return logitPval_to_Zscore(log(quantile) - log(1.0 - quantile));
}
...
double logP_to_Gamma(const double scale, const double shape, const double logP){return 0;} // TODO
