...
template<class C> void Bluestein::resample(C* iobuf, Bluestein* invblue) const{
	...
	template<int nbstate>
	ERRCODE HMM<nbstate>::load(FILE* f){
...
LFHTEMP double GaussElem< Tuple<C, SIZE>, 0 >::likelihoodratio_dist(const GaussElem< Tuple<C, SIZE>, 0u > &other, bool has_weight, bool has_covariance) const{
...
LFHTEMP template<unsigned int S_SIZE> GaussElem< Tuple<C, S_SIZE>, 0u > GaussElem< Tuple<C, SIZE>, 0 >::makeResidualGauss(const Tuple<unsigned int, S_SIZE>& s_d)const{ GaussElem< Tuple<C, S_SIZE>, 0u > fout;
	...
	LFHTEMP double GaussElem< Tuple<C, SIZE>, 0 >::Pvalue_Hotelling(const GaussElem< Tuple<C, SIZE>, 0u > &other, bool LogPvalue) const{
		...
		if ((mean.getSize() == 0)||(w == 0)||(other.w == 0)) return (LogPvalue) ? 0.0f : 1.0f; // invalid, or no data for one of the two;
...
LFHTEMP GaussElem< Tuple<C, SIZE>, 0 >& GaussElem< Tuple<C, SIZE>, 0 >::addGaussElem_free_mean(const GaussElem< Tuple<C, SIZE>, 0u > &other){
	if (w == 0.0f){
...
LFHTEMP ERRCODE GaussElem< Tuple<C, SIZE>, 0 >::load(FILE *f){
...
LFHTEMP GaussElem< Tuple<C, 0u>, 0u >& GaussElem< Tuple<C,0u>, 0 >::operator+=(const KeyElem<double, SparseTuple<C> > &vec){
    w += vec.k; w2 += vec.k;
    C* currow;
    if (auto ite = vec.d.mkIterator()) do{
        mean[ite()] += *ite;
        currow = cov.data + (ite()*(ite()+1) >> 1);
        if (auto ite2 = vec.d.mkIterator()) do{
...
LFHTEMP GaussElem< Tuple<C, 0u>, 0 >& GaussElem< Tuple<C, 0u>, 0 >::addSample(const Tuple<C, 0u> &o, double _w){
    unsigned int i,j,k;
    for(j=0,k=0;j<mean.getSize();j++) {
        C tmp = ExOp::mkTrju(o[j]) * _w;
        mean[j] += o[j] * _w;
        for(i=0;i<=j;i++,k++) cov.data[k] += tmp * o[i];
    }
    w += _w; w2 += _w * _w;
    ExOp::toZero(determinant);
return *this;}
LFHTEMP GaussElem< Tuple<C, 0u>, 0 >& GaussElem< Tuple<C, 0u>, 0 >::addSample(const C* o, double _w){
    unsigned int i,j,k;
    for(j=0,k=0;j<mean.getSize();j++) {
        C tmp = ExOp::mkTrju(o[j]) * _w;
        mean[j] += o[j] * _w;
    ...
    ExOp::toZero(determinant);
return *this;}
LFHTEMP Trianglix<C, 0u> GaussElem< Tuple<C,0u>, 0 >::getKernelDensity_Silverman(double factor)const{return getCovariance_biased() *= (factor * pow(0.25 * w * (mean.getSize()+ 2), -2.0 / (mean.getSize()+4)));}
...
LFHTEMP GaussElem< Tuple<C, 0u>, 0 >& GaussElem< Tuple<C,0u>, 0 >::addGaussElem_free_mean(const GaussElem< Tuple<C, 0u>, 0u > &other){
	if (w == 0.0f){
		w=other.w; w2=other.w2; mean = 	other.mean; cov = other.cov;
	}else if (other.w != 0){
...
LFHTEMP void GaussElem< Tuple<C,0u>, 0 >::runTaskPartialObservationMaximizeMK2(const SparseMatrix<double>& data, uint32_t nbrow, uint32_t itemax, double outlier_frequency, Tuple<double,2u> *outlier_prior, bool include_uncertainty , Vector<double>* weight){
    runTaskPartialObservationMaximizeMK2_task tscp(data, *this, outlier_frequency, foreach.getThreadArraySize(), include_uncertainty);
    uint32_t i,j,k;
    ...
    for(i=0;i<foreach.getThreadArraySize();i++) tscp.stats[i].setSize(k);
    double totweight;
    double* tmpweight;
    if ( weight == NULL) {
    ...
    for(i=foreach.getThreadArraySize()-1;i>0;i--) foreach.submit(tscp , i);
    foreach.submit_ThenWait(tscp, 0);

    if (tscp.thrprint != 0xFFFFFFFF) tscp.progb.finish();



    for(i=foreach.getThreadArraySize()-1;i>0;i--) {
    ...
    for(uint32_t loopy=0;loopy<itemax;loopy++){
        ...
        if (unknown.EMprocess(tscp.ll_or_prob(), totalLL, tscp.ll_or_prob.getSize(), outlier_prior)){
            ...
            foreach.submit_ThenWait(tscp, 0);
            if (tscp.thrprint != 0xFFFFFFFF) tscp.progb.finish();
            for(i=foreach.getThreadArraySize()-1;i>0;i--) tscp.stats[0] += tscp.stats[i];
...
LFHTEMP template<class F> GaussElem< Tuple<C,0u>, 0 >::TaskStochasticMaximize::TaskStochasticMaximize(ThreadBase &_tb, GaussElem< Tuple<C, 0u>, 0u > &_cur, const SparseMatrix<double>& _data, F f, uint32_t nbrow, uint32_t itemax,  double outlier_frequency, Tuple<double,2u> *outlier_prior, bool _include_uncertainty, Vector<double>* weight): tb(_tb), data(_data), cur(_cur),include_uncertainty(_include_uncertainty){
  //  suff_stats.setSize(nbthread);
    ...
    for(j=0; j< nbrow*3;j+=3){
        ...
        _cur.mean[(j/3)] = stats[0][j+1] / stats[0][j];
        _cur.cov.data[((j/3) * ((j/3)+3)) >> 1] = (stats[0][j+2] - _cur.mean[(j/3)] * stats[0][j+1]) / stats[0][j];
    }

    if ((!ExOp::isValid(_cur.cov))||(!ExOp::isValid(_cur.mean))) {
        printf("got NAs!\n");
        _cur.mean.show();
        _cur.cov.show();
        return;
    ...
    baseLL = -0.5 * (_cur.cov.log_determinant() + log(M_2PI) * _cur.cov.getSize());
    UnknownScope unknown(outlier_frequency);
    double totalLL;
    ...
    Tuple<double> daden;
    for(uint32_t loopy=0;loopy<itemax;loopy++){
        precision_matrix = _cur.cov.mkInverse();
        ...
        if (unknown.EMprocess(ll_or_prob(), totalLL, ll_or_prob.getSize(), outlier_prior)){
            ...
            for(k=0;k< nbrow;k++) {
                meannew[k] = (meanstat[k] + stats[0][k] + _cur.mean[k] * (totweight - weightstat[k])) / totweight;
                _cur.mean[k] = meannew[k] - _cur.mean[k];
            }
            for(j=0,k=0; j< nbrow;j++){
                for(i=0;i<=j;i++,k++) _cur.cov.data[k] = (stats[0][k+nbrow] / totweight) - _cur.mean[i] * _cur.mean[j];
            }
            _cur.mean.toMemmove(meannew);
            if ((!ExOp::isValid(_cur.cov))||(!ExOp::isValid(_cur.mean))) {
                printf("got NAs!\n");
                _cur.mean.show();
                _cur.cov.show();
...
LFHTEMP ERRCODE GaussElem< Tuple<C,0u>, 0 >::load(FILE *f){
	if (fread(&w,sizeof(double),1,f) !=1 ) return 1;
	if (fread(&w2,sizeof(double),1,f) !=1 ) return 1;
	if (ExOp::load(determinant,f)) return 1;
	if (ExOp::load(mean,f)) return 1;
...
//template<class C> void AdaptiveLearningScope::addDeriv(const char* target, C & deriv){*(C*)buffer[string(target)] += deriv;}

template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> CurvatureSearchScope& CurvatureSearchScope::initWithSecondGuess(I sec_guess){ // *must be different to initial guess, and both are within the search space domain*
	Vector<double> localbuffer;
	if (auto ite = sec_guess.mkIterator()) do{ localbuffer.push_back(*ite);*ite = 0.0;}while(ite++);
	size = localbuffer.getSize(); epsilon = pow(0.5, 53);
    if (lastderiv != NULL) delete[](lastderiv);
    lastderiv = NULL;
    curvature.setSize(size << 1);
    for(int i=0;i<size;i++) curvature[i << 1] = localbuffer[i];
    shrinkcount =1;
return *this;}

template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> CurvatureSearchScope& CurvatureSearchScope::initWithSecondDerivative(I sec_derivative){ // *must be different to initial guess, and both are within the search space domain*
	Vector<double> localbuffer;
	if (auto ite = sec_derivative.mkIterator()) do{ localbuffer.push_back(*ite);*ite = 0.0;}while(ite++);
	size = localbuffer.getSize(); epsilon = pow(0.5, 53);
    if (lastderiv != NULL) delete[](lastderiv);
    lastderiv = NULL;
    ...
    unsigned int i,j,k;
    for(i=0,k=0;i<size;i++) {
        curvature[k++] = fabs(1.0 / localbuffer[i]);
        for(j=i+1;j<size;j++) curvature[k++] = 0.0;
    }
    shrinkcount =0;
return *this;}

template<class I,class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool CurvatureSearchScope::checkDerivativeCrude(double value, I guess, J derivative, SparseMatrix<double>* addout, int rowout){// returns a safe bound on the parameter changes;
    unsigned int i;
    double sums,  sump;
    if (lastderiv == NULL) {
        if (!ExOp::isValid(value)) {
			if (shrinkcount == 2) fprintf(stderr, "the provided second guess has function value NaN, provide a valid second guess...\n");
			else fprintf(stderr, "initial guess has function value NaN, provide a valid guess...\n");
			return true;
		}
		size =0;
		sums = 0.0;
		if (auto dite = derivative.mkIterator()) do{sums += (*dite) * (*dite); size++;}while(dite++);
		if (size == 0) return true; // no parameters... ez!
		lastderiv = new double[2];
		lastderiv[0] = sums;
		i=0;
		shrinkcount =0;
		if (auto dite = derivative.mkIterator()) {
			lastderiv[1] =  *dite;
			do *dite = 0.0; while(dite++);
		}
		if (auto ite = guess.mkIterator()) {
			lastderiv[0] = pow(0.5, 15.0) * value / lastderiv[1];
			*ite += lastderiv[0];
		}
    }else{
		printf("%i: Predict %e %e Empirical (%f error)\n",shrinkcount++, lastderiv[1], (value - lastvalue) / lastderiv[0], pow(2.0, 15.0) * (value - lastvalue - lastderiv[0] * lastderiv[1]) / lastvalue);
		printf("%e %e dvalue %e expected was %e\n", value, lastvalue, value - lastvalue, lastderiv[0] * lastderiv[1] );
		if (shrinkcount >= size) shrinkcount =0;
		i=0;
		if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator()) do {
			if (i++ == shrinkcount) {lastderiv[1] =  *dite; *ite -= lastderiv[0];}
			*dite = 0.0;
			ite++;
		}while(dite++);
		double tmp;
...
return false;}
template<class I,class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool CurvatureSearchScope::checkDerivative(double value, I guess, J derivative){// returns a safe bound on the parameter changes;
    unsigned int i =0;
    //if (auto ite = guess.mkIterator()) do{printf("%e\t", *ite); }while(ite++); printf("<- eval\n");
    if (lastderiv == NULL) {
        if (!ExOp::isValid(value)) {
			if (shrinkcount == 2) fprintf(stderr, "the provided second guess has function value NaN, provide a valid second guess...\n");
			else fprintf(stderr, "initial guess has function value NaN, provide a valid guess...\n");
		...
		size =0;
		if (auto dite = derivative.mkIterator()) do{size++;}while(dite++);
		if (size == 0) return true; // no parameters... ez!
		lastderiv = new double[4+size];
		lastvalue = value;
		i=4;
		if (auto dite = derivative.mkIterator()) do{
			lastderiv[i++] = *dite;
			*dite = 0.0;
		}while(dite++);
		shrinkcount =0;
		lastderiv[2] = fabs(lastvalue * pow(0.5, 32.0) / lastderiv[4]);
		if (auto ite = guess.mkIterator()){
			if (fabs(*ite) * pow(0.5, 50.0) > lastderiv[2]) lastderiv[2] = fabs(*ite) * pow(0.5, 50.0);
			lastderiv[3] = *ite;
			if (!ExOp::isValid(lastderiv[2])) lastderiv[2] = *ite * pow(0.5, 15.0);
			*ite += lastderiv[2];
		}
    }else if (shrinkcount & 1){
    	if (ExOp::isValid(value)) {
			if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator()) do {
				if (i++ ==  (shrinkcount >> 1) ) {
					printf("Dvalue %e and %e   (value %e)\n", value - lastvalue, lastderiv[0] - lastvalue, lastvalue);
					printf("deriv %e Dderiv %e -> %e\n", lastderiv[4 + (shrinkcount>>1)], lastderiv[4 + (shrinkcount>>1)] - *dite, lastderiv[1] - lastderiv[4 + (shrinkcount>>1)]);
					printf("%i: Value Predict %e %e Empirical (%f error)",(shrinkcount >> 1), lastderiv[4 + (shrinkcount>>1)] , (lastderiv[0] - value) / (lastderiv[2] * 2.0),  lastderiv[4 + (shrinkcount>>1)] * (lastderiv[2] * 2.0) / (lastderiv[0] - value) - 1.0);
					printf(" Deriv Predict %e %e Empirical (%f error)\n", (lastderiv[1] - *dite) / (lastderiv[2] * 2.0),  (lastderiv[0] + value - lastvalue * 2.0) / (lastderiv[2] * lastderiv[2]) , (lastderiv[2] * 0.5) * (lastderiv[1] - *dite) / (lastderiv[0] + value - lastvalue * 2.0) - 1.0);
					...
					*ite = lastderiv[3];
					shrinkcount++;
					if (shrinkcount == (size << 1)) shrinkcount =0;
					else{ite++;
						lastderiv[2] = fabs(lastvalue * pow(0.5, 32.0) / lastderiv[4 + (shrinkcount>>1)]);
	//					printf("step size %e vs %e\n",fabs(*ite) * pow(0.5, 50.0) , lastderiv[2]);
						if (fabs(*ite) * pow(0.5, 50.0) > lastderiv[2]) lastderiv[2] = fabs(*ite) * pow(0.5, 50.0);
	//					printf("%e gofor %i\n",lastderiv[2] / lastderiv[4 + (shrinkcount>>1)] , shrinkcount>>1);
						lastderiv[3] = *ite;
						if (!ExOp::isValid(lastderiv[2])) lastderiv[2] = *ite * pow(0.5, 15.0);
						*ite += lastderiv[2];
						do{*dite = 0.0;}while(dite++);
			...
			if (auto dite = derivative.mkIterator()) if (auto ite = guess.mkIterator()) do {
				if (i++ == (shrinkcount >> 1)) {
					printf("%i: %e step backward gave nan(%e in mantissa)\n", shrinkcount >> 1, lastderiv[2], lastderiv[2] / *ite);
					lastderiv[2] *= 0.25;
					*ite = lastderiv[3] + lastderiv[2];
					shrinkcount--;
					do{*dite = 0.0;}while(dite++);
					break;
				}
			}while(ite++);
			if (auto dite = derivative.mkIterator()) do{*dite = 0.0;}while(dite++);
    	}
    }else{
    	if (!ExOp::isValid(value)){ // well, that failed!
			if (auto ite = guess.mkIterator()) do {
				if (i++ == (shrinkcount >> 1)) {
					printf("%i: %e step forward gave nan (%e in mantissa)\n", shrinkcount >> 1, lastderiv[2], lastderiv[2] / *ite);
					lastderiv[2] *= 0.25;
					*ite = lastderiv[3] + lastderiv[2];
					if (*ite == lastderiv[3]) shrinkcount++; // should be impossible, but who knows
					break;
				}
			}while(ite++);
			if (auto dite = derivative.mkIterator()) do{*dite = 0.0;}while(dite++);
		...
		//	printf("shrshr %e %e %i %e expdif %e vs %e\n", lastderiv[0], lastderiv[1], shrinkcount, lastderiv[2], lastderiv[0] - lastvalue, lastderiv[2] * lastderiv[4 + (shrinkcount>>1)]);
			shrinkcount++;
		}
    }
  //  printf("dabuffer %e %e %e %e %e\n", lastvalue, lastderiv[0], lastderiv[1], lastderiv[2], lastderiv[3]);
return false;}


template<class I,class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool CurvatureSearchScope::updateAscent(double value, I guess, J derivative, SparseMatrix<double>* addout, int rowout){// returns a safe bound on the parameter changes;
    unsigned int i,j,k;
    double sums,  sump;
    if (lastderiv == NULL) {
        if (!ExOp::isValid(value)) {
		...
		if (curvature.getSize() == 0){
			// not second guess provided... do everything!
			size =0;
			sums = 0.0;
			if (auto dite = derivative.mkIterator()) do{sums += (*dite) * (*dite); size++;}while(dite++);
			if (size == 0) return true; // no parameters... ez!
			sums = fabs(value * expdiff / sums);
			lastderiv = new double[size*4];
			curvature.setSize(size & 1 ? ((size >> 1)+1)*(size) : (size >> 1)*(size+1));
			unsigned int i,j,k;
			i=0,k=0;
			if (auto dite = derivative.mkIterator()) do{
				curvature[k++] = fabs(sums * (*dite));
				for(j=i+1;j<size;j++) curvature[k++] = 0.0;
				lastderiv[i++] = *dite;
				*dite = 0.0;
			}while(dite++);
			shrinkcount =0;
			lastvalue = value;
			if (auto ite = guess.mkIterator()) do{lastderiv[i+ size ] = *ite;}while(ite++);
			while(shrinkcount < 10){
				i=0; k=0;
				uint64_t convchk =0;
				if (auto ite = guess.mkIterator()) do{
					lastderiv[i+ size *2] = curvature[k] * lastderiv[i];
					*ite = lastderiv[i+ size ] + lastderiv[i+ size *2];
//					printf("%i: %e->%e (%e %e))\n", i, lastderiv[i+ size ], *ite,curvature[k], lastderiv[i]);
					k += size - i;
					if (isinf(*ite)){
						*ite = lastderiv[i+ size] * 8.0;
						if (isinf(*ite)) ExCo<double>::toMaximum(*ite);
						lastderiv[i+ size *2] = *ite - lastderiv[i+ size];
					}else if (!ExOp::isValid(*ite)) {*ite = lastderiv[i+ size]; lastderiv[i+ size *2] = 0.0;}
					convchk |= (*(uint64_t*) (&(*ite))) ^ (*(uint64_t*)(lastderiv + (i+ size)));
					if (*ite == lastderiv[i + size]) lastderiv[i+ size *2] = 0; // did underflow... so sad
					i++;
				}while(ite++);
				if (convchk != 0) return false;
				shrinkcount++; // well, that initialization criterion was off... value has 0 within rounding error?
				i=0,k=0;
				if (auto dite = derivative.mkIterator()) do{
					curvature[k] *= pow(4.0, shrinkcount);
					k += size - i;
					i++;
				}while(dite++);
			}
			...
			if (size == 0){
				printf("size should have been known... got 0\n");
				exit(1);
			}
			if (shrinkcount == 1){
				k=0; i=0;
			...
			lastderiv = new double[size*4];
			if (lastderiv == NULL) {printf("failed to allocate...\n"); exit(1);}
			lastderiv[size * 3] = ExCo<double>::mkMaximum();

			if (shrinkcount == 2){
				k=0; i=0;
				if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator())  do{
					lastderiv[i] = fabs((*ite - curvature[k]) / (*dite - curvature[k|1]));
					k+=2;
					if (!ExOp::isValid(lastderiv[i])) lastderiv[i] = pow(0.5, 53);
					i++;
					if (lastvalue > value) {
						*dite = curvature[k|1];
						*ite = curvature[k];
				...
				if (lastvalue > value) value = lastvalue;

				curvature.setSize(size & 1 ? ((size >> 1)+1)*(size) : (size >> 1)*(size+1));
				for(i=0,k=0;i<size;i++) {
					curvature[k++] = lastderiv[i];
					for(j=i+1;j<size;j++) curvature[k++] = 0.0;
				}
				shrinkcount =0;
			}
		...
		if (addout){
			addout->data[0][rowout] = value;
			addout->data[1][rowout] = lastvalue;
			addout->data[4][rowout] = -1.0;
			addout->data[8][rowout] = lastderiv[size];
		}
        ...
        i=0;
        if (auto ite = guess.mkIterator()) do{
            *ite = lastderiv[i+ size *2] + lastderiv[i + size];
			if (isinf(*ite)){
				*ite = lastderiv[i+ size] * 8.0;
				if (isinf(*ite)) ExCo<double>::toMaximum(*ite);
				lastderiv[i+ size *2] = *ite - lastderiv[i+ size];
			}else if (!ExOp::isValid(*ite)) {
				*ite = lastderiv[i+ size]; lastderiv[i+ size *2] = 0.0;
				/*
				printf("update madeN %e at %i\n", *ite , i); //
				i=0,k=0;
				for(i=0;i<size;i++) {
					printf("\n%.3e", curvature[k++]);
					for(j=i+1;j<size;j++){
						printf("\t%.3e", curvature[k++]);
					}
				}
				printf("\nlast deriv:");
				for(i=0;i<size;i++) printf("\t%e", lastderiv[i]);
				printf("\nlast guess:");
				for(i=0;i<size;i++) printf("\t%e", lastderiv[i+ size]);
				printf("\nlast step:");
				for(i=0;i<size;i++) printf("\t%e", lastderiv[i+ size*2]);
				printf("\nindeed\n");
				exit(0);*/
			}
			i++;
        ...
        if (auto dite = derivative.mkIterator()) do{*dite =0.0;} while(dite++);
     		if (addout){
			addout->data[7][rowout] = lastderiv[size];
		}
		...
		i=0;
		if (auto dite = derivative.mkIterator()) do{
			sums -= lastderiv[i] * lastderiv[i+ size*2]; // rounding errors would be accounted here!
			sump -= *dite * lastderiv[i+ size*2];
			i++;
		}while(dite++);

		/*
		if (!ExOp::isValid(sump)){
			sump =0;
			i=0;
			int tmperr=0;
			if (auto dite = derivative.mkIterator()) do{
				if (!ExOp::isValid(*dite)) {*dite = 0.0; tmperr++;}
				else sump -= *dite * lastderiv[i+ size*2];
				if (!ExOp::isValid(lastderiv[i+ size*2])) tmperr += 0x10000;
				i++;
			}while(dite++);
			/*printf("Got an NAN from derivatives... and value is %e->%e got to fix this??? %i fixes got %e now\n",lastvalue, value,tmperr, sump);
			i=0;
			if (auto ite = guess.mkIterator()) do{
				printf("%i: %e vs %e\n", i, lastderiv[i+ size*2], *ite - lastderiv[i+ size]);
				i++;
			}while(ite++);
		}*/
        ...
        // these are defined for *minimization* so flip sign!
         if (addout){
			addout->data[0][rowout] = value;
			addout->data[1][rowout] = lastvalue;
			addout->data[2][rowout] = sums;
			addout->data[3][rowout] = sump;
			addout->data[4][rowout] = (double)( ((value - lastvalue < -0.0001 * sums) ? 1 : 0) | ((sump < 0.75 * sums) ? 2 : 0));
			addout->data[8][rowout] = lastderiv[size];
		}
...
//lastderiv[size * 3]
        if (value - lastvalue  < fabs(0.0001 * sums)){ shrinkcount++; // step too long cannot accept update
            // scaling inverse hessian by 0.25
            // sums is the denominator!
			i=0;
			if (auto dite = derivative.mkIterator()) do{
				lastderiv[i+3*size] = *dite - lastderiv[i];
				i++;
			}while(dite++);

			i=size; if (auto ite = guess.mkIterator()) do{*ite = lastderiv[i++];}while(ite++); // store old in that thing

            if ((!ExOp::isValid(sump))||(sump < 0.75 * sums)) { // step is too short for hessian update AND too long... so this is it.
                // cannot update hessian... and step is too short...
				/*
				sump = 0.0;
				memset(lastderiv + size ,'\0', sizeof(double)*size);
				for(i=0,k=0;i<size;i++) {
					lastderiv[i + size] += curvature[k] * lastderiv[i + size];//printf("%e\t",curvature[k] );
					for(j=i+1,k++;j<size;j++,k++){
						lastderiv[j + size] += curvature[k] * lastderiv[i + size];
						lastderiv[i + size] += curvature[k] * lastderiv[j + size];//printf("%e\t",curvature[k] );
					}
				...
				//for(i=0;i<size;i++){printf("%e\t", lastderiv[i + size]);}	printf("step into deriv %e, fact %e\n", sump, 1.0 / sump);
				...
				*/
				...
				if (value - lastvalue  < 0.0001 * sums){
					if (addout) addout->data[4][rowout] += 32.0;
					for(i=0,k=0;i<size;i++) {
						for(j=i;j<size;j++) {
							curvature[k++] *= 0.25;
							//curvature[k++] += sump * lastderiv[i + size] *  lastderiv[j + size];
						}
					}
				}else{
					if (addout) addout->data[4][rowout] += 16.0;
					for(i=0,k=0;i<size;i++) {
						for(j=i;j<size;j++) {
							curvature[k++] *= 4.0;
							//curvature[k++] += sump * lastderiv[i + size] *  lastderiv[j + size];
				...
				sums = -1.0 / sums;
				if (addout){
					addout->data[5][rowout] = sums;
					addout->data[6][rowout] = sump;
					addout->data[4][rowout] += 10;
				}
				sump *= sums;


		   //     printf("K2 = %f\tK1 = %f\n", sump,sums);
				if ((!ExOp::isValid(sums))||(!ExOp::isValid(sump))) {
					printf("Lets pray... %e %e\n", sums, sump);
					if (!ExOp::isValid(sump)) sump =0.0;
					if (!ExOp::isValid(sums)) sums =0.0;
				}
			...
			if (shrinkcount > 10) {
				//printf("shrink exit\n");
				//int i=size;
				// *ite was already restored... to old guess !!!
				//if (auto ite = guess.mkIterator()) do{
				//	printf("set %e\n", lastderiv[i]);
				//	*ite = lastderiv[i++];
				//}while(ite++);
			...
			i=0;
			if (auto ite = guess.mkIterator()) do{
				lastderiv[i + size] = *ite; // restore the old position (really!)
				*ite += lastderiv[i + size *2];
				if (isinf(*ite)){
					*ite = lastderiv[i+ size] * 8.0; // well infinite derivative is probably not real, so scale up and pray
					if (isinf(*ite)) ExCo<double>::toMaximum(*ite);
					lastderiv[i+ size *2] = *ite - lastderiv[i+ size];
				}else if (!ExOp::isValid(*ite)) {
					*ite = lastderiv[i+ size]; lastderiv[i+ size *2] = 0.0;
					/*
					printf("update madeB  %e at %i\n", *ite , i); //
					i=0,k=0;
					for(i=0;i<size;i++) {
						printf("\n%.3e", curvature[k++]);
						for(j=i+1;j<size;j++){
            ...
            return convchk == 0;
        }else if ((sump < 0.75 * sums)||(!ExOp::isValid(sump))) { // step too short, accept change but not the normal hessian update
            if (lastvalue >  value) printf("short confusion... %e >= %e upper!!!\n",value -lastvalue, -0.0001 * sums);
            shrinkcount =0;
			...
			if (!ExOp::isValid(sump)){
				if (auto dite = derivative.mkIterator()) do{
					if (!ExOp::isValid(*dite)) *dite = 0.0;
				}while(dite++);
			}

            k=0,i=0;
            if (auto dite = derivative.mkIterator()) do{
				lastderiv[i + size] += curvature[k++] * (*dite);//printf("%e\t",curvature[k] );
				j=i+1;
				auto sdite = dite; while(sdite++) {
					lastderiv[(j++) + size] += curvature[k] * (*dite);
					lastderiv[i + size] += curvature[k++] * (*sdite);//printf("%e\t",curvature[k] );
				}
				sump += lastderiv[(i++) + size] * (*dite);
            }while(dite++);
            if (k != curvature.getSize()) {LFH_ALIVE; printf("expected %i items nested recution, got %i instead...\n", curvature.getSize(), k); exit(1);}

            /*for(i=0;i<size;i++){printf("%e\t", lastderiv[i + size]);}
            printf("step into deriv %e, fact %e\n", sump, 1.0 / sump);*/

            if (addout)	addout->data[7][rowout] = sump;


            sump = 3.0 / sump; // factor
            // multiply hessian: H <- (I - a sd)H(I + a ds)
            if (!ExOp::isValid(sump)){
				//printf("Would have been problematic... saved!\n");
				for(i=0,k=0;i<size;i++) {
            ...
            i=0,k=0;
			if (auto dite = derivative.mkIterator()) do{
                lastderiv[i+ size *2] += curvature[k++] * *dite;//printf("%e\t",curvature[k] );
				j = i+1;
				auto sdite = dite; while(sdite++) {
					lastderiv[(j++)+ size *2] += curvature[k] * (*dite);
					lastderiv[i+ size *2] += curvature[k++] * (*sdite);//printf("%e\t",curvature[k] );
				}
                lastderiv[i++] = *dite;
                *dite = 0.0;
            }while(dite++);
			if (k != curvature.getSize()) {printf("expected %i items nested recution, got %i instead...\n", curvature.getSize(), k); exit(1);}

            i=0;
			if (auto ite = guess.mkIterator()) do{
                sump += lastderiv[i+ size *2] * lastderiv[i+ size *2];
                lastderiv[i+ size] = *ite;
                *ite += lastderiv[i+ size *2];
				if (isinf(*ite)){
					*ite = lastderiv[i+ size] * 8.0;
					if (isinf(*ite)) ExCo<double>::toMaximum(*ite);
					lastderiv[i+ size *2] = *ite - lastderiv[i+ size];
				}else if (*ite < 0){ // hit a wall!
					lastderiv[i+ size *2] = -lastderiv[i+ size];
					*ite = 0.0;
				}else if (!ExOp::isValid(*ite)) {
					*ite = lastderiv[i+ size]; lastderiv[i+ size *2] = 0.0;/*
					printf("update madeF  %e at %i\n", *ite , i); //
					i=0,k=0;
					for(i=0;i<size;i++) {
						printf("\n%.3e", curvature[k++]);
						for(j=i+1;j<size;j++){
                ...
                if (*ite == lastderiv[i+ size]) lastderiv[i+ size *2] = 0;
                i++;
			}while(ite++);
		 ...
		 //   printf("AlRight!\n");
			...
			i=0; if (auto dite = derivative.mkIterator()) do{ lastderiv[i+3*size] = *dite - lastderiv[i]; i++;} while(dite++);
	   ...
	   //     printf("K2 = %f\tK1 = %f\n", sump,sums);
        ...
        if (lastvalue >  value) printf("confused...!!!\n");
		lastvalue = value;
    }
...
template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)>
bool CurvatureSearchScope::updateGotNAN(I guess){// returns a safe bound on the parameter changes;
    unsigned int i,j,k;
    if (lastderiv == NULL) {
		if (shrinkcount == 2) fprintf(stderr, "the provided second guess has function value NaN, provide a valid second guess...\n");
		else fprintf(stderr, "initial guess has function value NaN, provide a valid guess...\n");
		...
		*ite = lastderiv[i+ size *2] + lastderiv[i + size];
		if (isinf(*ite)){
			*ite = lastderiv[i+ size] * 8.0;
			...
			lastderiv[i+ size *2] = -0.75 * lastderiv[i+ size];
			*ite = 0.25 * lastderiv[i+ size];
		}else if (!ExOp::isValid(*ite)) {
			*ite = lastderiv[i+ size]; lastderiv[i+ size *2] = 0.0;
		}
		i++;
...
template<class I, class J,ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool CurvatureSearchScope::updateAscentPositiveDomain(double value, I guess, J derivative, SparseMatrix<double>* addout, int rowout){// returns a safe bound on the parameter changes;
    unsigned int i,j,k;
    double sums,  sump;
    if (addout) addout->data[6][rowout] = value;
    if (lastderiv == NULL) {
        if (!ExOp::isValid(value)) {
			if (shrinkcount == 2) fprintf(stderr, "the provided second guess has function value NaN, provide a valid second guess...\n");
			else fprintf(stderr, "initial guess has function value NaN, provide a valid guess...\n");
        ...
        // these are defined for *minimization* so flip sign!
         if (addout){
			addout->data[3][rowout] = value - lastvalue;
			addout->data[4][rowout] = -0.0001 * sums;

			addout->data[7][rowout] = sums;
			addout->data[8][rowout] = sump;
			addout->data[9][rowout] = (double)( (( value - lastvalue < -0.0001 * sums) ? 1 : 0) | ((sump < 0.75 * sums) ? 2 : 0));
		}

        //printf("wolfe 1: %e < %e  %c\n",  -lastderiv[size * 3], -0.0001f * sums, (lastderiv[size * 3] >= 0.0001f * sums) ? 'Y' : 'N' );
        //printf("wolfe 2: %e < %e  %c\n", -sump, -0.9f * sums , (sump >= 0.9f * sums) ? 'Y' : 'N' );
//lastderiv[size * 3]
        if (value - lastvalue  < -0.0001 * sums){  // step too long cannot accept update
            // scaling inverse hessian by 0.25
            // sums is the denominator!
			i=0;
			if (auto dite = derivative.mkIterator()) do{
			...
			i=0;
			if (auto ite = guess.mkIterator()) do{*ite = lastderiv[ (i++) +size];}while(ite++);
		   ...
		   //     printf("K2 = %f\tK1 = %f\n", sump,sums);
				if ((!ExOp::isValid(sums))||(!ExOp::isValid(sump))) {
					...
					for(i=0,k=0;i<size;i++) {
						curvature[k++] += sums * (lastderiv[i+size*2] * (sump * lastderiv[i+size*2] + 2.0 * lastderiv[i+size]));
						for(j=i+1;j<size;j++) curvature[k++] += sums *(sump * lastderiv[i+size*2]* lastderiv[j+size*2] + (lastderiv[i+size*2]* lastderiv[j+size] + lastderiv[i+size]* lastderiv[j+size*2]));
					}
				}
			}
			if ((shrinkcount++) > 10) {
				if (auto ite = guess.mkIterator()) do{
			if (*ite < 0.0) printf("negC\n");
			} while(ite++);
				return true;
			}
			memset(lastderiv + size * 2 ,'\0', sizeof(double)*size);
			...
			i=0; if (auto ite = guess.mkIterator()) do{
            ...
            //printf("%e\t%e\tExpand  Step %e \n", value, lastderiv[size * 3], sqrt(sump));
            if (lastvalue >  value) printf("short confusion...!!!\n");
            lastvalue = value;
			if (auto ite = guess.mkIterator()) do{
			if (*ite < 0.0) printf("negE\n");
        } while(ite++);
            return false;
        }else {
        	shrinkcount =0;
	...
	if (auto ite = guess.mkIterator()) do{
		if (*ite < 0.0) printf("negtyp\n");
	} while(ite++);

return convchk == 0;}

template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> CurvatureSearchScope& CurvatureSearchScope::initInvCurv(I inv_curvature){
	//printf("\nhello initinv...\n");
	if ((size == 0)||(lastderiv == NULL)){
		size = 0;
		if (auto dite = inv_curvature.mkIterator()) do{
			size++;
		}while(dite++);
	}
	if (lastderiv != NULL) delete[](lastderiv);
    lastderiv = NULL;
	shrinkcount =0;
	curvature.setSize(((size & 1) == 1) ? ((size >> 1)+1)*(size) : (size >> 1)*(size|1));
	int i,j,k;
	i=0,k=0;
	if (auto dite = inv_curvature.mkIterator()) do{
		curvature[k++] = *dite;
	...
	//printf("testing testing... %i %i %i\n", size, k, curvature.getSize());
return *this;}
template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> void CurvatureSearchScope::wrInvCurv(I inv_curvature) const{
	if (curvature.getSize() == 0) return;
	int k=0;
	int i=size;
	if (auto dite = inv_curvature.mkIterator()) do{
	*dite = curvature[k];
	...
	//printf("testing testing write ... %i %i %i\n", size, k, curvature.getSize());
}

template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> void CurvatureSearchScope::wrFinalGuess(I guess_to_write) const{
	if (lastderiv == NULL) return;
	int i=size;
	if (auto ite = guess_to_write.mkIterator()) do{
		*ite = lastderiv[i++];
	}while(ite++);
}
...
template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool MemoryLimitedOptimizationScope::updateAscent(double value, I guess, J derivative){
	double* curS;
	double* curY;
	double* curact;
	double* futureY;
	...
	uint32_t i,offset,j;
	if (!ExOp::isValid(value)){
		if (step == 0) {printf("Got NAN on starting guess... fix that start guess!\n"); return true;}
    }else if (step > 0){
		sums = 0.0;sump = 0.0;
		offset = (((step-1)<<1) % hhfact.getSize());
		curS = scope() + (( offset % hhfact.getSize()) * size);
		curY = scope() + (( (offset|1) % hhfact.getSize()) * size);
		curact = scope() + (((step<<1) % hhfact.getSize()) * size);

		if (auto dite = derivative.mkIterator()) do{
			sums -= (*curY) * (*curS); // rounding errors would be accounted here!
			sump -= *dite * (*curS);
			*(curact++) = *dite;
		}while(dite++);
		printf("delta:%e expected:%e extrapolated:%e\n", value - lastvalue, sums, sump);
		// sums expected improvement
		// sump expected improvement in the same direction

		if (value - lastvalue  < -0.0001 * sums){ // not an significant improvement given step size, needs to backtrack

        }else if (sump < 0.75 * sums){ // curvature did not change significantly, needs to accelerate

		//	return true;
			//if (auto dite = derivative.mkIterator()) do{
			//	sums -= (*curT) * (*curS); // rounding errors would be accounted here!
			//	sump -= *dite * (*curS);
			//}while(dite++);


        }else{ // NORMAL UPDATE!
			// first, store normalized derivative delta  y_k = (d_k+1 - d_k) / (d_k+1 - d_k) \cdot (x_k+1 - x_k)

			// first step
			offset = (((step-1)<<1) % hhfact.getSize());
		//	printf("step targ =%i for step %i\n", (step << 1) % hhfact.getSize(),step);

		//	printf("first step S=%i Y=%i\n", offset , offset|1);
			curS = scope() + (( offset % hhfact.getSize()) * size);
			curY = scope() + (( (offset|1) % hhfact.getSize()) * size);
			curact = scope() + (((step<<1) % hhfact.getSize()) * size);
			futureY = scope() + ((((step<<1)|1) % hhfact.getSize()) * size);
			hhfact[offset] = 0.0;
			hhfact[offset|1] = 0.0;
			sump = 0.0;
			if (auto dite = derivative.mkIterator()) do{
				...
				*(futureY++) = *dite;
				sump += (*dite) * (*dite);
				hhfact[offset] += *dite * *curS; // s_i * g_i
		//		printf("steppy %e %e %e\n", *dite, (*curY + *dite), *curS);
				*curY = *dite - *curY;
				hhfact[offset|1] += *(curS++) *  *(curY++); // s_i * g_i
				*dite = 0.0;
			}while(dite++);
			sums = hhfact[offset] / hhfact[offset|1]; //last factor

			lastvalue = hhfact[offset|1] / sump; // diagonal curv
		//	printf("P[%i] denum is %e\n", offset | 1, hhfact[offset|1] );

			// forward step
			for(j = 1; j < (hhfact.getSize() >> 1);j++){
				if (step <= j) break;
				offset = ((step - j -1) << 1) % hhfact.getSize();
				hhfact[offset] = 0.0;

		//		printf("fwd step%i S=%i Y=%i F=%e\n", j, offset , (offset+3) % hhfact.getSize(), sums);
				curact = scope() + (((step<<1) % hhfact.getSize()) * size);
				curS = scope() + ( offset * size);
				curY = scope() + (( (offset+3) % hhfact.getSize())  * size);
				for(i=0;i<size;i++){
					*curact -= *(curY++) * sums;
					hhfact[offset] += *(curS++) * *(curact++);
				}
				sums = hhfact[offset] / hhfact[offset|1];
			}
			// deepest step


			offset = (j == (hhfact.getSize() >> 1)) ? (step <<1) % hhfact.getSize() : 0;
			curY = scope() + (offset|1) * size;
			curact = scope() + (((step<<1) % hhfact.getSize()) * size);
		//	printf("deepest step Y=%i F=%e K=%e\n", offset|1, sums, lastvalue);

			for(i=0;i<size;i++){
				*curact -= *curY * sums;
				*curact *= lastvalue;
				hhfact[offset] -= *(curY++) * *(curact++);
			}
			sums = hhfact[offset] /  hhfact[offset | 1];

			// backward step
			for(j = step < (hhfact.getSize() >> 1) ?  step - 1 : (hhfact.getSize() >> 1)-1 ; j > 0;j--){

				offset = ((step - j - 1) << 1) % hhfact.getSize();
				curact = scope() + (((step<<1) % hhfact.getSize()) * size);
				curS = scope() + (offset * size);
				curY = scope() + (( (offset+3) % hhfact.getSize())  * size);
		//		printf("bwd step%i S=%i Y=%i F=%e\n", j, offset , (offset+3), sums);
				for(i=0;i<size;i++){
					*curact += *(curY++) * sums;
					hhfact[offset] -= *(curS++) * *(curact++);
				}
			...
			offset = (j == (hhfact.getSize() >> 1)) ? ((step <<1)| 1) % hhfact.getSize() : 0;
			curS = scope() + (offset * size);
		//	printf("last step S=%i F=%e\n", offset, sums);

			curact = scope() + (((step<<1) % hhfact.getSize()) * size);
			if (auto ite = guess.mkIterator()) do{
				*curact += *(curS++) * sums;
				*curact = -(*curact); // con-servative
				*ite += *(curact++);
			}while(ite++);

			lastvalue = value; step++;
        }
		return false; // only once for now!
    }else{
    	if (scope.getSize() == 0){ // start with no curvature guess at all!
			size =0; sums = 0.0;
			if (auto dite = derivative.mkIterator()) do{sums += (*dite) * (*dite); size++;} while(dite++);
			if (size == 0) return true; // no parameters... ez!
			sums = fabs(value * lastvalue / sums);
			scope.setSize(size * hhfact.getSize() );
			unsigned int i,j,k;
			i=0,k=0;
			curS = scope();
			curY = scope() + size;
...
template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
bool MemoryLimitedOptimizationScope::checkDerivative(double value, I guess, J derivative){
	if (scope.getSize() == 0){
		if (!ExOp::isValid(value)) {
			fprintf(stderr, "initial guess has function value NaN, provide a valid guess...\n");
			return true;
		}

		if (auto dite = derivative.mkIterator()) {
			size = 1;
			while(dite++){size++; *dite = 0.0;}
		}else size =0;
		...
		if (auto ite = guess.mkIterator()) {
			scope[0] = pow(0.5, 45.0) * value / scope[1];
			*ite += scope[0];
		}
		step=0;
	}else{
		printf("%i: Predict %e %e Empirical (%f rel error)\n",step++, scope[1], (value - lastvalue) / scope[0], pow(2.0, 45.0) * (value - lastvalue) / lastvalue - 1.0);
		if (step == size) step =0;
		uint32_t i=0;
		if (auto dite = derivative.mkIterator()) do {
			if (i++ == step) scope[1] =  *dite;
			*dite = 0.0;
		}while(dite++);
		scope[0] = pow(0.5, 45.0) * value / scope[1];
		i=0;
		if (auto ite = guess.mkIterator()) do {
			if (i++ == step) {*ite += scope[0]; break;}
		}while(ite++);
...
/*
template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> AdaptiveLearningScope& AdaptiveLearningScope::initVariable(const char* target, I variable, double alpha, double beta1, double beta2){
	int size=0;
	if (auto ite = variable.mkIterator()) do{ size++;}while(ite++);
	KeyElem< Tuple<double> , Tuple<double, 4u> > scope;
	scope.k.setSize(size << 1).toZero();
	scope.d[0] = 0.0;
	scope.d[1] = alpha;
	scope.d[2] = beta1;
	scope.d[3] = beta2;
	uint32_t newalias;
	if ((newalias = variables.find(string(target))) != 0xFFFFFFFF) variables.erase_from_iterator(newalias);
	variables.createEntry(string(target), newalias) = scope;
return *this;}
*/


template<class I, class J, ENABLEIF_WRITE_ITERATOR_ANYKEY(I), ENABLEIF_WRITE_ITERATOR_ANYKEY(J)>
double AdaptiveLearningScope::updateVariable(const AdativeLearningParameters &param, I guess, J derivative){
	double* cur = vars();
	if (param.step == 0){
		vars.toMemfree();
        uint32_t length = 0;
        if (auto ite = guess.mkIterator()) do{ length++;}while(ite++);
		if (length == 0) {printf("tried to search an empty parameter space... why???\n"); exit(0);}
		vars.setSize(length<<1);
		if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator()) do{
			cur[0] = (*dite);
			cur[1] = fabs(*dite);
			*ite = *ite + ((cur[0] > 0.0) ? param.alpha : -param.alpha);
			cur += 2;
		}while((ite++) && (dite++));
	}else if (ExOp::isValid(param.cur_value)){
		double fact[2];
		double fisqrt;
		fact[1] = (1.0 - param.beta) / (1.0 - pow(param.beta, param.step+1));
		fact[0] = 1.0 - fact[1];
		if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator()) do{
			if (ExOp::isValid(*dite)){
				cur[0] = fact[0] * cur[0] + fact[1] * (*dite);
				fisqrt = fabs(*dite);
				if ((cur[1] *= param.beta2) < fisqrt) cur[1] = fisqrt;
			}
			//printf("%e\t", param.alpha * cur[0] / cur[1]);
			*ite += param.alpha * cur[0] / cur[1];   // 2.0 / sqrt(cur[1])
			cur += 2;
		}while((ite++) && (dite++));
		//printf("delta\n");
		/*
		fisqrt = fact[1] * cur[1] + fact[3] * (*dite) * (*dite);
		if (ExOp::isValid(fisqrt)) cur[1] = fisqrt;
		(*(uint64_t*)&fisqrt) = 0x5FE6EB50C7B537A9 - ( (*(uint64_t*)(cur + 1)) >> 1 ); // what the fuck?
		cur[2] = *ite;
		*ite -= param.alpha * cur[0] * fisqrt * ( 3.0 - ( cur[1] * fisqrt * fisqrt) );   // 2.0 / sqrt(cur[1])
		*/
	}else{ // hit NAN... does not want to loose momemtum needlessly, so
		if (auto ite = guess.mkIterator()) if (auto dite = derivative.mkIterator()) do{
			if (rand() & 1) {
				cur[0] = -cur[0];
				*ite += 2.0 * param.alpha * cur[0] / cur[1];
			}
			cur += 2;
			*dite = 0.0;
		}while((ite++) && (dite++));
...
return 0.0;}
/*template<class I, ENABLEIF_WRITE_ITERATOR_ANYKEY(I)> void AdaptiveLearningScope::writeBest(I guess){
...
LFHTEMP void DomainMappedTuple<C,D,LFHDOMAINMAP_ZERO_TO_ONE>::applyUpdate(Tuple<C, D>& deriv, double alpha){
	...
	template <class C> const C& getValue(int flagindex){ C tmp; LFH_exit(1); return(tmp);}
            ...
            if (unitsize == sizeof(unsigned char)*8){
                ...
                { //:
                    ...
                    if (ite.first()) do{
                        ...
                        if (flags & 2) ExOp::bytereverse(buffer[k]);
                        f_out.data[j++] = (C) buffer[k++];
                    } while(ite.next());
                    delete[](buffer);
                } //:
...
bool TiffFile::fetch(DataGrid<C,3>& f_out, char * type_out){
    ...
    for(i=0;i<curflaglist.getSize()/12;i++){
        ...
        if (flags & 2) ExOp::bytereverse(tmpflag);
        switch (tmpflag){
            ...
            case TIFFFLAG_Compression: tmp_iscompressed = getValue<int>(i); if ((tmp_iscompressed != 0)&&(tmp_iscompressed != 1)) {fprintf(stderr,"Tiff file is saved in a compressed format (compress code:%i), it needs to be uncompressed.\n",tmp_iscompressed&0xFFFF);  LFH_exit(1);} break;
            case 258: unitsize = getValue<unsigned int>(i); break; //  printf("%X %X,%X,%X\n", tmpflag, *(uint16_t*)&(curflaglist[i*12+2]), *(uint32_t*)&(curflaglist[i*12+4]), *(uint32_t*)&(curflaglist[i*12+8]) );
    ...
    //printf("da required flag %i\n", required);
    if (required == 59){
        f_out.setSizes(size);
    	//printf("%i,,,%i,,,%i\n",stripro, unitsize ,stripco.size());
        k = stripro * size[1]* size[0];
        j=i=0;
        i--;

        switch(type | unitsize | ((flags << 2) & 4)){
        case 9: // unsigned int
            ...
            { //:
                ...
                if (ite.first()) do{
                    if (k >= stripro * size[1]* size[0]){
                        i++;
                        fseek(f,stripof[i],SEEK_SET);
                        if ((rout = fread(buffer,sizeof(unsigned char), stripro * size[1]* size[0],f)) != (int)(stripro * size[1]* size[0])) {
							printf("Unexpected end of chunk %i != %i!\n", rout, stripro * size[1]* size[0]);
							return false;
						}
                        k=0;
                    }
        ...
        break;  case 5: case 13:{
			if (type_out) type_out[0] = 'C';
			const uint8_t * buffer;
			if (ite.first()) do{
				if (k >= stripro * size[1]* size[0]){
					i++;
					buffer = raw + stripof[i];
					k=0;
				}
				f_out.data[j++] = (C) buffer[k++];
			} while(ite.next());
        ...
        break; case 3 | (sizeof(float) << 3):{ // float
			...
			delete[](buffer);
        }break; case 7 | (sizeof(float) << 3):{ // float
			if (type_out) type_out[0] = 'f';
			const float * buffer;
			if (ite.first()) do{
				if (k >= stripro * size[1]* size[0]){
					i++;
					buffer = (float*)(raw + stripof[i]);
					k=0;
				}
				f_out.data[j++] = (C) buffer[k++];
			} while(ite.next());
        }break; case 3 | (sizeof(double) << 3): // float
            { //:
        ...
        break; default:
        	printf("datype %i,%i,%i for a %ix%i %i image\n",type,unitsize , ((flags << 2) & 4),  size[1],size[2]  , size[0]);
        	printf("Code missing for loading tif,3 with flag %X\n", type | unitsize | ((flags << 2) & 4));
        }
    }else return false;
return(true);}
template <class C> bool TiffFile::fetch(DataGrid<C,2>& f_out, const unsigned int channel){
    //printf("fetchC called!\n");
    ...
    if (required == 59){
        ...
        case 9: {
			...
			delete[](buffer);
         }break; default: printf("Code missing for loading tif,2 with flag %X\n", type | unitsize | ((flags << 2) & 4));
        }
    }else return false;
    return(true);
}
            ...
            case 1: // unsigned intTIFFFLAG_Compression
                if (unitsize == sizeof(unsigned char)*8){
                    ...
                    { //:
                        ...
                        if (ite.first()) do{
                            if (size[0] ==3){
                                value = 0.30 * pow(buffer[k++], 2.2);
                                value += 0.59 * pow(buffer[k++], 2.2);
                                value += 0.11 * pow(buffer[k++], 2.2);
                                f_out.data[j++] = (C) pow(value, 1.0 / 2.2);
                            }else {
...
template <class C> bool TiffFile::put(const DataGrid<C,3u>& f_out, bool updateRange){return this->put(f_out,ExCo<C>::mkMinimum(),ExCo<C>::mkMaximum(),updateRange);}
template <class C, unsigned int D, Tuple_flag Cflag> bool TiffFile::put(const DataGrid< Tuple<C,D, Cflag> ,2u>& f_out, bool updateRange){return this->put(f_out,ExCo<C>::mkMinimum(),ExCo<C>::mkMaximum(),updateRange);}

template <class C, class TYPE> bool TiffFile::put(const DataGrid<C,3u>& f_out, TYPE min, TYPE max,bool updateRange){
...
template <class C, class TYPE> bool TiffFile::put(const DataGrid<C,2u>& f_out, TYPE min, TYPE max,bool updateRange){
		//	printf("%i\t%i\t%i\n", f_out.dims[0],f_out.dims[1],f_out.dims[2]);
		if (endfile_pos == 0){ // nothing was written so far
			endfile_pos = curfp_pos + 4;
		}
...
template <class C, class TYPE, unsigned int SIZE, Tuple_flag Cflag> bool TiffFile::put(const DataGrid<Tuple<C,SIZE, Cflag>,2u>& f_out, TYPE min, TYPE max,bool updateRange){
...
LFHTEMP class Forest<C,nbdata>::Task_cluster_likelihood{
    ...
    Task_cluster_likelihood(const GaussCloud<uint32_t>& _cloud):cloud(_cloud){ // reference initializations for shared scope
    }
...
//    Forest<C,nbdata>::Task_cluster_likelihood locask(hie);
...
LFHTEMP class Forest<C,nbdata>::Task_cluster_likelihood_bruteforce{
    ...
    const Vector< Tuple<uint32_t, 2u> > &to_merge;

    Task_cluster_likelihood_bruteforce(const SparseMatrix< double > &_data, const Vector< Tuple<uint32_t, 2u> > &_to_merge):data(_data), thrray(foreach.mkThreadArray()), to_merge(_to_merge){ // reference initializations for shared scope
    }
    uint32_t operator()(uint32_t threadID){
        switch(task){
        case 0:{ // generate a random matrix
        ...
        case 1:{
            ...
            for(uint32_t i = thranges[threadID]; i <thranges[threadID+1];i++){ // , tb.updateProgress(threadID)
                ...
                if (auto ite = data.data[i].mkIterator()) do{
                  //  for(uint32_t j = 0 ; j< brojection.getSize();j++)
            ...
            while(thrray.getRangeVal(jobID)){
                toins.k = 0.0f;
                toins.d = to_merge[jobID][0];
                nodetree[to_merge[jobID][1]].insert(toins);
            }
        }break;
        }
        //tb.finishProgress(threadID);
...
LFHTEMP ERRCODE Forest<C,nbdata>::cluster_likelihood_bruteforce(const SparseMatrix< double > &data, TMatrix<double> &proj, bool use_one){
    Vector< Tuple<uint32_t, 2u> > to_merge;
    Forest<C,nbdata>::Task_cluster_likelihood_bruteforce locask(data, to_merge);
    ...
    */
    ...
    locask.thrray.initRange(to_merge.getSize(), "Computing pair distances");
    locask.task=2; locask.thrray.submit(locask);
return 0;}
...
public:
    ...
    uint32_t operator()(uint32_t thrID){
        ...
        if (damove[thrID] >= thrID) {
            damove[thrID]++;
            for(j=0;j< data.getNBcols();j++){ foreach.updateProgress(thrID);
...
template<class C> Tuple<uint32_t> MPReachDistribution::learnPermutation(const SparseMatrix<C>& data, double prior_count, double &LL_to_change){ // return permutation of data
    ...
    foreach.startProgress("Permuting states and updating transition probabilities", data.getNBcols() * (getSize()+1) );
    for(int i=getSize();i>0;i--) foreach.submit(lotask , i);
...
LFHTEMP	void SerialStore<IDTYPE>::compress(char* buffer){
    ...
    while(j != 0xFFFFFFFF){
        ...
        while(l >= 65536) {
            ...
            b -= 65536;
            fseek(f, a,SEEK_SET);
            if (fread(buffer,sizeof(char),65536,f) != 65536) LFH_exit(1);
            fseek(f, b,SEEK_SET);
            if (fwrite(buffer,sizeof(char),65536,f)  != 65536) LFH_exit(1);
            l -= 65536;
        }
        a -= l;
        b -= l;
        fseek(f, a,SEEK_SET);
        if (fread(buffer,sizeof(char),l,f) != l) LFH_exit(1);
        fseek(f, b,SEEK_SET);
        if (fwrite(buffer,sizeof(char),l,f) != l) LFH_exit(1);
        }
        chlist[j].k = b;
        j--;
    }
	...
	LFHTEMP	void SerialStore<IDTYPE>::flush(char* alt_path){
		...
		if (g != NULL){
			...
			for(int l=(size >> 16);l>0;l--) {
				if (fread(buffer,sizeof(char),65536,f) != 65536) LFH_exit(1);
				if (fwrite(buffer,sizeof(char),65536,g) != 65536) LFH_exit(1);
			}
			if ((size &65535) != 0 ) {
				if (fread(buffer,sizeof(char),(size &65535),f) != (size &65535)) LFH_exit(1);
				if (fwrite(buffer,sizeof(char),(size &65535),g) != (size &65535)) LFH_exit(1);
			}
		fclose(g);
...
LFHTEMP	ERRCODE SerialStore<IDTYPE>::saveTo(const char* const path) const{
	...
	LFHTEMP	template<class T> ERRCODE SerialStore<IDTYPE>::load(IDTYPE whatyp, T & what){
		...
		if (ite2 != 0xFFFFFFFF){
			...
			return ExOp::load(what,f); // the original reason for a sized load..., hash_scope.deref(ite2).d
		} else return 1;
	}
...
template<class ...A> Tuple<uint32_t> SymbolicScope::mkAliases_routine(Vector<uint32_t> &fout, const char* val, A... a){
    fout.push_back(alias.getAlias(string(val)));
...
template<class ...A> Tuple<uint32_t> SymbolicScope::mkAliases_routine(Vector<uint32_t> &fout, string val, A... a){
    fout.push_back(alias.getAlias(val));
return this->mkAliases_routine(fout, a...);}
