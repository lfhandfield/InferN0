 ...
 */
	...
	template<class C, unsigned int nbstates>
	void Classifier<C,nbstates>::operator()(Tuple<double, nbstates>& _out, const C& _in )const{
		...
		for(i=0;i<nbstates;i++){
			...
			/*	if (!ExCo<double>::isValid(tmp[i])) {
			 ...
			 ((NormalDistribution<2> *)classes[0])->show(stdout);
			 ((NormalDistribution<2> *)classes[1])->show(stdout);
...
// (par_RM.sizes[0])
    ...
    */
    ...
    myHashmap<uint32_t> genes_to_exclude;
    ...
    Task_run2D_EM_v4(const BiClassifier<C,R,S> &_target, const SparseMatrix<C>& _data, uint32_t _nbthread, bool _use_heavy_prior, const Vector<uint32_t> &_excl):target(_target), data(_data), use_heavy_prior(_use_heavy_prior){
        trdata = _data.mkTranspose();
        rowrange.setSize(_nbthread+1);
        colrange.setSize(_nbthread+1);
        alphas.setSize(_nbthread).toOne() *= 0.001;

        if (target.colhid.data.getSize() != data.getNBcols()){printf("nb col mismatch %i, %i", target.colhid.data.getSize(), data.getNBcols()); LFH_exit(1);}
        if (target.rowhid.data.getSize() != trdata.getNBcols()){
            if (target.rowhid.data.getSize() < trdata.getNBcols()) {printf("warning nb row mismatch %i, %i", target.rowhid.data.getSize(), trdata.getNBcols()); LFH_exit(1);}
      ...
      //  row_nbmax_hcol.toSizes(coor).toZero();
        ...
        taskID=0;
        if (auto ite = _excl.getIterator()) do{
            genes_to_exclude.addEntry(*ite);
        }while(ite++);
    }
    ...
    uint32_t operator()(uint32_t threadID){
        ...
        switch(taskID){
            ...
            ExOp::toZero(sum);
            for(coor[1]=colrange[threadID];coor[1] < colrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                //printf("a%ib%ic\n", threadID, coor[1]);
                const SparseTuple<double>& curcol = target.colhid.getColumn(coor[1]);
                if (auto ite =curcol.mkIterator()) do {if (ite() >= target.par_RM.sizes[1]) {printf("IMPOSSIBLBLBLB!!!\n"); curcol.show(); LFH_exit(1);} }while(ite++);
                if (!ExOp::isValid(target.par_colscale[coor[1]][0])){
                    ...
                    for(acoor[0] =0;acoor[0]< target.par_RM.sizes[0]; acoor[0]++){
                        coor[0] = acoor[0];
                        if (freq_rowh[coor[0]] <= col_freq_hrow(coor)- 0.001) foreach.msgs.insert(to_string(coor[0]) + string("got negative!") + to_string((double)col_freq_hrow(coor)));
                    ...
                    for(acoor[0] =0;acoor[0]< target.par_RM.sizes[0]; acoor[0]++){
                     ...
                     if (j == 2){
                        ...
                        for(i = 0; i < data.data[coor[1]].getSize();i++){
                            ...
                            if (auto ite2 = target.rowhid.data[ data.data[coor[1]].deref_key(i)]()) do{
                                acoor[0] = ite2();
                    ...
                    if (auto ite =newhidden.data[coor[1]].mkIterator()) do {if (ite() >= target.par_RM.sizes[1]) {printf("IMPOSSdddIBLBLBLB!!!\n"); curcol.show(); LFH_exit(1);} }while(ite++);
                    ...
                    if (coor[1] >= target.colhid.data.getSize()) LFH_exit(1);
                    ...
                    for(i=0;i<data.data[coor[1]].getSize();i++){
                        ...
                        if (auto ite = target.rowhid.getColumn(coor[0])()) do{
                            ...
                            if (auto ite2 = target.rowhid.getColumn(coor[0])()) do{
                    ...
                    if ((solution[0] <= 0.0)||(solution[0] >= 1.0)) { // solution is outside allowed range... try dropping dimension or just update scale only
                        ...
                        sum[0] = 0.0f;
                        if (auto ite = candidates[1].mkIterator()) do{
                            sum[0] += *ite;
                        }while(ite++);
                    ...
                    sum[0] = 0.0f;
                    if (auto ite = candidates[0].mkIterator()) {
                        do{
                            sum[0] += *ite;
                        }while(ite++);
                    ...
                    if (auto ite = curcol.mkIterator()) do{
                        acoor[1] = ite();
                        for(acoor[0] =0;acoor[0]< target.par_RM.sizes[0]; acoor[0]++){
                            coor[0] = acoor[0];
                            if (freq_rowh[coor[0]] <= col_freq_hrow(coor)- 0.001) foreach.msgs.insert(to_string(coor[0]) + string("got negative!") + to_string((double)col_freq_hrow(coor)));
                    ...
                    if (!ExOp::isValid(lld[2])){
                        foreach.msgs.insert(string("Default Dropout likelihood is NA"));
                        foreach.msgs.insert(string("ite current size is") + to_string(curcol.getSize()));
                    }
                    ...
                    while(true){
                        ...
                        for(j=0;j<2;j++){
                            ...
                            if (candidates[j].hasSameSparsity(curcol)) lld[j] = lld[2];
                                ...
                                if (auto ite = candidates[j].mkIterator()) do{
                                    acoor[1] = ite();
                                    for(acoor[0] =0;acoor[0]< target.par_RM.sizes[0]; acoor[0]++){
                                        coor[0] = acoor[0];
                                ...
                                if (!ExOp::isValid(lld[j])) foreach.msgs.insert(string("Canditate dropoutLL is invalid! ")+ to_string(j));
                    ...
                    //    for(coor[0]=0;coor[0] < new_nbmax_h.getSize();coor[0]++) new_nbmax_h(coor) = row_nbmax_hcol(coor);
                        ...
                        if (j != 2){
                            if (candidates[j].hasSameSparsity(curcol)) update_type[coor[1]] = 'S';
                                ...
                                if (auto ite = curcol.mkIterator()) do{freq_diff[threadID][ite()] -= tosolve_num2[0];} while(ite++);
                                if (auto ite = candidates[j].mkIterator()) do{freq_diff[threadID][ite()] += tosolve_num2[1];} while(ite++);
                                ...
                                for(i = 0; i < data.data[coor[1]].getSize();i++){
                                    ...
                                    if (auto ite2 = target.rowhid.data[ data.data[coor[1]].deref_key(i)].mkIterator()) do{
                                        acoor[0] = ite2();
                                        if (auto ite = curcol.mkIterator()) do{acoor[1] = ite(); nonzero_freq_diff[threadID](acoor) -= tosolve_num2[0];} while(ite++);
                                        if (auto ite = candidates[j].mkIterator()) do{acoor[1] = ite(); nonzero_freq_diff[threadID](acoor) += tosolve_num2[1];} while(ite++);
                                    }while(ite2++);
                                }
                                nb_flags[threadID][2]++;
                                /*
                        ...
                        if (j != 2) break;
                        if (insist == 0) {
                            if (candidates[1] == curcol) {update_type[coor[1]] = 'F'; foreach.msgs.insert(string("Failed to update scale alone!\n")); break;}
                        ...
                        if (auto ite = candidates[0].mkIterator()) do{
                            *ite *= 0.5;
                        }while(ite++);
                        d_scale[0] *= 0.5;
                    } // insist loop end
                    ...
                    /*if ((coor[1] % 100) == 57){
                        foreach.msgs.insert( to_string((int) coor[1]) + string("LLs[j]") + to_string((double)lls[j]));
                        foreach.msgs.insert( to_string((int) coor[1]) + string("LLs[3]") + to_string((double)lls[3]));
                    }*/
                    candidates[1].toMemfree();

                    if (auto ite =newhidden.data[coor[1]].mkIterator()) do {if (ite() >= target.par_RM.sizes[1]) {printf("IMPOfsdSSdddIBLBLBLB!!!\n"); curcol.show(); LFH_exit(1);} }while(ite++);



                    future_factor.toMemfree();
                    if (auto ite = newhidden.data[coor[1]].mkIterator()) do{
                    ...
                    if (auto ite = newhidden.data[coor[1]].mkIterator()) do{
                        if (auto ite2 = newhidden.data[coor[1]].mkIterator()) do{
                            if (ite2() > ite()) continue;
                            tmp = (*ite2) * (*ite);
                            uint32_t k;
            ...
            if (!ExOp::isValid(lls[0])) foreach.msgs.insert( string("thread")+  to_string(threadID) +  string("got invalid likelihood"));
            ll[threadID][0] += lls[0];
            ll[threadID][1] += lls[0];
            ll[threadID][4] += lls[0];
        break;
        case 1: // time for rows
            ...
            for(coor[1]=rowrange[threadID];coor[1] < rowrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                //printf("d%ie%if\n", threadID, coor[1]);
                if (trdata.data[coor[1]].getSize() > target.colhid.getNBcols()) {printf("Holly Molly! %i and %i",trdata.data[coor[1]].getSize(), target.colhid.getNBcols());LFH_exit(1);}
                //printf("cur coor %i/%i\n", coor[1], target.colhid.getNBcols());fflush(stdout);
                ...
                if (!ExOp::isValid(target.par_rowscale[coor[1]][0])){
                    ...
                    // try 2 random alternatives
                     ...
                     for(acoor[1]=0;acoor[1]< target.par_RM.sizes[1]; acoor[1]++){
                         coor[0] = acoor[1];
                         if (freq_colh[coor[0]] <= row_freq_hcol(coor)- 0.001) foreach.msgs.insert(string("got negative!") + to_string(freq_colh[coor[0]] - row_freq_hcol(coor)));
                     ...
                     if (j == 2){
                        ...
                        for(i = 0; i < trdata.data[coor[1]].getSize();i++){
                            ...
                            if (auto ite2 = target.colhid.data[trdata.data[coor[1]].deref_key(i)]()) do{
                                acoor[1] = ite2();
                   ...
                   // printf("loopya\n"); fflush(stdout);
                    for(i=0;i<trdata.data[coor[1]].getSize();i++){
                        ...
                        if (auto ite = target.colhid.getColumn(coor[0])()) do{
                            future_statistics[2][ite()][0] += ld.evilterm_alt.d[0] * (*ite);
                            future_statistics[2][ite()][1] += ld.evilterm_alt.d[1] * (*ite);
                            trix_input[0] = ld.evilterms[3] * (*ite);
                            trix_input[1] = ld.evilterms[4] * (*ite);
                            trix_input[2] = ld.evilterms[5] * (*ite) * 0.5;
                            if (auto ite2 = target.colhid.getColumn(coor[0])()) do{
                                if (ite2() > ite()) continue;
                                future_statistic_trix[2].data[ite2() + ((ite() * (ite() + 1)) >> 1)] += trix_input * (*ite2);
                            }while(ite2++);
                    ...
                    while(true){
                            ...
                            if (candidates[j].hasSameSparsity(curcol)) lld[j] = lld[2];
                                ...
                                if (auto ite = candidates[j].mkIterator()) do{
                                    ...
                                    for(acoor[1] =0;acoor[1]< target.par_RM.sizes[1]; acoor[1]++){
                                        ...
                                        if ((!ExOp::isValid(log(target.par_D(acoor))))||(!ExOp::isValid(log(1.0 - target.par_D(acoor))))) foreach.msgs.insert(string("got problem! with " + to_string(acoor[0])+ string(" ")+  to_string(acoor[1]))) ;
                        ...
                        if (j != 2){
                            if (candidates[j].hasSameSparsity(curcol)) update_type[coor[1]] = 'S';
                                ...
                                tosolve_num2[1] = 1.0 / candidates[j].getSize();
                                if (auto ite = curcol.mkIterator()) do{ freq_diff[threadID][ite()] -= tosolve_num2[0];} while(ite++);
                                if (auto ite = candidates[j].mkIterator()) do{acoor[0] = ite(); freq_diff[threadID][ite()] += tosolve_num2[1];} while(ite++);
                                for(i = 0; i < trdata.data[coor[1]].getSize();i++){
                                    ...
                                    if (auto ite2 = target.colhid.data[trdata.data[coor[1]].deref_key(i)].mkIterator()) do{
                                        acoor[1] = ite2();
                                        if (auto ite = curcol.mkIterator()) do{acoor[0] = ite(); nonzero_freq_diff[threadID](acoor) -= tosolve_num2[0];} while(ite++);
                                        if (auto ite = candidates[j].mkIterator()) do{acoor[0] = ite(); nonzero_freq_diff[threadID](acoor) += tosolve_num2[1];} while(ite++);
                                    }while(ite2++);
                                }
                                nb_flags[threadID][2]++;
                            /*
                        ...
                        if (j != 2) break;
                        if (insist == 0) {
                            if (candidates[1] == curcol) {foreach.msgs.insert(string("Failed to update scale alone!\n")); update_type[coor[1]] = 'F'; break;}
                            candidates[0].toMemmove(candidates[1]);
                        }
                        if (insist++ > 10) {update_type[coor[1]] = 'F'; break;}
        ...
        case 2:
            for(coor[1]=rowrange[threadID];coor[1] < rowrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                if (auto ite = trdata.data[coor[1]]()) do{
                    if (update_type[ite()] == 'D'){
                        ...
                        if (auto ite2 = newhidden.data[ite()]()) do {coor[0] = ite2(); row_freq_hcol(coor) += tosolve_num2[0];} while(ite2++);
                        if (auto ite2 = target.colhid.data[ite()]()) do {coor[0] = ite2(); row_freq_hcol(coor) -= tosolve_num2[1];} while(ite2++);
                    }
                }while(ite++);
            }
            /* to check the update...
            for(coor[1]=rowrange[threadID];coor[1] < rowrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                if (auto ite = trdata.data[coor[1]].getIterator()) do{
                    ...
                    if (auto ite2 = newhidden.data[ite()]()) do{coor[0] = ite2(); row_freq_hcol(coor) -= tmp;} while(ite2++);
                }while(ite++);
            }*/
        ...
        case 3:
            for(coor[1]=colrange[threadID];coor[1] < colrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                if (auto ite = data.data[coor[1]]()) do{
                    if (update_type[ite()] == 'D'){
                    tosolve_num2[0] = 1.0 / newhidden.data[ite()].getSize();
                    tosolve_num2[1] = 1.0 / target.rowhid.data[ite()].getSize();
                    if (auto ite2 = newhidden.data[ite()]()) do {coor[0] = ite2(); col_freq_hrow(coor) += tosolve_num2[0];} while(ite2++);
                    if (auto ite2 = target.rowhid.data[ite()]()) do {coor[0] = ite2(); col_freq_hrow(coor) -= tosolve_num2[1];} while(ite2++);
                    }
                }while(ite++);
            }
        ...
        case 4:
            nonzero_freq_diff[threadID].toZero();
            for(i=colrange[threadID];i < colrange[threadID+1];i++){ foreach.updateProgress(threadID);
                if (data.data[i].getSize() == 0){
                    ...
                    if (auto ite = data.data[i]()) do{
                        ...
                        if (auto inrte = target.rowhid.data[ite()]()) do{coor[0] = inrte(); col_freq_hrow(coor) += tmp;} while(inrte++);
                        tmp = 1.0 / (target.colhid.data[i].getSize() * target.rowhid.data[ite()].getSize());
                        if (auto inrte = target.rowhid.data[ite()]()) do{
                            coor[0] = inrte();
                            if (auto inrte2 = target.colhid.data[i]()) do{coor[1] = inrte2(); nonzero_freq_diff[threadID](coor)+= tmp;} while(inrte2++);
                        }while(inrte++);
                    }while (ite++);
                }
            }
            ...
            for(coor[1]=rowrange[threadID];coor[1] < rowrange[threadID+1];coor[1]++){ foreach.updateProgress(threadID);
                if (trdata.data[coor[1]].getSize() == 0){
                    ...
                    if (auto ite = trdata.data[coor[1]]()) do{
                        ...
                        if (auto inrte = target.colhid.data[ite()]()) do{coor[0] = inrte(); row_freq_hcol(coor) += tmp;} while(inrte++);
                    }while(ite++);
                }
            }
        break;
...
LFHTEMP void BiClassifier<C,R,S>::run2D_EM_v4(const SparseMatrix<C>& data, const Vector<uint32_t> &excl_list,  uint32_t nbstep, bool use_heavy_prior){
    BiClassifier<C,R,S>::Task_run2D_EM_v4 lotask(*this,data,foreach.getThreadArraySize(), use_heavy_prior,excl_list);
    Tuple<uint32_t, 2u> coor, coor2, coorC, coorR;
    double tmp;
    uint32_t i,j,k;
    i=0;
    for(coor[1]=0;coor[1]<rowhid.getNBcols();coor[1]++) {
        ...
        if (auto ite = rowhid.data[coor[1]].mkIterator()) do{lotask.freq_rowh[ite()] += tmp;}while(ite++);
    }
    ...
    for(coor[1]=0;coor[1]<colhid.getNBcols();coor[1]++){
        ...
        if (auto ite = colhid.data[coor[1]].mkIterator()) do{lotask.freq_colh[ite()] += tmp;}while(ite++);
    }
    ...
    lotask.taskID = 4;
    foreach.startProgress("Populating Search Scope", lotask.colrange.last() + lotask.rowrange.last());
...
/*
 ...
 //       scp.new_nbmax_h.setSizes((scp.istimefor_columns)? scp.row_nbmax_hcol.getDims() : scp.col_nbmax_hrow.getDims()  );
       ...
       /* if (fuz != 0){
        lotask.taskID ^= 1;
        foreach.startProgress(((lotask.taskID == 0) ? "Fake Updating Columns" : "Fake Updating Rows"), (lotask.taskID == 0) ? lotask.colrange.last() : lotask.rowrange.last() );

        //for(i=foreach.getThreadArraySize()-1;i>0;i--) foreach.submit(std::bind(&Task::operator(), std::ref(lotask), i) );
        //foreach.submit_ThenWait(std::bind(&Task::operator(), std::ref(lotask), 0));

        //foreach.submit_Array(lotask,foreach.getThreadArraySize());
        for(i=foreach.getThreadArraySize()-1;i>0;i--) foreach.submit(lotask , i);
        foreach.submit_ThenWait(lotask, 0);
        ...
        foreach.startProgress(((lotask.taskID == 0) ? "Updating Columns" : "Updating Rows"), (lotask.taskID == 0) ? lotask.colrange.last() : lotask.rowrange.last() );
      ...
      //  printf("total local ll %e %e\n", lotask.ll[0][0], lotask.ll[0][1]);
        ...
        if ((old_LL < lotask.ll[0][1])||(fuz == 0)||((lotask.tmptmp)&&(fuz == (nbstep/2)))){
            ...
            foreach.startProgress(((lotask.taskID == 2) ? "Accepting new column states" : "Accepting new row states"), (lotask.taskID == 2) ? lotask.rowrange.last() : lotask.colrange.last());
            for(i=foreach.getThreadArraySize()-1;i>0;i--) foreach.submit(lotask , i);
            foreach.submit_ThenWait(lotask, 0);
...
LFHTEMP void BiClassifier<C,R,S>::run2D_EM_v5(const SparseMatrix<C>& data, const Vector<uint32_t> &excl_list, const Vector<uint16_t> &cellbatch,  uint32_t nbstep, bool use_heavy_prior){

class Task_run2D_EM_v5{
    public:
    const BiClassifier<C,R,S> &target;
    const SparseMatrix<C>& data;
    SparseMatrix<C> trdata;
...
// (par_RM.sizes[0])
    ...
    */
    ...
    Task_run2D_EM_v5(const BiClassifier<C,R,S> &_target, const SparseMatrix<C>& _data, uint32_t _nbthread, bool _use_heavy_prior, const Vector<uint32_t> &_excl):target(_target), data(_data), use_heavy_prior(_use_heavy_prior){
        trdata = _data.mkTranspose();
        rowrange.setSize(_nbthread+1);
        colrange.setSize(_nbthread+1);
        alphas.setSize(_nbthread).toOne() *= 0.001;
    ...
    operator std::function<uint32_t(uint32_t)> (){using std::placeholders::_1; return std::bind(&Task_run2D_EM_v5::operator(),this,_1);}
...
/*
    ...
    Task_run2D_EM_v5 lotask(*this,data,foreach.getThreadArraySize(), use_heavy_prior,excl_list);
    Tuple<uint32_t, 2u> coor, coor2, coorC, coorR;
    double tmp;
    uint32_t i,j,k;
    i=0;
            ...
            if (!ExOp::isValid(target.par_colscale[coor[1]][0])){
                ...
                if (candidates[0].getSize() < 2u) { // stuck in a corner,... that's good but...
                    ...
                    do{
                        ...
                        if (torem.getSize() == 0u) {
                            sum[0] = 0.0;
                            if (auto ite = candidates[0].mkIterator()) do{sum[0] += (*ite);}while(ite++);
                            if (fabs(sum[0]) < 0.0001) break;
                            if ((coor[0]++) == 5) {
                                coor[0] = 0;
                                if (auto ite = candidates[0].mkIterator()) do{if ((*ite)>= 0) coor[0]++;}while(ite++);
                                if (auto ite = candidates[0].mkIterator()) do{(*ite) = ((*ite)>= 0) ?  candidates[0].getSize() - coor[0]: coor[0];}while(ite++);
                                if (auto ite = candidates[0].mkIterator()) do{(*ite) /=  candidates[0].getSize();}while(ite++);
                    ...
                    if (auto ite = candidates[0].mkIterator()){
                        ...
                        if (!ignore_drop){
                            if (candidates[0].deref_key(0) != curcol.deref_key(0)) {
                                ...
                                if (auto ite = candidates[1].mkIterator()) {
                                    sum[0] = 0.0;
                                    do{
                                        sum[0] += *ite;
                                    }while(ite++);
                ...
                //    for(coor[0]=0;coor[0] < new_nbmax_h.getSize();coor[0]++) new_nbmax_h(coor) = row_nbmax_hcol(coor);
                    ...
                    if (j != 2) break;
                    if (insist++ > 10) break;
                    if (auto ite = curcol.mkIterator()) do{ candidates[0][ite()] += *ite; }while(ite++);
                    if (auto ite = candidates[0].mkIterator()) do{ *ite *= 0.5; }while(ite++);
                    d_scale *= 0.5;
...
LFHTEMP void BiClassifier<C,R,S>::doRecenter(){
    ...
    for(i=0;i< par_rowscale.getSize();i++){
        ...
        if (auto ite = rowhid.data[i].mkIterator()) do{
    ...
    for(i=0;i< par_colscale.getSize();i++){
        ...
        if (auto ite = colhid.data[i].mkIterator()) do{
            hidcol_frequency[ite()] += *ite;
        }while(ite++);
        counts[1]++;
    }
    hidcol_frequency /= counts[1];
    if (auto ite = par_RM.mkIterator()) do{
...
LFHTEMP class BiClassifier<C,R,S>::normalizeMK2_task{
    ...
    uint32_t operator()(uint32_t threadID){
        ...
        int i;
        uint32_t row;
        for(uint32_t col = colrange[threadID]; col < colrange[threadID+1]; col++){foreach.updateProgress(threadID);
            if (auto ite =  target.colhid.getColumn(col)()) do {if (ite() >= target.par_RM.sizes[1]) printf("coc c%i!\n", ite());}while(ite++);
            ...
            for(i =0; i<input.data[col].getSize();i++){
                ...
                if (auto ite =  target.rowhid.getColumn(row)()) do {if (ite() >= target.par_RM.sizes[0]) printf("ror r%i!\n", ite());}while(ite++);
...
LFHTEMP SparseMatrix<double> BiClassifier<C,R,S>::normalizeMK2(const SparseMatrix<uint32_t> &input)const{SparseMatrix<double> fout;
     BiClassifier<C,R,S>::normalizeMK2_task lotask(*this, input, fout, foreach.getThreadArraySize());
     fout.setNBcols(input.data.getSize());

    foreach.startProgress("Normalizing", lotask.colrange.last());
    for(int i=foreach.getThreadArraySize()-1;i>0;i--) foreach.submit(lotask , i);
    foreach.submit_ThenWait(lotask, 0);
return fout;}
...
//LFHTEMP template<int F> void BiClassifier<C,R,S>::Shared_EMSCOPE::initCounts(const SparseMatrix<C, F>& data, SparseMatrix<double, 0u> &rowhid, SparseMatrix<double, 0u> &colhid){
    if (excl_list.getSize() == 0){
        ...
        for(uint32_t i=0;i<excl_list.getSize();i++) which.addEntry(excl_list[i]);
        for(uint32_t i=0;i<_data.getNBcols();i++){
            if (auto ite = _data.data[i].mkIterator()) do{
                if (which.find(ite()) == 0xFFFFFFFF) data.data[i][ite()] = *ite;
            } while(ite++);
        }
    }
    ...
    if (scp->istimefor_columns){
        ...
        for(coor[1]=colrange[0];coor[1] < colrange[1];coor[1]++){
            ...
            if (!ExOp::isValid(scp->target.par_colscale[coor[1]][0])){
                ...
                if (coor[1] >= scp->target.colhid.data.getSize()) LFH_exit(1);
                // Step 1 cache line specific data
                ...
                for(i=0;i<scp->data.data[coor[1]].getSize();i++){
                    ...
                    coeff = curproj[0].mkInnerProdOLD(scp->target.rowhid.getColumn(coor[0]));
                    ...
                    if (auto ite = scp->target.rowhid.getColumn(coor[0]).mkIterator()) do{
                        ...
                        if (auto ite2 = scp->target.rowhid.getColumn(coor[0]).mkIterator()) do{
                            if (ite2() > ite()) continue;
                            future_statistic_trix[2].data[ite2() + ((ite() * (ite() + 1)) >> 1)] += trix_input * (*ite2);
                        }while(ite2++);
                    }while(ite++);
                ...
                if (candidates[0].getSize() < 2u) { // stuck in a corner,... that's good but...
                    ...
                    if (auto ite = candidates[0].mkIterator()){
                        ...
                        for(i=0;i<scp->data.data[coor[1]].getSize();i++){
                            coor[0]= scp->data.data[coor[1]].deref_key(i);
                            if (!ExOp::isValid(scp->target.par_rowscale[coor[0]])) continue;
                            coeff = rever_buf[i].mkInnerProdOLD(candidates[0]);
                            tosolve_num2[0] += coeff[0] * dd_buf[i][1]+ coeff[1] * dd_buf[i][2];
                            tosolve_den2.data[0] += dd_buf[i][3] * coeff[0] * coeff[0] + dd_buf[i][4] * coeff[1] * coeff[1] + dd_buf[i][5] * coeff[0] * coeff[1];
                            tosolve_den2.data[1] += dd_buf[i][3] * coeff[0] + dd_buf[i][5] * coeff[1] * 0.5;
                            tosolve_den2.data[3] += dd_buf[i][4] * coeff[1] + dd_buf[i][5] * coeff[0] * 0.5;
                ...
                while(true){
                    ...
                    for(i=0;i<scp->data.data[coor[1]].getSize();i++){
                        ...
                        if (!ExOp::isValid(scp->target.par_rowscale[coor[0]])) continue;
                        for(j=0;j<2;j++){
                            coeff = curproj[j].mkInnerProdOLD(scp->target.rowhid.getColumn(coor[0]));
                            coeff += scp->target.par_rowscale[coor[0]];
                            coeff += scp->target.par_colscale[coor[1]];
                            coeff += d_scale;
                ...
                future_factor.toMemfree();
                if (auto ite = scp->newhidden.data[coor[1]].mkIterator()) do{
                    future_factor[ite()][0] = *ite; future_factor[ite()][1] = *ite;
                }while(ite++);
                ...
                if (auto ite = scp->newhidden.data[coor[1]].mkIterator()) do{
                    if (auto ite2 = scp->newhidden.data[coor[1]].mkIterator()) do{
                        if (ite2() > ite()) continue;
                        tmp = (*ite2) * (*ite);
                        uint32_t k;
        ...
        for(coor[1]=rowrange[0];coor[1] < rowrange[1];coor[1]++){
            ...
            if (scp->trdata.data[coor[1]].getSize() > scp->target.colhid.getNBcols()) {printf("Holly Molly! %i and %i",scp->trdata.data[coor[1]].getSize(), scp->target.colhid.getNBcols());LFH_exit(1);}
            //printf("cur coor %i/%i\n", coor[1], scp->target.colhid.getNBcols());fflush(stdout);
            ...
            if (!ExOp::isValid(scp->target.par_rowscale[coor[1]][0])){
               ...
               // printf("loopya\n"); fflush(stdout);
                for(i=0;i<scp->trdata.data[coor[1]].getSize();i++){
                    ...
                    coeff = curproj[0].mkInnerProdOLD(scp->target.colhid.getColumn(coor[0]) );
                    ...
                    if (auto ite = rever.mkIterator()) do{
                        RM_deriv[ite()][0] += (*ite)[0] * ld.evilterms[1];
                        RM_deriv[ite()][1] += (*ite)[1] * ld.evilterms[2];
                    }while(ite++);


                    if (auto ite = scp->target.colhid.getColumn(coor[0]).mkIterator()) do{
                        future_statistics[2][ite()][0] += ld.evilterm_alt.d[0] * (*ite);
                        future_statistics[2][ite()][1] += ld.evilterm_alt.d[1] * (*ite);
                        trix_input[0] = ld.evilterms[3] * (*ite);
                        trix_input[1] = ld.evilterms[4] * (*ite);
                        trix_input[2] = ld.evilterms[5] * (*ite) * 0.5;
                        if (auto ite2 = scp->target.colhid.getColumn(coor[0]).mkIterator()) do{
                            if (ite2() > ite()) continue;
                            future_statistic_trix[2].data[ite2() + ((ite() * (ite() + 1)) >> 1)] += trix_input * (*ite2);
                        }while(ite2++);
                ...
                while(true){
                    ...
                    for(i=0;i<scp->trdata.data[coor[1]].getSize();i++){
                        ...
                        for(j=0;j<2;j++){
                            coeff = curproj[j].mkInnerProdOLD(scp->target.colhid.getColumn(coor[0]));
                            coeff += scp->target.par_colscale[coor[0]];
                            coeff += scp->target.par_rowscale[coor[1]];
                            coeff += d_scale;
                            deflatedNBterm_wr_f_d2(ld.evilterms, scp->trdata.data[coor[1]].deref(i), coeff[0], coeff[1]);
