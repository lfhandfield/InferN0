...
LFHTEMP void AsyncBuffer<C,S>::show(FILE*f, int level)const{
	fprintf(f,"Asyncbuffer %i and %i\n", pos_wr, pos_rd);
}
/*
#undef LFHTEMP
#define LFHTEMP template <class C>
LFHTEMP void AsyncDeque<C>::push_back(const C& val){std::lock_guard<std::mutex>(mut); data.push_back(val); }
LFHTEMP void AsyncDeque<C>::push_front(const C& val){std::lock_guard<std::mutex>(mut); data.push_front(val); }
LFHTEMP void AsyncDeque<C>::pop_back(){std::lock_guard<std::mutex>(mut); data.pop_back(); }
LFHTEMP void AsyncDeque<C>::pop_front(){std::lock_guard<std::mutex>(mut); data.pop_front(); }
*/
...
LFHTEMP ERRCODE Tuple<C,TSIZE,Cflag>::save( FILE *f) const {
	if (ExCo<C>::IsPOD::value) return (fwrite(data,sizeof(C),TSIZE,f)== TSIZE) ? 0 : 1;
	ERRCODE fout = ExOp::save(data[0],f);
	for(unsigned int i=1;i<TSIZE;i++) fout |= ExOp::save(data[i],f);
}
LFHTEMP ERRCODE Tuple<C,TSIZE,Cflag>::load( FILE *f) { ERRCODE fout =0;
	if (ExCo<C>::IsPOD::value) fout |= fread(data,sizeof(C),TSIZE,f) == TSIZE ? 0 : 1;
	else for(unsigned int i=0;i<TSIZE;i++) fout |= ExOp::load(data[i],f);
return fout;}
...
LFHTEMP template<class O> auto Tuple<C,TSIZE,Cflag>::operator+(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkAdd(data[0], other.data[0])),TSIZE,Cflag>{
    Tuple< decltype( ExOp::mkAdd(data[0], other.data[0])),TSIZE,Cflag> f_out;
    for(unsigned int i=0;i<TSIZE;i++) f_out[i] = ExOp::mkAdd((*this)[i],other[i]);
return( f_out );}
LFHTEMP template<class O> auto Tuple<C,TSIZE,Cflag>::operator-(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkSubt(data[0], other.data[0])),TSIZE,Cflag>{
    Tuple< decltype( ExOp::mkSubt(data[0], other.data[0])),TSIZE,Cflag> f_out;
    for(unsigned int i=0;i<TSIZE;i++) f_out[i] = ExOp::mkSubt((*this)[i], other[i]);
return( f_out );}
LFHTEMP template<class O> auto Tuple<C,TSIZE,Cflag>::operator*(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkMult(data[0], other.data[0])),TSIZE,Cflag>{
    Tuple< decltype( ExOp::mkMult(data[0], other.data[0])),TSIZE,Cflag> f_out;
    for(unsigned int i=0;i<TSIZE;i++) f_out[i] = ExOp::mkMult((*this)[i], other[i]);
return( f_out );}
LFHTEMP template<class O> auto Tuple<C,TSIZE,Cflag>::operator/(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkDivi(data[0], other.data[0])),TSIZE,Cflag>{
    Tuple< decltype( ExOp::mkDivi(data[0], other.data[0])),TSIZE,Cflag> f_out;
...
LFHTEMP GaussScope< Tuple<C,TSIZE> , typename MT_IFTYPE<ExCo<C>::IS_COMMUTATIVE::value, Tuple<C, TEMPLATE_TRIANGLE_NUMBER<TSIZE, 2>::ans> , TMatrix<C,TSIZE,TSIZE> >::TYPE >  Tuple<C,TSIZE,Cflag>::mkgaussstat(double &w) const{
    ...
    return GaussScope< Tuple<C,TSIZE> , typename MT_IFTYPE<ExCo<C>::IS_COMMUTATIVE::value, Tuple<C, TEMPLATE_TRIANGLE_NUMBER<TSIZE, 2>::ans> , TMatrix<C,TSIZE,TSIZE> >::TYPE >(*this, outer ,w);
    }
...
// Super Operators!
LFHTEMP template<class I> void Tuple<C,TSIZE,Cflag>::operator() (Oper1<I> const & op){ // not a match
for(unsigned int i = 0;i < TSIZE;i++) data[i](op);
}
LFHTEMP void Tuple<C,TSIZE,Cflag>::operator() (Oper1< C> const & op){ // match
for(unsigned int i = 0;i < TSIZE;i++)  op(data[i], data[i]);
}
LFHTEMP template<class A_1, class A_2, class C_2, unsigned int TSIZE_2> void Tuple<C,TSIZE,Cflag>::operator() (Oper2<A_1,A_2> const & op, Tuple<C_2, TSIZE_2,Cflag> const & a_2 ){ // not a match
unsigned int i = (TSIZE > TSIZE_2) ? TSIZE_2 : TSIZE;
for(i--;i!=ExCo<unsigned int>::mkMaximum();i--) data[i](op,a_2.data[i]);
}

LFHTEMP template<class C_2, unsigned int TSIZE_2> void Tuple<C,TSIZE,Cflag>::operator() (Oper2<C,C_2> const & op, Tuple<C_2, TSIZE_2,Cflag> const & a_2){ // match
unsigned int i = (TSIZE > TSIZE_2) ? TSIZE_2 : TSIZE;
for(i--;i!=ExCo<unsigned int>::mkMaximum();i--) op(data[i], a_2.data[i]);
}
LFHTEMP template<class A_1, class A_2, class C_2, unsigned int TSIZE_2> void Tuple<C,TSIZE,Cflag>::operator() (Oper2<A_1,A_2> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2 ){ // not a match
unsigned int i = (TSIZE > TSIZE_2) ? TSIZE_2 : TSIZE;
for(i--;i!=ExCo<unsigned int>::mkMaximum();i--) data[i](op,a_2.data[i]);
}
LFHTEMP template<class C_2, unsigned int TSIZE_2> void Tuple<C,TSIZE,Cflag>::operator() (Oper2<C,C_2> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2){ // match
unsigned int i = (TSIZE > TSIZE_2) ? TSIZE_2 : TSIZE;
for(i--;i!=ExCo<unsigned int>::mkMaximum();i--) op(data[i], a_2.data[i]);
}

LFHTEMP template<class A_1, class A_2, class A_3, class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<A_1,A_2,A_3> const & op, Tuple<C_2, TSIZE_2,Cflag> const & a_2, Tuple<C_3, TSIZE_3,Cflag> const & a_3 ){ // not a match
...
LFHTEMP template<class A_1, class A_2, class A_3, class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<A_1,A_2,A_3> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2, Tuple<C_3, TSIZE_3,Cflag> const & a_3 ){ // not a match
	int i = (TSIZE > TSIZE_2) ? (TSIZE_2 > TSIZE_3 ? TSIZE_3-1 : TSIZE_2 -1) : (TSIZE > TSIZE_3 ? TSIZE_3-1 : TSIZE - 1);
	for(;i>=0;i--) data[i](op,a_2.data[i],a_3.data[i]);
}
LFHTEMP template<class A_1, class A_2, class A_3, class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<A_1,A_2,A_3> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2, Tuple<C_3, TSIZE_3,Cflag> & a_3 ){ // not a match
	int i = (TSIZE > TSIZE_2) ? (TSIZE_2 > TSIZE_3 ? TSIZE_3-1 : TSIZE_2 -1) : (TSIZE > TSIZE_3 ? TSIZE_3-1 : TSIZE - 1);
	for(;i>=0;i--) data[i](op,a_2.data[i],a_3.data[i]);
}
LFHTEMP template<class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<C,C_2,C_3> const & op, Tuple<C_2, TSIZE_2,Cflag> const & a_2, Tuple<C_3, TSIZE_3,Cflag> const & a_3){ // match
	int i = (TSIZE > TSIZE_2) ? (TSIZE_2 > TSIZE_3 ? TSIZE_3-1 : TSIZE_2 -1) : (TSIZE > TSIZE_3 ? TSIZE_3-1 : TSIZE - 1);
	for(;i>=0;i--) op(data[i], a_2.data[i], a_3.data[i]);
}
LFHTEMP template<class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<C,C_2,C_3> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2, Tuple<C_3, TSIZE_3,Cflag> const & a_3){ // match
	int i = (TSIZE > TSIZE_2) ? (TSIZE_2 > TSIZE_3 ? TSIZE_3-1 : TSIZE_2 -1) : (TSIZE > TSIZE_3 ? TSIZE_3-1 : TSIZE - 1);
	for(;i>=0;i--) op(data[i], a_2.data[i], a_3.data[i]);
}

LFHTEMP template<class C_2, unsigned int TSIZE_2, class C_3, unsigned int TSIZE_3> void Tuple<C,TSIZE,Cflag>::operator() (Oper3<C,C_2,C_3> const & op, Tuple<C_2, TSIZE_2,Cflag> & a_2, Tuple<C_3, TSIZE_3,Cflag> & a_3){ // match
	int i = (TSIZE > TSIZE_2) ? (TSIZE_2 > TSIZE_3 ? TSIZE_3-1 : TSIZE_2 -1) : (TSIZE > TSIZE_3 ? TSIZE_3-1 : TSIZE - 1);
	for(;i>=0;i--) op(data[i], a_2.data[i], a_3.data[i]);
}

LFHTEMP void Tuple<C,TSIZE,Cflag>::fourierTransform_routine(){
...
LFHTEMP const C& Tuple<C,0u,Cflag>::operator[](unsigned int index)const{
	LFHDebug(index >=tup_size,"Out of bound access %i > %i for tuple!", index, tup_size);
return data[index];}
LFHTEMP C& Tuple<C,0u,Cflag>::operator[](unsigned int index){
	LFHDebug(index >=tup_size,"Out of bound access %i > %i for tuple!", index, tup_size);
return data[index];}
LFHTEMP const C& Tuple<C,0u,Cflag>::operator[](int index)const{
	LFHDebug(index >=tup_size,"Out of bound access %i > %i for tuple!", index, tup_size);
return data[index];}
LFHTEMP C& Tuple<C,0u,Cflag>::operator[](int index){
	LFHDebug(index >=tup_size,"Out of bound access %i > %i for tuple!", index, tup_size);
return data[index];}
...
LFHTEMP Tuple<C,0u,Cflag>& Tuple<C,0u,Cflag>::removeAt(uint32_t position){
    if (position >= tup_size) return *this;
    tup_size--;
    if (tup_size == 0) delete[](data);
    else{
        C* ndata = new C[tup_size];
        uint32_t i;
        for(i =0; i< position; i++) ndata[i] = std::move(data[i]);
        for(; i< tup_size; i++) ndata[i] = std::move(data[i+1]);
        delete[](data);
        data = ndata;
    }
return(*this);}
...
LFHTEMP Tuple<C,0u, Cflag> Tuple<C,0u,Cflag>::operator+(const Tuple<C,0u, Cflag> &other)const{ Tuple<C,0u, Cflag> fout;
	unsigned int i = 0;
	if (this->tup_size > other.tup_size){
		fout.setSize(this->tup_size);
		for(i =0; i < other.tup_size ; i++) fout[i] = data[i] + other[i];
		for(; i < this->tup_size ; i++) fout[i] = data[i];
	}else{
		fout.setSize(other.tup_size);
		for(i =0; i < this->tup_size ; i++) fout[i] = data[i] + other[i];
		for(; i < other.tup_size ; i++) fout[i] = other[i];
	}
return fout;}
LFHTEMP Tuple<C,0u, Cflag> Tuple<C,0u,Cflag>::operator-(const Tuple<C,0u, Cflag> &other)const{ Tuple<C,0u, Cflag> fout;
	unsigned int i = 0;
...
LFHTEMP Tuple<C,0u, Cflag> Tuple<C,0u,Cflag>::operator*(const Tuple<C,0u, Cflag> &other)const{ Tuple<C,0u, Cflag> fout;
	if (this->tup_size > other.tup_size){
		fout.setSize(this->tup_size);
		for(int i =0; i < other.tup_size ; i++) fout[i] = data[i] * other[i];
	}else{
		fout.setSize(other.tup_size);
		for(int i =0; i < this->tup_size ; i++) fout[i] = data[i] * other[i];
	}
return fout;}
LFHTEMP Tuple<C,0u, Cflag> Tuple<C,0u,Cflag>::operator/(const Tuple<C,0u, Cflag> &other)const{ Tuple<C,0u, Cflag> fout;
	int i = 0;
	...
	if (this->tup_size > other.tup_size){
		for(i =0; i < other.tup_size ; i++) fout[i] = data[i] / other[i];
		for(; i < this->tup_size ; i++) fout[i] = data[i] / ExCo<C>::mkZero();
	}else{
		fout.setSize(other.tup_size);
		for(i =0; i < this->tup_size ; i++) fout[i] = data[i] / other[i];
	}
...
LFHTEMP template<class O, uint32_t TSIZE> auto Tuple<C,0u,Cflag>::operator+(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkAdd(data[0u], other.data[0u])),0u,Cflag>{
    Tuple< decltype( ExOp::mkAdd(data[0u], other.data[0u])),0u,Cflag> f_out;
    for(unsigned int i=0;i<other.getSize();i++) f_out[i] = (*this)[i] + other[i];
return( f_out );}
LFHTEMP template<class O, uint32_t TSIZE> auto Tuple<C,0u,Cflag>::operator-(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkSubt(data[0u], other.data[0u])),0u,Cflag>{
    Tuple< decltype( ExOp::mkSubt(data[0u], other.data[0u])),0u,Cflag> f_out;
    for(unsigned int i=0;i<other.getSize();i++) f_out[i] = (*this)[i] - other[i];
return( f_out );}
LFHTEMP template<class O, uint32_t TSIZE> auto Tuple<C,0u,Cflag>::operator*(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkMult(data[0u], other.data[0u])),0u,Cflag>{
    Tuple< decltype( ExOp::mkMult(data[0u], other.data[0u])),0u,Cflag> f_out;
    for(unsigned int i=0;i<other.getSize();i++) f_out[i] = (*this)[i] * other[i];
return( f_out );}
LFHTEMP template<class O, uint32_t TSIZE> auto Tuple<C,0u,Cflag>::operator/(Tuple<O,TSIZE,Cflag> const & other) const
 -> Tuple< decltype( ExOp::mkDivi(data[0u], other.data[0u])),0u,Cflag>{
    Tuple< decltype( ExOp::mkDivi(data[0u], other.data[0u])),0u,Cflag> f_out;
    for(unsigned int i=0;i<other.getSize();i++) f_out[i] = (*this)[i] / other[i];
return( f_out );}

//LFHTEMP template<class I, ENABLEIF_ITERATOR(I, uint32_t)> auto Tuple<C,0u,Cflag>::mkInnerProd(const I& x) const
// -> decltype(data[0] * (*(x.getIterator()))){
LFHTEMP template<class I, ENABLEIF_ITERATOR(I, uint32_t)> auto Tuple<C,0u,Cflag>::mkInnerProd(I x) const
 ->	decltype(data[0] * (*(x.mkIterator()))){
	decltype(data[0] * (*(x.mkIterator()))) fout;
    uint32_t ite;
    uint32_t ind;
    bool first = true;
    if (auto ite = x.mkIterator()) do{
        if (ite() < this->getSize()){
...
return fout;}
/*LFHTEMP template<class I, ENABLEIF_ITERATORMKR(I, uint32_t)> auto Tuple<C,0u,Cflag>::mkInnerProd(const I& x) const
 ->	decltype(data[0] * (*(x.mkIterator()))){
	decltype(data[0] * (*(x.mkIterator()))) fout;
    uint32_t ite;
    uint32_t ind;
    bool first = true;
    if (auto ite = x.getIterator()) do{
        if (ite() < this->getSize()){
            if (first) fout = (*this)[ite()] * (*ite);
            else fout += (*this)[ite()] * (*ite);
            first = false;
...
LFHTEMP template<class I> auto Tuple<C,0u,Cflag>::mkInnerProdOLD(const I& other, ITERABLE_DEF(I)) const
 -> decltype(data[0] * other[0]){
    decltype(data[0] * other[0]) fout;
    uint32_t ite;
    uint32_t ind;
    bool first = true;
    if (auto ite = other.mkIterator()) do{
        if (ite() < this->getSize()){
            if (first) fout = (*this)[ite()] * (*ite);
            else fout += (*this)[ite()] * (*ite);
            first = false;
...
LFHTEMP template<class O, unsigned int S, Tuple_flag CFLAG2> auto Tuple<C,0u,Cflag>::mkInnerProd(const Tuple<O,S,CFLAG2>&other)const
 -> decltype(data[0] * other.data[0]){
    decltype(data[0] * other.data[0]) fout;
    uint32_t i;
    if ((i = other.getSize()) != this->getSize()) {
		printf("warning, inner product has sizes %i and %i\n", this->getSize(), other.getSize() );
		if (i > this->getSize()) i = this->getSize();
	}
    if (i == 0) ExOp::toZero(fout);
    else{
        fout = data[0] * other.data[0];
        for(i--;i>0;i--) fout += data[i] * other.data[i];
    }
return fout;}
...
LFHTEMP ERRCODE Tuple<C,0u,Cflag>::save( FILE *f) const {
	...
	if (ExCo<C>::IsPOD::value) return (fwrite(data,sizeof(C),tup_size,f) == tup_size) ? 0 : 1;
	ERRCODE fout = ExOp::save(data[0],f);
	for(unsigned int i=1;i<tup_size;i++) fout |= ExOp::save(data[i],f);
	return fout;
}
LFHTEMP ERRCODE Tuple<C,0u,Cflag>::load( FILE *f) { ERRCODE fout =0;
	uint32_t i;
	if ((fout = (fread(&i,sizeof(uint32_t),1,f) == 1) ? 0 : 1)) return fout;
	this->setSize(i);
	if (ExCo<C>::IsPOD::value) {if (tup_size) fout |= fread(data,sizeof(C),tup_size,f);
	}else for(unsigned int i=0;i<tup_size;i++) fout |= ExOp::load(data[i],f);
return fout;}

LFHTEMP ERRCODE Tuple<C,0u,Cflag>::load(const uint8_t * &chunk, const uint8_t* const endpos){
	if ((endpos - chunk) < sizeof(uint32_t)) return 1;
	uint32_t i = *(uint32_t*)chunk;
	this->setSize(i);
	chunk += sizeof(uint32_t);
	for(i = 0;i <tup_size;i++) {
		if (ExOp::load(data[i],chunk,endpos) != 0) return 1;
	}
return 0;}

LFHTEMP ERRCODE Tuple<C,0u,Cflag>::save(uint8_t * &chunk, const uint8_t* const endpos) const{
	if ((endpos - chunk) < 4) return 1;
	*(uint32_t*)chunk = tup_size;
	chunk += sizeof(uint32_t);
	for(uint32_t i=0;i < tup_size;i++) if (ExOp::save(data[i],chunk,endpos)) return 1;
return 0;}
...
#define LFHTEMP template<class C>
/*
LFHTEMP Tensor<C>& Tensor<C>::setShape(const Tuple<uint32_t> &shape){
	if (dims.getSize() != 0) delete[](data);
	dims = shape;
    data = new C[dims.mkProduct()];
return *this;}*/
...
LFHTEMP ERRCODE Tuple<Anything,0u,Cflag>::load( FILE *f) { ERRCODE fout =0;
	unsigned int i;
...
#undef LFHTEMP
#define LFHTEMP template<class C, unsigned int NBD>

LFHTEMP TMatrix<C> RemoteMemory<C,NBD>::eigenVectorOfInnerProduct(int direction, int nbeigen, Tuple<double> *eigenvalues)const{TMatrix<C> fout;
	Trianglix<C> inner;
return fout;}

LFHTEMP RemoteMemory<C,NBD-1> RemoteMemory<C,NBD>::operator[](int slice){ RemoteMemory<C,NBD-1> fout;
	if ((unsigned int)slice >= dims[NBD-1]) {fout.remote_data = NULL; return fout;}
	fout.remote_data = remote_data + offsets[NBD-1] * slice;
	for(uint32_t i =0; i < NBD-1;i++){
		fout.dims[i] = dims[i];
		fout.offsets[i] = offsets[i];
	}
return fout;}
LFHTEMP RemoteMemory<C,NBD-1> RemoteMemory<C,NBD>::operator[](unsigned int slice){ RemoteMemory<C,NBD-1> fout;
	if (slice >= dims[NBD-1]) {fout.remote_data = NULL; return fout;}
	fout.remote_data = remote_data + offsets[NBD-1] * slice;
	for(uint32_t i =0; i < NBD-1;i++){
		fout.dims[i] = dims[i];
		fout.offsets[i] = offsets[i];
...
return fout;}
LFHTEMP RemoteMemory<C, NBD-1> RemoteMemory<C,NBD>::selectSlice(int sl, uint32_t which_dim){ RemoteMemory<C,NBD-1> fout;
	if ((which_dim >= NBD)||((unsigned int)sl >= dims[which_dim])) {fout.remote_data = NULL; return fout;}
	fout.remote_data = remote_data + offsets[which_dim] * sl;
	uint32_t i =0;
	for(; i < which_dim;i++){fout.dims[i] = dims[i];fout.offsets[i] = offsets[i];}
	for(; i < NBD-1;i++){fout.dims[i] = dims[i+1];fout.offsets[i] = offsets[i+1];}
return fout;}
LFHTEMP RemoteMemory<C, NBD-1> RemoteMemory<C,NBD>::selectSlice(unsigned int sl, uint32_t which_dim){RemoteMemory<C,NBD-1> fout;
	if ((which_dim >= NBD)||(sl >= dims[which_dim])) {fout.remote_data = NULL; return fout;}
	fout.remote_data = remote_data + offsets[which_dim] * sl;
	uint32_t i =0;
	for(; i < which_dim;i++){fout.dims[i] = dims[i];fout.offsets[i] = offsets[i];}
	for(; i < NBD-1;i++){fout.dims[i] = dims[i+1];fout.offsets[i] = offsets[i+1];}
...
LFHTEMP RemoteMemory<C, NBD-1> RemoteMemory<C,NBD>::vectorizeOntoDimension(uint32_t intdim){RemoteMemory<C,NBD-1> fout;
	if (intdim >= NBD) {fout.remote_data = NULL; return fout;}
	uint32_t i = dims[intdim] * offsets[intdim];
	uint32_t otherdim;
	for(otherdim=0;otherdim < NBD;otherdim++) if (i == offsets[otherdim]) break;
	if (otherdim >= NBD) {fout.remote_data = NULL; return fout;}
	if (otherdim > intdim){
		for(i=0;i< intdim;i++) {fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i];}
		fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i] * dims[otherdim];
		for(i++;i< otherdim;i++) {fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i];}
		for(;i< NBD-1;i++) {fout.offsets[i] = offsets[i+1]; fout.dims[i] = dims[i+1];}
	}else{
		for(i=0;i< otherdim;i++) {fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i];}
		do{ fout.offsets[i] = offsets[i+1]; fout.dims[i] = dims[i+1];
		i++;}while(i< intdim);
		fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i] * dims[otherdim];
		for(i++;i< NBD-1;i++) {fout.offsets[i] = offsets[i+1]; fout.dims[i] = dims[i+1];}
	}
	fout.remote_data = remote_data ;
return fout;}
LFHTEMP RemoteMemory<C, NBD+1> RemoteMemory<C,NBD>::splitDimension(uint32_t intdim, uint32_t length){RemoteMemory<C,NBD+1> fout;
	if (intdim >= NBD) {fout.remote_data = NULL; return fout;}
	if (dims[intdim] % length != 0) {printf("Dimension[%i] of length %i is not a multiple of %i, cannot split\n", intdim,dims[intdim], length);fout.remote_data = NULL; return fout;}
	fout.remote_data = remote_data;
	int i;
	for(i=0;i< intdim;i++) {fout.offsets[i] = offsets[i]; fout.dims[i] = dims[i];}
	fout.offsets[i] = offsets[i]; fout.dims[i] = length;
	fout.offsets[i+1] = offsets[i]*length; fout.dims[i+1] = dims[i]/length;
	for(i+=2;i< NBD+1;i++) {fout.offsets[i] = offsets[i-1]; fout.dims[i] = dims[i-1];}
return fout;}

#undef LFHTEMP
#define LFHTEMP template<class C>

LFHTEMP template<class I, ENABLEIF_ITERATOR(I, uint32_t)> auto RemoteMemory<C,1u>::mkInnerProd(I x) const
 ->	decltype(remote_data[0] * (*(x.mkIterator()))){
	decltype(remote_data[0] * (*(x.mkIterator()))) fout;
    uint32_t ite;
    uint32_t ind;
    bool first = true;
    if (auto ite = x.mkIterator()) do{
...
LFHTEMP template<class I, ENABLEIF_ITERATOR(I, uint32_t) > RemoteMemory<C, 1u>& RemoteMemory<C,1u>::operator+=(I x){
	if (auto ite = x.mkIterator()) do{
        if (ite() < dims[0]) (*this)[ite()] += (*ite);
    }while(ite++);
return *this;}
LFHTEMP template<class I, ENABLEIF_ITERATOR(I, uint32_t) > RemoteMemory<C, 1u>& RemoteMemory<C,1u>::operator-=(I x){
	if (auto ite = x.mkIterator()) do{
...
LFHTEMP const RemoteMemory<C,1u>& RemoteMemory<C,1u>::show(FILE*f, int level) const{
	fprintf(f, "RemoveMemoryArray: [");
	for(int i=0;i < dims[0];i++) {ExOp::show((*this)[i], f, 1); fprintf(f, "\t");}
	fprintf(f, "]\n");
return *this;}
...
#define LFHTEMP template<class C,unsigned int order>
	...
	LFHTEMP ERRCODE WeightElem<C,order>::load( FILE *f) {return ExOp::load(e,f) | ExOp::load(w,f);}
...
LFHTEMP TMatrix<C,sizex,sizey,TF>& TMatrix<C,sizex,sizey,TF>::operator=(TMatrix<C, sizex, sizey,TF> const & idata){
	for(unsigned int k=0;k<sizex*sizey;k++) data[k] = idata.data[k];
return(*this);}

LFHTEMP	void TMatrix<C,sizex,sizey,TF>::operator()(Tuple<C, sizey> &_out, Tuple<C, sizex> &_in) const{
	_out = (*this) * _in;
}

LFHTEMP void TMatrix<C,sizex,sizey,TF>::derivative(Tuple<C, sizey> &_out, Tuple<C, sizex > &_in, int in_direct) const{
	int i;
...
LFHTEMP template<class O,Tuple_flag FO> TMatrix<C,sizex,sizey,TF> TMatrix<C,sizex,sizey,TF>::scale_cols(Tuple<O, sizex, FO> const &fact) const{
	TMatrix<C,sizex,sizey,TF> out;
...
LFHTEMP template<class O> auto TMatrix<C,sizex,sizey,TF>::operator*(Tuple<O,sizex> const & other)const
 -> Tuple<decltype(data.data[0] *other.data.data[0]),sizey>{
    Tuple<decltype(data.data[0] *other.data.data[0]),sizey> _out;
    unsigned int i,j;
    for(j=0;j<sizey;j++){
...
LFHTEMP template<class O> auto TMatrix<C,sizex,sizey,TF>::mkBackMult(Tuple<O,sizey> const & other)const
 -> Tuple<decltype(data.data[0] *other.data.data[0]),sizex>{
    Tuple<decltype(data.data[0] *other.data.data[0]),sizex> _out;
    unsigned int i,j;
    for(j=0;j<sizex;j++){
...
LFHTEMP TMatrix<C,0u,0u,TF>::TMatrix(TMatrix<C,0u,0u,TF>&& other): data(other.data){
    sizes[0] = other.sizes[0];
    sizes[1] = other.sizes[1];
    ExOp::toZero(other.sizes);
}
LFHTEMP TMatrix<C,0u,0u,TF>::TMatrix(RemoteMemory<const C, 2u> source){
	sizes[0] = source.dims[0];
    sizes[1] = source.dims[1];
    data.setSize(sizes[0]*sizes[1]);
    C* cur = data();
    if (auto ite = source.mkIterator()) do{*(cur++) = *ite;}while(ite++);
}
...
LFHTEMP TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::operator=(TMatrix<C,0u,0u,TF> const & other){
    ...
    if (i == 0) return *this;
    if (ExCo<C>::IsPOD::value) memcpy(data(),other.data(), sizeof(C) * i);
    else{
        for(i--;i != ExCo<uint32_t>::mkMaximum() ; i--) data[i] = other.data[i];
    }
return *this;}
LFHTEMP TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::operator=(TMatrix<C,0u,0u,TF>&& other){
    data = other.data;
    sizes[0] = other.sizes[0];
    sizes[1] = other.sizes[1];
    ExOp::toZero(other.sizes);
...
LFHTEMP template<class O, Tuple_flag  OTF> TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::operator=(TMatrix<O, 0u, 0u,OTF> const & other){
    ...
    for(i--;i != ExCo<uint32_t>::mkMaximum() ; i--) data[i] = other.data[i];
return *this;}
LFHTEMP template<class ARR, typename RequireArray<ARR>::type> TMatrix<C,0u,0u,TF>::TMatrix(ARR arr){
	Tuple<uint32_t, 2u> dim = arr.getShape();
	this->setSizes(dim[0], dim[1]);
	if (auto ite = this->getIterator()) do{*ite = arr[ite()()];}while(ite++);
}
LFHTEMP template<class ARR, typename RequireArray<ARR>::type> TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::operator=(ARR arr){
	Tuple<uint32_t, 2u> dim = arr.getShape();
	this->setSizes(dim[0], dim[1]);
	if (auto ite = this->getIterator()) do{*ite = arr[ite()()];}while(ite++);
return *this;}
...
LFHTEMP template<class O> TMatrix<decltype(declval<C>() * declval<O>()),0u,0u,TF> TMatrix<C,0u,0u,TF>::mkLeftMult(RemoteMemory<const O,2u> & other)const{
    TMatrix<C,0u,0u,TF> fout; fout.setSizes(other.dims[0],sizes[1]);
    uint32_t maxdim = other.dims[1] > sizes[0] ? sizes[0] : other.dims[1];
    if (other.dims[1] != sizes[0]) printf("warning, dimension mismatch in left matrix multiplication %ix%i * %ix%i\n", other.dims[0], other.dims[1], sizes[0], sizes[1]);
    Tuple<uint32_t, 2u> coor;
    for(uint32_t k=0;k<sizes[1];k++){
        for(coor[0]=0;coor[0]<other.dims[0];coor[0]++){
			coor[1] = 0;
            fout.data[coor[0] + k * other.dims[0]] = data[k * sizes[0]] * other[coor];
            for(coor[1]++;coor[1]<maxdim;coor[1]++){
                fout.data[coor[0] + k * other.dims[0]] += data[coor[1] + k * sizes[0]] * other[coor];
            }
        }
    }
return(fout);}
LFHTEMP template<class O> TMatrix<decltype(declval<C>() * declval<O>()),0u,0u,TF> TMatrix<C,0u,0u,TF>::operator*(RemoteMemory<const O,2u> & other)const{
    TMatrix<C,0u,0u,TF> fout; fout.setSizes(sizes[0],other.dims[1]);
    uint32_t maxdim = sizes[1] > other.dims[0] ? other.dims[0]  : sizes[1];
    if (sizes[1] != other.dims[0]) printf("warning, dimension mismatch in matrix multiplication %ix%i * %ix%i\n", sizes[0], sizes[1], other.dims[0], other.dims[1]);
    Tuple<uint32_t, 2u> coor;
    for(uint32_t k=0;k<sizes[0];k++){
        for(coor[1]=0;coor[1]<other.dims[1];coor[1]++){
			coor[0] = 0;
            fout.data[k + coor[1] * sizes[0]] = data[k] * other[coor];
            for(coor[0]++;coor[0]<maxdim;coor[0]++){
                fout.data[k + coor[1] * sizes[0]] += data[k + coor[0] * sizes[0]] * other[coor];
            }
        }
    }
return(fout);}
LFHTEMP template<class O,Tuple_flag OTF> TMatrix<C,0u,0u,TUPLE_FLAG_CONSTRUCT_MASK(TF)> TMatrix<C,0u,0u,TF>::operator*(TMatrix<O, 0u, 0u,OTF> const & other)const{
    TMatrix<C,0u,0u,TF> fout; fout.setSizes(sizes[0],other.sizes[1]); // did this change...?
    uint32_t maxdim = sizes[1] > other.sizes[0] ? other.sizes[0]  : sizes[1];
    if (sizes[1] != other.sizes[0]) printf("warning, dimension mismatch in matrix multiplication %ix%i * %ix%i\n", sizes[0], sizes[1], other.sizes[0], other.sizes[1]);
    unsigned int i,j,k;
    for(k=0;k<sizes[0];k++){
        for(j=0;j<other.sizes[1];j++){
            fout.data[k + j * sizes[0]] = data[k] * other.data[j * other.sizes[0]];
            for(i=1;i<maxdim;i++){
                fout.data[k + j * sizes[0]] += data[k + i * sizes[0]] * other.data[i + j * other.sizes[0]];
            }
        }
    }
return(fout);}
...
LFHTEMP template<class O, Tuple_flag OFLAG> auto TMatrix<C,0u,0u,TF>::operator*(const Tuple<O, 0u, OFLAG> & a)const
 -> Tuple<decltype(data.data[0] * a[0] ),0u,TUPLE_FLAG_NULL>{
    Tuple<decltype(data.data[0] * a[0] ),0u,TUPLE_FLAG_NULL> fout;
...
return fout;}
LFHTEMP Tuple<C> TMatrix<C,0u,0u,TF>::sumRows() const{ Tuple<C> fout; fout.setSize(sizes[1]);
    for(uint32_t i= 0 ; i< sizes[1];i++){
		fout[i] = data[i*sizes[0]];
        for(uint32_t j= 1 ; j< sizes[0];j++) fout[i] += data[j+i*sizes[0]];
	}
return fout;}
LFHTEMP Tuple<C> TMatrix<C,0u,0u,TF>::sumCols() const{ Tuple<C> fout; fout.setSize(sizes[0]);
    for(uint32_t i= 0 ; i< sizes[0];i++){
		fout[i] = data[i];
        for(uint32_t j= 1 ; j< sizes[1];j++) fout[i] += data[i+j*sizes[0]];
	}
return fout;}
...
return(fout);}
LFHTEMP TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::toTranspose(){
    Tuple<C, 0u, TF> ndata; ndata.setSize(sizes[1] * sizes[0]);
    for(unsigned int i= 0 ; i< sizes[0];i++)
        for(unsigned int j= 0 ; j< sizes[1];j++) ndata[j+i*sizes[1]] = move(data[i+j*sizes[0]]);
	data.toMemmove(ndata);
	ExOp::toMemswap(sizes[1],sizes[0]);
return *this;}
...
LFHTEMP	TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::leftHouseHolderMultiply(const RemoteMemory<const C,1> vec, const double sqrt_den, uint32_t start, bool applytoall){
	uint32_t alt_start = applytoall ? 0 : start;
	Tuple<C> inner; inner.setSize(sizes[0] - alt_start);
	if ((sqrt_den == 0.0f)||(!ExCo<double>::isValid(sqrt_den))) return *this;
	double fact = -1.0f / sqrt_den;
	uint32_t i,j;
	C* shiftedata = data() + alt_start + start * sizes[0];
	for(j=0;j< inner.getSize();j++) inner[j] = shiftedata[j] * vec[0];
	for(i=1;i< vec.dims[0];i++){
		for(j=0;j<inner.getSize();j++) inner[j] += shiftedata[j + i * sizes[0]] * vec[i];
	}
	for(j=0;j<inner.getSize();j++){
		inner[j] *= fact;
		for(i=0;i< vec.dims[0];i++) shiftedata[j + i * sizes[0]] += inner[j] * vec[i];
	}
return *this;}

LFHTEMP	TMatrix<C,0u,0u,TF>& TMatrix<C,0u,0u,TF>::rightHouseHolderMultiply(const RemoteMemory<const C,1> vec, const double sqrt_den, uint32_t start, bool applytoall){
	uint32_t alt_start = applytoall ? 0 : start;
	Tuple<C> inner; inner.setSize(sizes[1]- alt_start);
	if ((sqrt_den == 0.0f)||(!ExCo<double>::isValid(sqrt_den))) return *this;
	double fact = -1.0f / sqrt_den;
	uint32_t i,j;
	C* shiftedata = data() + start + alt_start * sizes[0];
	for(j=0;j< inner.getSize();j++) inner[j] = shiftedata[j* sizes[0]] * vec[0];
	for(i=1;i< vec.dims[0];i++){
		for(j=0;j< inner.getSize();j++) inner[j] += shiftedata[ i + j * sizes[0]] * vec[i];
	}
	for(j=0;j<inner.getSize();j++){
		inner[j] *= fact;
		for(i=0;i< vec.dims[0];i++) shiftedata[i + j * sizes[0]] += inner[j] * vec[i];
	}
return *this;}

LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::rightHouseHolderMultiply_inplace(const double sqrt_den, uint32_t start){
	Tuple<C> inner; inner.setSize(sizes[1] - start - 1);
	if (inner.getSize() == 0) return *this;
	if ((sqrt_den == 0.0f)||(!ExCo<double>::isValid(sqrt_den))) return *this;
	double fact = -1.0f / sqrt_den;
	uint32_t i,j;
	C* shiftedata = data() + start * (sizes[0]+1);
	for(j=0;j< inner.getSize();j++) inner[j] = shiftedata[(j+1)* sizes[0]] * shiftedata[0];
	for(i=1;i< sizes[0] - start ;i++){
		for(j=0;j< inner.getSize();j++) inner[j] += shiftedata[ i + (j+1) * sizes[0]] * shiftedata[i];
	}
	for(j=0;j<inner.getSize();j++){
		inner[j] *= fact;
		for(i=0;i< sizes[0] - start;i++) shiftedata[i + (j+1) * sizes[0]] += inner[j] * shiftedata[i];
	}
return *this;}

LFHTEMP TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::toUpperTriangular(bool make_squarre){
	Tuple<C> hv; hv.setSize(sizes[0]);
	for(uint32_t j =0 ; j < sizes[1];j++){
		uint32_t k = j * (sizes[0]+1);
		double normbuf = 0.0; // ExOp::pnorm(data[k]);
		for(uint32_t i=1;i<sizes[0]-j;i++) {
			normbuf += ExOp::pnorm(data[k+i]);
			hv[i] = data[k+i];
		}
		hv[0] = data[k] * (1.0 - sqrt(1.0 + normbuf / ExOp::pnorm(data[k])));
		normbuf += ExOp::pnorm(hv[0]);
		this->rightHouseHolderMultiply(RemoteMemory<const C,1>(hv(), sizes[0]-j), normbuf * 0.5,j);
	}
	if (make_squarre){
		hv.setSize(sizes[1] *sizes[1]);
		for(uint32_t j=0;j< sizes[1];j++){
			for(uint32_t i=0;i< sizes[1];i++) hv[i + j*sizes[1]] = data[i + j*sizes[0]];
		}
		data.toMemmove(hv);
		sizes[0] = sizes[1];
	}
return *this;}
LFHTEMP TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::toUpperTriangulariser(RemoteMemory<const C,2> matrix){ // Find X such that X * I = upper triangular
	this->setSizes(matrix.dims[matrix.dims[0] > matrix.dims[1] ? 1 : 0], matrix.dims[0]).toOne();
	Tuple<double> normbuf; normbuf.setSize(matrix.dims[1]);
	TMatrix<C, 0u, 0u,TF> buffer = matrix;
	for(uint32_t j =0 ; j < sizes[0];j++){
		uint32_t k = j * (buffer.sizes[0]+1);
		normbuf[j] = 0.0; // ExOp::pnorm(data[k]);
		for(uint32_t i=1;i<buffer.sizes[0]-j;i++) {
			normbuf[j] += ExOp::pnorm(buffer.data[k+i]);
		}
		buffer.data[k] *= (1.0 - sqrt(1.0 + normbuf[j] / ExOp::pnorm(buffer.data[k])));
		normbuf[j] += ExOp::pnorm(buffer.data[k]);
		normbuf[j] *= 0.5;
		buffer.rightHouseHolderMultiply_inplace(normbuf[j],j);
	}
	for(uint32_t j = sizes[0] ; j > 0;){
		j--;
		uint32_t k = j * (buffer.sizes[0]+1);
		this->leftHouseHolderMultiply(RemoteMemory<const C,1>(buffer.data() + k, buffer.sizes[0]-j), normbuf[j],j);
	}
return *this;}


LFHTEMP TMatrix<C, 0u, 0u,TF>& TMatrix<C, 0u, 0u,TF>::toSingularDecomposition(RemoteMemory<const C,2> matrix, Tuple<C> &eigen, TMatrix<C> &orthogonal){
//	this->setSize(matrix.dims[0],matrix.dims[1]);
//	TMatrix<C, 0u, 0u,TF> buffer = matrix;
//	Tuple<double> normbuf;
//	int j;
	Trianglix<C> trib;
	if (matrix.dims[0] > matrix.dims[1]){
		orthogonal = trib.toInnerProduct(matrix).getEigenVectors(eigen);
		this->toUpperTriangulariser(orthogonal.mkLeftMult(matrix));
	}else{ // not working!!!
		orthogonal = trib.toOuterProduct(matrix).getEigenVectors(eigen);
		this->toUpperTriangulariser(orthogonal.mkTranspose() * matrix);
	}
	for(int i=0;i<eigen.getSize();i++) eigen[i] = sqrt(eigen[i]); // TOFIX
return *this;}
LFHTEMP Tuple<C> TMatrix<C, 0u, 0u,TF>::mkSingualDecomposition(TMatrix<C> &U, TMatrix<C> &Vt)const{ Tuple<C> fout;


return fout;};

LFHTEMP	Trianglix<C, 0u> TMatrix<C,0u,0u,TF>::mkInnerProduct() const{ Trianglix<C, 0u> fout; fout.setSize(sizes[1]);
    uint32_t i,j,k,l;
    for(k=0,i=0;i<fout.getSize();i++,k++){
    	for(j=0;j<i;j++,k++){
			fout.data[k] = data[i* sizes[0]] * data[j* sizes[0]];
			for(l=1;l<sizes[0];l++) fout.data[k] +=  data[i* sizes[0]+ l] * data[j* sizes[0]+ l ];
    	}
		fout.data[k] = ExOp::pnorm(data[i* sizes[0]]);
		for(l=1;l<sizes[0];l++) fout.data[k] +=  ExOp::pnorm(data[i* sizes[0]+ l]);
    }
return fout;}
LFHTEMP	Trianglix<C, 0u> TMatrix<C,0u,0u,TF>::mkOuterProduct() const{ Trianglix<C, 0u> fout; fout.setSize(sizes[0]);
    unsigned int i,j,k,l;
    for(k=0,i=0;i<fout.getSize();i++,k++){
    	for(j=0;j<i;j++,k++){
			fout.data[k] = data[i] * data[j];
			for(l=1;l<sizes[1];l++) fout.data[k] +=  data[i+ l * sizes[0]] * data[j+ l * sizes[0]];
    	}
		fout.data[k] = ExOp::pnorm(data[i]);
		for(l=1;l<sizes[1];l++) fout.data[k] +=  ExOp::pnorm(data[i+ l * sizes[0]]);
    }
return fout;}

LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::rotColPair(uint32_t i, double cosval, double sinval){
	C tmp;
	uint32_t j;
	for(j=0;j<sizes[0];j++){
		tmp = data[ j + i* sizes[0]] * cosval + data[ j + (i+1)* sizes[0]] * sinval;
		data[ j + (i+1)* sizes[0]] *= cosval;
		data[ j + (i+1)* sizes[0]] -= data[ j + i * sizes[0]] * sinval;
		data[ j + i* sizes[0]] = tmp;
	}
return *this;}
LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::negateColumn(uint32_t i){for(uint32_t j=0;j<sizes[0];j++) data[ j + i* sizes[0]] = -data[ j + i* sizes[0]];return *this;}
LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::rotRowPair(uint32_t i, double cosval, double sinval){
	C tmp;
	uint32_t j;
	for(j=0;j<sizes[1];j++){
		tmp = data[ i + j* sizes[0]] * cosval + data[ i+1 + j* sizes[0]] * sinval;
		data[ i+1 + j* sizes[0]] *= cosval;
		data[ i+1 + j* sizes[0]] -= data[ i + j * sizes[0]] * sinval;
		data[ i + j* sizes[0]] = tmp;
	}
return *this;}
LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::negateRow(uint32_t i){for(uint32_t j=0;j<sizes[1];j++) data[ i + j* sizes[0]] = -data[ i + j* sizes[0]];return *this;}

LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::toFlipColumns(){
	uint32_t j,k;
	for(j=0;j<(sizes[1] >> 1);j++){
		k = (sizes[1] -1 - j) * sizes[0];
		for(uint32_t i = 0 ; i< sizes[0];i++) ExOp::toMemswap(data[ i + j* sizes[0]], data[ i + k]);
	}
return *this;}
LFHTEMP	TMatrix<C, 0u, 0u,TF>& TMatrix<C,0u,0u,TF>::toFlipRows(){
	C tmp;
	uint32_t j,k;
	for(j=0;j<(sizes[0] >> 1);j++){
		k = (sizes[0] -1 - j);
		for(uint32_t i = 0 ; i< sizes[1];i++) ExOp::toMemswap(data[ j + i* sizes[0]], data[ k + i* sizes[0]]);
	}
return *this;}
...
LFHTEMP template<class O> auto TMatrix<C,0u,0u,TF>::mkInnerMult(const Tuple<O> & left, const Tuple<O> & right)const
 -> decltype((this->data[0] * right[0]) * left[0]){
    Tuple<decltype((this->data[0] * right[0]) * left[0])> tmpbuf; tmpbuf.setSize(left.getSize());
	C* cur = data;
	for(int j=0;j<sizes[0];j++) tmpbuf[j] = *(cur++) * right[0];
	for(int i=1;i<sizes[1];i++){
        for(int j=0;j<sizes[0];j++) tmpbuf[j] += *(cur++) * right[i];
    }
return tmpbuf.mkInnerProd(left);}
...
LFHTEMP ERRCODE TMatrix<C,0u,0u,TF>::load(FILE *f){
    ...
    if (totsize != 0) {
        if (ExCo<C>::IsPOD::value) {
            ...
            for(uint32_t i =0; i< totsize;i++) if ((fout |= ExOp::load(data[i],f))) break;
        }
    }
return fout;}
LFHTEMP ERRCODE TMatrix<C,0u,0u,TF>::save(FILE *f) const{
    ...
    if ((sizes[0] != 0)&&(sizes[1] != 0)) {
        ...
        if (ExCo<C>::IsPOD::value) {
            ...
            for(uint32_t i =0; i< totsize;i++) if ((fout |= ExOp::save(data[i],f))) break;
        }
    }
return fout;}
...
LFHTEMP template<class O> bool SparseTuple<C>::operator==(const SparseTuple<O>& other)const{
    ...
    if (auto ite = this->mkIterator()) do{
...
LFHTEMP SparseMatrix<C>& SparseMatrix<C>::purgeValue(C value){
	for(int i=0;i<data.getSize();i++){
		for(int j=0;j<data[i].getSize();j++){
			if (data[i].deref(j) == value){
				data[i].erase_from_iterator(j--);
			}
		}
	}
return *this;}

LFHTEMP Tuple<C> SparseMatrix<C>::sumRows() const{ Tuple<C> fout; fout.setSize(this->getNBcols());
	for(int i=0; i<data.getSize();i++){
		if (auto ite = data[i]()){
			fout[i] = *ite;
			while(ite++) fout[i] += *ite;
		}else ExOp::toZero(fout[i]);
	}
return(fout);}
LFHTEMP Tuple<C> SparseMatrix<C>::sumCols() const{ Tuple<C> fout;
	Vector<C> tmpfout;
    for(int i=0;i< data.getSize();i++){
        if (auto ite = data[i]()) do{
            if (ite() >= tmpfout.getSize()) {
				int j = tmpfout.getSize();
				tmpfout.upSize(ite()+1);
				while(j<tmpfout.getSize()) tmpfout[j++] = 0;
			}
            tmpfout[ite()] += *ite;
        }while(ite++);
    }
...
LFHTEMP Tuple<double> SparseMatrix<C>::UTestZScore(const Tuple<uint32_t> &listX, const Tuple<uint32_t> &listY, Tuple<double> *opt_logit_auroc, Tuple<double> * opt_logit_seen_auroc, bool print_prog, bool return_logitpval, bool hypergeo_correction)const{ // compute rank test for each column, for specific rows only statistics
    ...
    for(uint32_t i=0;i<this->getNBcols();i++){
        ...
        if ((hypergeo_correction)&&((cnts.getSize() < 2)||(zeros[0] == listX.getSize())||(zeros[1] == listY.getSize()))) {
            // all non-zero counts have the same value! => variance is 0
            // 1 or both sample are all 0 => variance is 0
            if (cnts.getSize() == 0) {
                fout[i] = 0.0;
...
LFHTEMP TMatrix<Zscore>  SparseMatrix<C>::UTestZScoreSplit(const Tuple< Tuple<uint32_t> > &lists, const Tuple<uint32_t> &ordering, uint32_t nbthreads, uint32_t splitgroups, const Tuple<uint32_t> &partit, TMatrix<double> *opt_logit_auroc,TMatrix<double> *opt_mean, TMatrix<double> *opt_drop_enrich,  bool print_prog, bool hypergeo_correction, bool downsample)const{
	// compute rank test for each column for specific rows only statistics , Zscore and optionnaly logit Auroc reported
   ...
   /* double val;
    ...
    val2 = val =1.0; printf("%e -> %c, %e -> %c  %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
    val2 = val =0.0; printf("%e -> %c, %e -> %c %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
    val2 = val =1.0 / 0.0; printf("%e -> %c, %e -> %c %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
    val2 = val =-1.0/ 0.0; printf("%e -> %c, %e -> %c %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
    val2 = val =0.0 / 0.0; printf("%e -> %c, %e -> %c %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
    val2 = val =nan("NAN"); printf("%e -> %c, %e -> %c %i %i\n", val, isfinite(val) ? 'V' : 'I', val2, isfinite(val2) ? 'V' : 'I', fpclassify(val), fpclassify(val2));
*/
  ...
  //  if (return_logitpval) tb.flags["return_logitpval"] = true;
	...
	for(coor[1] =0;coor[1] < splitgroups * dapart.getSize();coor[1]++) {
		for(coor[0]=0;coor[0]<lists.getSize();coor[0]++){
			printf("%i\t", splitlists(coor).getSize());
		}
		printf("\n");
	}
...
//        printf("Downsample probs:\n"); ExOp::show(dasampling);
	...
	printf("daout length is %i\n", daout.getSize());
	printf("daout first is %i, %i\n", daout[0].sizes[0], daout[0].sizes[1]);
...
public:
                ...
                if (ind == 0){ // row only has zeros...
                   ...
                   // if (hypergeo_correction){
                        ...
                        for(coor3[1] = 0, coor3[0] = coor3[2] * list_grid.sizes[0]; coor3[1] < list_grid.sizes[0]; coor3[1]++,coor3[0]++){
                            ...
                            //
                            if ((cnts[coor3[2]].getSize() < 2)||( zeros[coor3[0]] == sizes[coor3[0]] )||(row_zeros - zeros[coor3[0]] == row_sizes[coor3[2]]-sizes[coor3[0]])) {
                                //tb.printf("%i [%i,%i] got %i %i %i %i %i\n",threadID, coor3[1], coor3[2], cnts[coor3[2]].getSize(), row_zeros, sizes[coor3[0]], sizes[coor3[0]] - row_zeros, row_sizes[coor3[2]] -sizes[coor3[0]]);
                                fout[i].data[coor3[0]] = (cnts[coor3[2]].getSize() == 0) ? 0.0 : -HypergeomericLogitPval( zeros[coor3[0]] , row_zeros, sizes[coor3[0]], row_sizes[coor3[2]], 0.5);
                            }else{
                            //
                        ...
                        //        printf("%i and %i\n", zerosum[coor3[1]], sum[coor3[1]] );
                                ...
                                if (!ExOp::isValid(fout[i].data[coor3[0]])) {
                                    tb.printf("NAN generated by %e+-%e %i %i %i \n", mu,  erffact,row_zeros,sizes[coor3[0]], row_sizes[coor3[2]]);
                                    cnts[coor3[2]].show();
                                    tb.printf("cat %i (neg class) %i %i\n", cnts[coor3[2]].getSize(), sizes[coor3[0]] - row_zeros, row_sizes[coor3[2]]-sizes[coor3[0]]);
    ...
    printf("is using hypergeo correction? %c\n", tb.flags.find("hypergeo_correction") != 0xFFFFFFFF);
...
LFHTEMP class SparseMatrix<C>::UTestPaired_Task{
    const SparseMatrix<C> &data;
public:
    ThreadBase::ThreadArrayScope thrray;
    TMatrix<double> fout;
    Tuple<Vector<KeyElem<C, uint32_t > > > sorted_vals;
    Tuple<uint32_t> zeroranks;
    Tuple< Tuple<double, 3> > ties_within;  // sum ti(ti-1) ; sum ti(ti-1)(ti-2) ; sum ti(ti-1)(2*ti+5)
    bool cz;
    uint32_t task;
    uint32_t nbrows;
    UTestPaired_Task(const SparseMatrix<C> &_target, bool _cz): data(_target), thrray(foreach.mkThreadArray()), cz(_cz) {}
    uint32_t operator()(uint32_t threadID){
        uint32_t jobID;
        uint32_t lastID =0;
        uint32_t tmpint =0;
        ...
        switch(task){
        case 0:{
            while(thrray.getRangeVal(jobID)){
                lastID =0;
                zeroranks[jobID] =0;
                sorted_vals[jobID].setSize(data.data[jobID].getSize());
                if (auto ite = data.data[jobID].mkIterator()) do{
                    sorted_vals[jobID][ite.getOffset()].k = *ite;
                    sorted_vals[jobID][ite.getOffset()].d = ite();
                    if (ExOp::isNegative(*ite)) zeroranks[jobID]++;
                }while(ite++);
                sorted_vals[jobID].sort();
                lastID = nbrows - data.data[jobID].getSize();
                ties_within[jobID][0] = (double)(lastID) * (lastID-1);
                ties_within[jobID][1] = ties_within[jobID][0] * (lastID-2);
                ties_within[jobID][2] = ties_within[jobID][0] * ((lastID<<1) + 5);
                tmpint=0;
                for(lastID=1;lastID< sorted_vals[jobID].getSize();lastID++) {
                    if (sorted_vals[jobID][lastID].k == sorted_vals[jobID][lastID-1].k) tmpint++;
                    else if (tmpint != 0) {
                        tmpdouble = (double)(tmpint+1) * (tmpint);
                        ties_within[jobID][0] += tmpdouble;
                        ties_within[jobID][1] += tmpdouble * (tmpint -1);
                        ties_within[jobID][2] += tmpdouble * ((tmpint <<1)+7);
                        tmpint =0;
                        //sorted_vals[jobID][lastID-1 - tmpint].k = 1.0 + tmpint;
                    }//else sorted_vals[jobID][lastID-1].k = 1.0;
                }
                tmpdouble = (double)(tmpint+1) * (tmpint);
                ties_within[jobID][0] += tmpdouble;
                ties_within[jobID][1] += tmpdouble * (tmpint -1);
            ...
            uint32_t colA =0;
            uint32_t colB =1;
            uint32_t nbA,nbB, iteA,iteB;
            uint32_t wrtar, eqtar;
            Vector<C> dabuf;
            int32_t indiff;
            uint32_t dasel;
            uint32_t signsum[4];
            while(thrray.getRangeVal(jobID)){
                colA += jobID - lastID; lastID = jobID;
                while(colA >= colB) {colA -= colB; colB++;}
                if (sorted_vals[colA].getSize() > sorted_vals[colB].getSize()){
                    ExOp::toMemswap(colA, colB);
                }

                // find missing entries in respective lists and filling
                dabuf.setSize(sorted_vals[colA].getSize());
                tmpint = zeroranks[colA];
                if (auto ite = data.data[colB].mkIterator()) do{
                    if ( data.data[colA].find(ite()) == 0xFFFFFFFF) {dabuf.push_back(); dabuf[tmpint++] = *ite;}
                }while(ite++);
                // nbrows - dabuf.getSize() -> number of zero pairs




                indiff = (nbrows * (nbrows- 1) + (nbrows - dabuf.getSize()) * (nbrows - dabuf.getSize() -1) - ((uint32_t)(ties_within[colA][0] + ties_within[colB][0]))) >> 1; // maximum concordant
                if (!cz)  {
                    printf("hehe! %i\n", 0xFFFFFFFF >> 1);
                    printf("%i, %i, %i -> %i \n", nbrows, dabuf.getSize(), nbrows - dabuf.getSize(), indiff);
                    printf("%i and %i\n", nbrows * (nbrows- 1),  (nbrows - dabuf.getSize()) * (nbrows - dabuf.getSize() -1));
                indiff = (nbrows - dabuf.getSize()) * (nbrows -1) + ((((int32_t)dabuf.getSize()) * (dabuf.getSize()- 1) - ((int32_t)(ties_within[colA][0] + ties_within[colB][0]))) >> 1); // maximum concordant
                    printf("%i and %i  -> %i\n", 2*(nbrows - dabuf.getSize()) * (nbrows -1),  dabuf.getSize() * (dabuf.getSize()- 1), indiff);

                }


                //printf("da zeros are: %i %i,  and sums %i %i\n", nbrows - sorted_vals[colA].getSize(), nbrows - sorted_vals[colB].getSize(), ((uint32_t)(ties_within[colA][0])) >> 1, ((uint32_t)(ties_within[colB][0])) >>1);
                //printf("\n\nconcordant... %i - %i - %i = %i\n" , (dabuf.getSize() * (dabuf.getSize()- 1))>>1, ((uint32_t)(ties_within[colA][0]))>>1 ,  ((uint32_t)(ties_within[colB][0]))>>1 , indiff);
                indiff += dabuf.sortWithinRangeAndSumPerm(zeroranks[colA], tmpint);

                ExOp::toZero(signsum);

                if (auto ite = data.data[colA].mkIterator()) {
                        dasel = (zeroranks[colA] == 0) ? 2 : 0;
                    eqtar = wrtar = (zeroranks[colA] == 0) ?  dabuf.getSize() - data.data[colA].getSize() : 0;
                    tmpint = 0;
                    if (ExOp::isZero(data.data[colB][sorted_vals[colA][tmpint].d])) signsum[dasel | 1]++;
                    else if (ExOp::isNegative(data.data[colB][sorted_vals[colA][tmpint].d])) signsum[dasel]++;
                    dabuf[wrtar++] = data.data[colB][sorted_vals[colA][tmpint++].d];
                    while(ite++){
                        if (sorted_vals[colA][tmpint].k != sorted_vals[colA][tmpint-1].k){
                            if ((wrtar - eqtar) > 1) indiff += dabuf.sortWithinRangeAndSumPerm(eqtar, wrtar);
                            if (ite.getOffset() == zeroranks[colA]) {wrtar += dabuf.getSize() - data.data[colA].getSize(); dasel =2; }
                            eqtar = wrtar;
                        }
                        if (ExOp::isZero(data.data[colB][sorted_vals[colA][tmpint].d])) signsum[dasel | 1]++;
                        else if (ExOp::isNegative(data.data[colB][sorted_vals[colA][tmpint].d])) signsum[dasel]++;
                ...
                dabuf.sortAndCountPermutation(tmpint); // number of discordant

                signsum[2] = signsum[2] + zeroranks[colA] - signsum[1] - signsum[0];

                // n *(n-1) /2 = discordant + concordent + ("equal", aka 1 or 2 are equal)
                // n *(n-1) /2 = discordant + concordent + equal in A + equal in B - equal in both A and B!
                //

                tmpint += (nbrows - dabuf.getSize()) * signsum[2];
                indiff -= tmpint;
                //printf("number of discordant: %i, concordant: %i\n", tmpint, indiff);
                signsum[0] = 0;
                signsum[1] = 0;

                for(int i =0;i < nbrows-1;i++){
                    for(int j =i+1;j < nbrows;j++){
                        if ((data.data[colA][j] == data.data[colA][i])||(data.data[colB][j] == data.data[colB][i])) continue;
                        if ((data.data[colA][j] > data.data[colA][i])^(data.data[colB][j] > data.data[colB][i])) signsum[1]++;
                        else signsum[0]++;
                    }
                }
                if ((signsum[1] != tmpint)||(signsum[0] != indiff)) {printf("GOT ERRROR!\n"); LFH_exit(1);}

                double n = nbrows; // dabuf.getSize();
                double numerator = ((double)indiff) - tmpint; // number of concordant - number of discordant (temp! is not accounting for ties!!)

                double variance = ((n * (n -1.0)* (n * 2.0 + 5.0) - ties_within[colA][2] - ties_within[colB][2])/18.0)
                                     +(ties_within[colA][0] * ties_within[colB][0] / (2.0*n*(n-1.0)))
                                     +(ties_within[colA][1] * ties_within[colB][1] / (9.0*n*(n-1.0)*(n-2.0)));

                if (colA >= colB) ExOp::toMemswap(colA, colB);
                fout(colA,colB) = numerator / sqrt(variance);
            }
        }break;}
...
/** \brief Compare all columns pairs from sparse matrix and test the significance of the agreement for ordering of value in each row
 *
 * rank of entries within columns are compared, with "0" being the only entry allowed to tie
 * the tau statistic counts the number of pair of pairs whose rank are concordant and discordant
 * \sum(i<j) (rank(x_i) > rank(y_i))^(rank(x_j) > rank(y_j)) - \sum(i<j) !(rank(x_i) > rank(y_i))^(rank(x_j) > rank(y_j))
 ...
 * (sqtr / )
 *
 * \return Sub-triangular matrix for Zscores of statistical significance
 *
 */
LFHTEMP TMatrix<double> SparseMatrix<C>::UTestPaired(bool cz) const{
    SparseMatrix<C>::UTestPaired_Task lotask(*this, cz);
    uint32_t nbpairs = this->getNBcols();
    lotask.nbrows = this->computeNBrows();
    printf("Performing Kendall test on all %i column pairs with length %i (using %i threads)\n", this->getNBcols(), lotask.nbrows, lotask.thrray.nbthreads);
    lotask.fout.setSizes(nbpairs,nbpairs).toZero();
    lotask.sorted_vals.setSize(nbpairs);
    lotask.zeroranks.setSize(nbpairs);
    lotask.ties_within.setSize(nbpairs);
    lotask.thrray.initRange(nbpairs, "Computing rank orderings");
    lotask.task=0; lotask.thrray.submit(lotask);
    nbpairs = ((nbpairs -1) * nbpairs)>>1;
    lotask.thrray.initRange(nbpairs, "Comparing column orderings");
    lotask.task=1; lotask.thrray.submit(lotask);

return lotask.fout;}// do a wilcoxon test for the *difference* of all paired row

LFHTEMP TMatrix<double> SparseMatrix<C>::computeTFIDF(const Tuple< Tuple<uint32_t> > &lists) const{
	class Task : public Event<uint32_t>{
	public:
		const SparseMatrix<C> &data;
		TMatrix<double> fout;
		myHashmap<uint32_t, uint32_t> index_to_cluster;
		Task(const SparseMatrix<C>& _data, const Tuple< Tuple<uint32_t> > &lists):data(_data){
			fout.setSizes(data.data.getSize(), lists.getSize());
			for(int i=0; i < lists.getSize();i++)
				for(int j=0; j < lists[i].getSize();j++) {
					index_to_cluster[lists[i][j]] = i;
				}
			printf("populated hash!\n");
		}
		uint32_t operator()(uint32_t threadID){
			Tuple<uint32_t> curfreq;
			int ite;
			curfreq.setSize(fout.sizes[1]);
			foreach.printf("thr%i\n", threadID);
			if (auto rite = foreach.getRangeIterator(threadID, data.data.getSize(),true)) do{
				curfreq.toZero();
				if (auto cite = data.data[rite()]()) do{
					if ((ite = index_to_cluster.find(cite())) != 0xFFFFFFFF) curfreq[index_to_cluster.deref(ite)]++;
				}while(cite++);

			}while(rite++);
		return 0;}
	};
	Task datast(*this,lists);
    foreach.startThreadArray();
    uint32_t thite;
    foreach.printf("%i genes and %i threads\n", data.getSize(), foreach.getThreadArraySize());
    foreach.startProgress("Computing Tf-Idf", data.getSize());
    for(thite = foreach.getThreadArraySize()-1; thite> 0;thite--) foreach.submit(datast, thite);
    foreach.submit_ThenWait(datast, thite);
    foreach.printf("done!\n");
    foreach.stopThreadArray();
return datast.fout;}
// sh xclipitout.sh ; sh repackage6.sh ; export R_LIBS=~/R/x86_64-pc-linux-gnu-library/3.6/ ; R CMD INSTALL InferN0 ; cd /lustre/scratch117/cellgen/team292/lh20/revision/ ; R --vanilla ; cd
// library(InferN0) ; scp <- InferN0LoadFile("integrated.infr.scp"); meta <- readRDS("meta.rds") ; out <- InferN0FindMarkers(scp, 4,  meta$subcluster_epithelial_balanced@.Data - 1, order(meta$log2p1_count), cell.use = (meta$subcluster_epithelial_balanced@.Data <= 10))
...
LFHTEMP template<class D> auto SparseMatrix<C>::operator*(SparseTuple<D> const & other) const
 -> SparseTuple<decltype( data.darray[0].heap[0].k.d * other.heap.k.d[0u] )>{
    SparseTuple<decltype( data.darray[0].heap[0].k.d * other.heap.k.d[0u] )> fout;
return fout;}
...
//        if (toomuch++ > 17000) {fclose(f); return 1;}
    ...
    if (is_floatingpoint){
        ...
        while(!feof(f)){
            ...
            c=0;
            while((sep[0] != '\n')&&(sep[0] != '\r')){
                if ( fscanf(f,argbuffer2, &dval, &sep) != 2) return 1;
        ...
        while(!feof(f)){
            ...
            c=0;
            while((sep[0] != '\n')&&(sep[0] != '\r')){
                if (fscanf(f,argbuffer2, &ival, sep) != 2) return 1;
...
LFHTEMP ERRCODE SparseMatrix<C>::readMTXTable(const char* path_prefix, Vector<string> &rownames, Vector<string> &colnames, string &tname, bool is_floatingpoint, bool is_min_addr_one){
    ...
    while(1 == fscanf(f," %1[%]", buffer)) {
        if (tname.length() == 0) { if(fscanf(f,"%[^\n]\n",buffer) != 1) return 1; tname = string(buffer);} // first comment line detected
        else if (fscanf(f,"%*[^\n]\n") != 0 ) return 1; // comment line detected
    }
    uint32_t buf[3];
    double dbuf;
...
LFHTEMP ERRCODE SparseMatrix<C>::load(FILE* f){return data.load(f);}
LFHTEMP const SparseMatrix<C>& SparseMatrix<C>::show(FILE* f, int level) const{
    Tuple<uint32_t, 2u> coor;
    uint32_t ite;
	...
	unsigned int nb_rows = this->computeNBrows();
	fprintf(f, "Sparse matrix with size %i,%i", nb_rows, data.getSize());
	if (level == 0){
		fprintf(f, "\n");
		for(coor[1] = 0; coor[1] < nb_rows; coor[1]++){
			coor[0] = 0;
			while(true){
				ite = data[coor[0]].find(coor[1]);
				ExOp::show((ite == 0xFFFFFFFF) ? zero : data[coor[0]].deref(ite),f,2);
...
LFHTEMP template<class O> void Trianglix<C, 0u>::HouseHolderMultiply(const O * const vec, double denum2, unsigned int length, C* buf, bool hint ){
    // house holder multiplication, done twice!
    // = T - vec * vec' T - T' vec * vec' + vec * vec' * T *vec * vec'
    unsigned int i,j,k;
    C s;
    ...
    for(j=0,k=0;j<length;j++) {
        for(i=0;i<j;i++) data[k++] += s * vec[j] * ExOp::mkTrju(vec[i]) - buf[j] * ExOp::mkTrju(vec[i]) - ExOp::mkTrju(buf[i]) * vec[j] ;
        data[k++] += s * vec[j] *ExOp::mkTrju(vec[j]) - buf[j]*ExOp::mkTrju(vec[j]) - ExOp::mkTrju(buf[j]) * vec[j];
    }
    if (hint) return;
    for(;j<t_size;j++) {
        for(i=0;i<length;i++) data[k++] -= buf[j]*ExOp::mkTrju(vec[i]);
...
LFHTEMP template<class O> void Trianglix<C, 0u>::offdiagelimination_down(const O &fact,unsigned int col, C * buf){
	    // = T - vec * vec' T - T' vec * vec' + vec * vec' * T *vec * vec'
    unsigned int i,k;
    k = (col *(col+1))/2;
    for(i=0;i<=col;i++) {buf[i] = data[k+i] * fact;}
    for(;i<t_size;i++) {k += i; buf[i] = data[k+col] * fact;}
    k = ((col+2) *(col+1))/2;
    for(i=0;i<=col;i++) data[k+i] += buf[i];
    k++;
    data[k+col] += buf[i] + ExOp::mkTrju(buf[i]) + data[k-2] * (fact * ExOp::mkTrju(fact));
    for(i++;i<t_size;i++) {k += i; data[k+col] += buf[i];}
}
...
return fout;}
LFHTEMP template<unsigned int SIZE> Tuple<C,SIZE> Trianglix<C, 0u>::leftDivision( const Tuple<C, SIZE>& other) const{ Tuple<C,SIZE> fout;
    // TO DO ! partial inversion!
    return(fout);
}
LFHTEMP Tuple<C,0u> Trianglix<C, 0u>::leftDivision( const Tuple<C,0u>& dev) const{ Tuple<C,0u> fout; fout.setSize(dev.getSize());

    // dev (P+P2/2)-1 dev
    Trianglix<C, 0u> invsc = (*this);
    Tuple<C, 0u> xdev = dev;
	if (getSize() < xdev.getSize()) LFH_exit(1);
	...
	//	printf("%i\t%t\n", invsc.getSize(), xdev.getSize()); fflush(stdout);
		for(j=siz-1;j>1;j--){
            ...
            hv[k+i] *= 1.0 - sqrt(1.0 + normbuf[j-2] / ExOp::pnorm(hv[k+i]));
...
LFHTEMP template<unsigned int SIZE,Tuple_flag TF> C Trianglix<C, 0u>::Xformed_inner_product( const Tuple<C, SIZE,TF>& other) const{
    ...
    C s2;
    unsigned int i,j,k;
    if (other.getSize() == 0) {ExOp::toZero(s); return s;}
    j=0,k=0;
    s = data[k++] * ExOp::mkTrju(other[0]) * other[0];
	for(j++;j<other.getSize();j++) {
		s2 = data[k++] * other[0];
		for(i=1;i<j;i++) s2 += data[k++] * other[i];
		// s += other[j] * ((ExOp::mkTrju(s2) + s2) + data[k++] * other[j]); WORKS FOR REAL ONLY!
		s +=  data[k++] * ExOp::mkTrju(other[j]) * other[j] + (ExOp::mkrealproj(s2) * ExOp::mkrealproj(other[j]) - ExOp::mkimmaproj(s2) * ExOp::mkimmaproj(other[j]));
	}
return s;}
...
LFHTEMP C Trianglix<C, 0u>::Xformed_inner_product( const Tuple<C, 0u>& other) const{
    ...
    //if (ExOp::mkTrju(other[0]) * other[0] < 0.0f) { fprintf(stderr, "Should not be negative!: "); ExOp::show(ExOp::mkTrju(other[0]) * other[0],stderr); LFH_exit(1); }
	for(j=1,k=1;j<t_size;j++) {
        s2 = data[k++] * other[0];
        for(i=1;i<j;i++) s2 += data[k++] * other[i];
        s += (s2 * 2.0 + data[k++] * other[j]) * other[j]; // WORKS FOR REAL ONLY!

        // s +=  data[k++] * ExOp::mkTrju(other[j]) * other[j] + (ExOp::mkrealproj(s2) * ExOp::mkrealproj(other[j]) - ExOp::mkimmaproj(s2) * ExOp::mkimmaproj(other[j]));
		//if (ExOp::mkTrju(other[j]) * other[j] < 0.0f) { fprintf(stderr,"Should not be negative!: "); ExOp::show(ExOp::mkTrju(other[j]) * other[j],stderr); LFH_exit(1); }
	}
return s;}

LFHTEMP template<class O> C Trianglix<C, 0u>::Xformed_inner_product(RemoteMemory<const O, 1u> other) const{
	C s;
    C s2;
    unsigned int i,j,k;
    if (t_size == 0) {ExOp::toZero(s); return s;}
...
return s;}
LFHTEMP template<class O> C Trianglix<C, 0u>::Xformed_inner_product(RemoteMemory<O, 1u> other) const{
	C s;
    C s2;
    unsigned int i,j,k;
    if (t_size == 0) {ExOp::toZero(s); return s;}
...
LFHTEMP template<class O> C Trianglix<C, 0u>::Xformed_inner_product( const Tuple<O, 0u>& other) const{
	C s;
    C s2;
    unsigned int i,j,k;
    if (t_size == 0) {ExOp::toZero(s); return s;}
...
LFHTEMP template<class O> C Trianglix<C, 0u>::trace_of_division(const Trianglix<O,0u> &divisor) const{C fout; ExOp::toZero(fout);
    // dev (P+P2/2)-1 dev
    Trianglix<O, 0u> invsc = divisor;
    Trianglix<C, 0u> xdev = *this;
	if (getSize() < xdev.getSize()) LFH_exit(1);
	unsigned int siz = xdev.getSize();
    ...
    O* buf = new O[siz*2]; LFH_NICE_ALLOCERROR(buf,"")
    O* hv = new O[siz *(siz+1)/2]; LFH_NICE_ALLOCERROR(hv,"")
    C* hvc = new C[siz];
    double* normbuf = new double[siz-2]; LFH_NICE_ALLOCERROR(normbuf,"")
    ...
    unsigned int i,j,k;
		for(j=siz-1;j>1;j--){
            ...
            normbuf[j-2] += ExOp::pnorm(hv[k+i]);ExOp::toZero(hv[k+j]); normbuf[j-2] *=0.5f;
            invsc.HouseHolderMultiply(hv+k, normbuf[j-2], j+1, buf,true);
            xdev.HouseHolderMultiply(hv+k, normbuf[j-2], j+1, hvc,false);
        }
        ...
        for(j=1;j<siz;j++){
            k = (j*(j+3))/2;
            xdev.offdiagelimination_down(-buf[siz+j-1]*invsc.data[k-1],j-1,hvc);
            invsc.data[k] -= ExOp::mkTrju(invsc.data[k-1]) * buf[siz+j-1]*invsc.data[k-1];
            buf[siz+j] = ExOp::mkInverse(invsc.data[k]);
            fout += xdev.data[k] * buf[siz+j];
            }
...
LFHTEMP template<class O> Tuple<C, 0u,TUPLE_FLAG_NULL> Trianglix<C, 0u>::operator*( RemoteMemory<const O, 1u> other) const{
	Tuple<C, 0u,TUPLE_FLAG_NULL> fout;
    uint32_t i,k;
    fout.setSize(other.dims[0] < t_size ?  other.dims[0] : t_size);
    for(k=0;k<fout.getSize();k++){
        const C* currow = data + ((k * (k+1u)) >> 1);
        fout[k] = currow[k] * other[k];
        for(i=0;i<k;i++) {
            fout[k] += currow[i] * other[i];
            fout[i] += currow[i] * other[k];
        }
    }
return fout;}
LFHTEMP template<class O> Tuple<C, 0u,TUPLE_FLAG_NULL> Trianglix<C, 0u>::operator*( RemoteMemory<O, 1u> other) const{
	Tuple<C, 0u,TUPLE_FLAG_NULL> fout;
    uint32_t i,k;
    fout.setSize(other.dims[0] < t_size ?  other.dims[0] : t_size);
    for(k=0;k<fout.getSize();k++){
...
LFHTEMP Trianglix<C, 0u> Trianglix<C, 0u>::operator*(const Matrix<C>& other) const{ // does not work with com
    Trianglix<C, 0u> fout; if (other.sizey != t_size) LFH_exit(1); fout.setSize(other.sizex);fout.toZero();
...
LFHTEMP void Trianglix<C, 0u>::HHscope::runFindEigen(const Trianglix<C, 0u> &target){
    Trianglix<C, 0u> invsc = target;
    unsigned int i,j,k;
    if (target.t_size <= 2) LFH_exit("dont call this on small matrices");
...
LFHTEMP Trianglix<C,0u>& Trianglix<C,0u>::toEigenTransform(typename ExCo<C>::FUNCTION_TYPE fnc){
    ...
    if (t_size < 3){
        ...
        for(o=hh_scope.t_size-1;o>1;o--){
            ...
            vec[i] *= 1.0 - sqrt(1.0 + normbuf[o-2] / ExOp::pnorm(vec[i]));
...
LFHTEMP template<class O> Trianglix<C, 0u>& Trianglix<C, 0u>::toInnerProduct(RemoteMemory<const O,2u> & other){
	this->setSize(other.dims[1]);
	C* cur = data;
	Tuple<uint32_t, 2u> coor, trcoor;
	for(coor[1]=0;coor[1]<other.dims[1];coor[1]++,cur++){
    	for(trcoor[1]=0;trcoor[1]<coor[1];trcoor[1]++,cur++){
			coor[0] = trcoor[0] = 0;
			*cur = other[coor] * other[trcoor];
			for(coor[0]++;coor[0]<other.dims[0];coor[0]++) {trcoor[0]++; *cur +=  other[coor] * other[trcoor];}
    	}
    	coor[0] = 0;
		*cur = ExOp::pnorm(other[coor]);
		for(coor[0]++;coor[0]<other.dims[0];coor[0]++) *cur +=  ExOp::pnorm(other[coor]);
    }
return *this;}
LFHTEMP template<class O> Trianglix<C, 0u>& Trianglix<C, 0u>::toOuterProduct(RemoteMemory<const O,2u> & other){
	this->setSize(other.dims[0]);
	C* cur = data;
	Tuple<uint32_t, 2u> coor, trcoor;
	for(coor[0]=0;coor[0]<other.dims[0];coor[0]++,cur++){
    	for(trcoor[0]=0;trcoor[0]<coor[0];trcoor[0]++,cur++){
			coor[1] = trcoor[1] = 0;
			*cur = other[coor] * other[trcoor];
			for(coor[1]++;coor[1]<other.dims[1];coor[1]++) {trcoor[1]++; *cur +=  other[coor] * other[trcoor];}
    	}
    	coor[1] = 0;
		*cur = ExOp::pnorm(other[coor]);
		for(coor[1]++;coor[1]<other.dims[1];coor[1]++) *cur +=  ExOp::pnorm(other[coor]);
    }
return *this;}
...
LFHTEMP TMatrix<C> Trianglix<C, 0u>::getEigenVectors(Tuple<double> &eigenvals)const{TMatrix<C> fout; fout.setSizes(t_size,t_size); eigenvals.setSize(t_size);
	if (t_size < 2){
		if (t_size == 1) {eigenvals[0] = data[0]; ExOp::toOne(fout.data[0]);}
		else {
			eigenvals[0] = data[0] - data[2];
			eigenvals[1] = sqrt(eigenvals[0] * eigenvals[0] + (ExOp::mkTrju(data[1]) * data[1] * 4.0f) ) ;
			eigenvals[0] = (data[0] + data[2] - eigenvals[1]) *0.5f;
			eigenvals[1] += eigenvals[0];
			// TODO
		}
	}else{
		Trianglix<C, 0u> bout(*this);
		fout.toOne();
		C* buf = new C[t_size*2]; LFH_NICE_ALLOCERROR(buf,"")
        C* hv = new C[t_size *(t_size+1)/2]; LFH_NICE_ALLOCERROR(hv,"")
        double* normbuf = new double[t_size-2];
        ...
        //TMatrix<C> datridiago; datridiago.setSizes(bout.getSize(),bout.getSize()).toZero();

        for(j=t_size-1;j>1;j--){
            k = (j * (j+1))/2;
            normbuf[j-2] = ExOp::pnorm(bout.data[k]);hv[k] = ExOp::mkTrju(bout.data[k]);
            for(i=1;i<j-1;i++) {normbuf[j-2] += ExOp::pnorm(bout.data[k+i]); hv[k+i] = ExOp::mkTrju(bout.data[k+i]);}
            hv[k+i] = ExOp::mkTrju(bout.data[k+i]);
            hv[k+i] *= 1.0 - sqrt(1.0 + normbuf[j-2] / ExOp::pnorm(hv[k+i]));
            normbuf[j-2] += ExOp::pnorm(hv[k+i]);ExOp::toZero(hv[k+j]); normbuf[j-2] *=0.5f;
            bout.HouseHolderMultiply(hv+k, normbuf[j-2], j+1, buf,true);
            fout.leftHouseHolderMultiply(RemoteMemory<const C,1>(hv + k, j+1), normbuf[j-2]);
        }

        // flip v
		bool didflip = bout.data[1] < 0.0;
		eigenvals[t_size-1] = bout.data[0];
		//datridiago.data[0] = bout.data[0];
        for(j=0,k=1;j<t_size-1;){
            buf[t_size-2 - (j++)] = bout.data[k] * ExOp::mkTrju(bout.data[k]);

			if ((didflip)&&(j != t_size)) fout.negateRow(j);

            //datridiago.data[j + (j-1) * bout.getSize()] = fabs(bout.data[k]);
            //datridiago.data[j-1 + j * bout.getSize()] = fabs(bout.data[k]);
            k++;
            eigenvals[t_size-1 - j] = bout.data[k];
            //datridiago.data[j * (bout.getSize()+1)] = bout.data[k];
            k += j+1;
            didflip ^= (bool)(bout.data[k] < 0.0);
        }
		//TMatrix<C> datridiago2 = datridiago;
		//TMatrix<C> datridiago3 = datridiago;
        ...
        do{
            for(loopy++;(loopy & 15) != 0;loopy++){
                ...
                s[2] = buf[0] / (p + buf[0]);
                s[pr] = p / (p + buf[0]);
                denum = pow(p + buf[0], -0.5);

				c[1] = g * denum * eigenvals[1] - buf[0]* denum;
				c[0] = g * denum;

                fout.rotRowPair(t_size-2, g * denum, -sqrt(buf[0]) * denum);
                u = s[2] * (g + eigenvals[1]);
                eigenvals[0] = g + u;
                //datridiago2.rotColPair(t_size-2,g * denum, -sqrt(buf[0]) * denum);
                //datridiago2.rotRowPair(t_size-2,g * denum, -sqrt(buf[0]) * denum);
               //datridiago3.rotColPair(t_size-2,g * denum, -sqrt(buf[0]) * denum);
                ...
                for(j=1;;j++){
                    ...
                    if (!ExOp::isValid(p)) p = s[pr^1] * buf[j-1];
                    if (j + 1 == t_size) break;
                    denum = pow(p + buf[j], -0.5);

                    ds =  -sqrt(buf[j]) * denum ; // (j >= 2) ? sqrt(buf[j]) * denum :
                    c[pr] *= denum;
                    fout.rotRowPair(t_size-2 - j , c[pr], ds);
					//datridiago2.rotRowPair(t_size-2 - j , c[pr] , ds );
					//datridiago2.rotColPair(t_size-2 - j , c[pr] , ds );
					//datridiago3.rotColPair(t_size-2 - j , c[pr] , ds );
                    c[pr^1] = c[pr] * eigenvals[j+1] - buf[j] * denum * c[pr^1];
...
/*
				printf("Testy test\n");
				datridiago.data[0] = eigenvals[t_size-1];
				for(j=1;j<t_size;j++){
					datridiago.data[j + (j-1) * bout.getSize()] = datridiago2.data[j + (j-1) * bout.getSize()];
					datridiago.data[j-1 + j * bout.getSize()] =  datridiago2.data[j-1 + j * bout.getSize()] ;
					datridiago.data[j * (bout.getSize()+1)] = eigenvals[t_size-1-j];
				}
				(fout * datridiago * fout.mkTranspose()).show();
				printf("Testy test d2\n");
                datridiago2.show();
				printf("Testy test d1\n");
                datridiago.show();*/
              }
            qual[0] = log(ExOp::pnorm(eigenvals[t_size-1]));
            qual[1] = 0.0f;
            for(j=0;j<t_size-1;j++){
                qual[0] += log(ExOp::pnorm(eigenvals[j]));
                qual[1] += log(ExOp::pnorm(buf[j]));
            }
        ...
        // silly column permutation
        fout.toTranspose().toFlipColumns();
	}
return fout;}

LFHTEMP Trianglix<C, 0u> Trianglix<C, 0u>::mkInverse(Magscaled<C>* opt_determinant, short* is_definite) const{
    Trianglix<C, 0u> fout(*this);
    Trianglix<C, 0u> resid;
    ...
    unsigned int i,j,k;
      for(j=t_size-1;j>1;j--){
        ...
        if (opt_determinant != NULL){
            Magscaled<C> magbuf; // contains -determinents!
            int pf;
            if (t_size & 1){
                *opt_determinant = ExOp::mkMagscaled(fout.data[0]);
                magbuf.value = ExCo<C>::mkOne();
                magbuf.exponent = 0.0;
                pf = ExOp::isNegative(fout.data[0]) ? 2 : 1;
                fout.data[0] = ExOp::mkInverse(fout.data[0]);
                buf[t_size] = fout.data[0];
            }else{
                pf = ExOp::isNegative(fout.data[0]) ? 2 : 1;
                magbuf = ExOp::mkMagscaled(fout.data[0]);
                *opt_determinant = ExOp::mkMagscaled(fout.data[0] * fout.data[2] - ExOp::pnorm(fout.data[1]));
                pf &= ExOp::isNegative(opt_determinant->value) ? 0 : 3;
                fout.data[0] = ExOp::mkInverse(fout.data[0]);
                buf[t_size] = fout.data[0] * fout.data[1];
                fout.data[2] = ExOp::mkInverse(fout.data[2] - ExOp::mkTrju(fout.data[1]) * buf[t_size]);
                ExOp::toZero(fout.data[1]);
                buf[t_size+1] = fout.data[2];
            }
            for(j=2-(t_size & 1);j<t_size;j+=2){
                k = (j*(j+3))/2;
                buf[t_size+j-1] *= fout.data[k-1];
                magbuf *= ExOp::mkMagscaled(-ExOp::pnorm(fout.data[k-1]));
                magbuf += (*opt_determinant) * ExOp::mkMagscaled(fout.data[k]);
                fout.data[k] = ExOp::mkInverse(fout.data[k] - ExOp::mkTrju(fout.data[k-1]) * buf[t_size+j-1]);
                buf[t_size+j] = fout.data[k];
                ExOp::toZero(fout.data[k-1]);
                //printf("part det %e => Ldet %e\n", magbuf.value * exp(magbuf.exponent), magbuf.exponent + log(fabs(magbuf.value)),pf);

                k = ((j+1)*(j+4))/2;
                buf[t_size+j] *= fout.data[k-1];
                (*opt_determinant) *= ExOp::mkMagscaled(-ExOp::pnorm(fout.data[k-1]));
                (*opt_determinant) += magbuf * ExOp::mkMagscaled(fout.data[k]);
                fout.data[k] = ExOp::mkInverse(fout.data[k] - ExOp::mkTrju(fout.data[k-1]) * buf[t_size+j]);
                buf[t_size+j+1] = fout.data[k];
                ExOp::toZero(fout.data[k-1]);
                if (j & 1) pf &= (ExOp::isNegative(magbuf.value) ? 0 : 3)&(ExOp::isNegative(opt_determinant->value) ? 2 : 1);
                else pf &= (ExOp::isNegative(magbuf.value) ? 2 : 1)&(ExOp::isNegative(opt_determinant->value) ? 0 : 3);
                //printf("part det %e => Ldet %e\n", opt_determinant->value * exp(opt_determinant->exponent), opt_determinant->exponent + log(fabs(opt_determinant->value)),pf);
            }
            if (is_definite != NULL) *is_definite = (short)pf;
        }else{
            fout.data[0] = ExOp::mkInverse(fout.data[0]);
            buf[t_size] = fout.data[0];
...
/*
 ...
 //   printf("after\n");   (Matrix<double>(fout) * tmptmp_triangle).show();
    ...
    errnorm /= k;
    while(errnorm > 0.000000000001f){
        printf("Needed refinement %e\n", errnorm);
        for(j=t_size-1;j>1;j--){
    ...
    delete[](normbuf);
        ...
        fout.data[1] = data[2] * data[5] - ExOp::pnorm(data[4]);
        fout.data[0] = data[0] * fout.data[1] + ExOp::mkTrju(data[1]) * (ExOp::mkTrju(data[4])* data[3] - data[1]*data[5]) + ExOp::mkTrju(data[3])* (data[1]*data[4] - data[2]*data[3]);

        if (opt_determinant != NULL) *opt_determinant = ExOp::mkMagscaled(fout.data[0]);
        if (is_definite != NULL) {
            *is_definite = (short)((ExOp::isNegative(data[5]) ? 2 : 1)&(ExOp::isNegative(fout.data[1]) ? 0 : 3)&(ExOp::isNegative(fout.data[0]) ? 2 : 1));
        }

        C denum = ExOp::mkInverse(fout.data[0]);
        fout.data[0] = (data[2] * data[5] - ExOp::pnorm(data[4])) * denum;
        fout.data[2] = (data[0] * data[5] - ExOp::pnorm(data[3])) * denum;
        fout.data[5] = (data[0] * data[2] - ExOp::pnorm(data[1])) * denum;
        fout.data[1] = (data[4] * ExOp::mkTrju(data[3]) - ExOp::mkTrju(data[1]) * data[5]) * denum;
        fout.data[3] = (data[1] * ExOp::mkTrju(data[4]) - ExOp::mkTrju(data[3]) * data[2]) * denum;
        fout.data[4] = (data[1] * ExOp::mkTrju(data[3]) - ExOp::mkTrju(data[4]) * data[0]) * denum;
    }else if (t_size == 2){
        fout.data[0] = data[0] * data[2] - ExOp::pnorm(data[1]);
        if (opt_determinant != NULL) *opt_determinant = ExOp::mkMagscaled(fout.data[0]);
        if (is_definite != NULL) *is_definite = ExOp::isNegative(fout.data[0]) ? 0 : ExOp::isNegative(data[0]) ? 2 : 1;
        C denum = ExOp::mkInverse(fout.data[0]);
        fout.data[0] = data[2] * denum;
        fout.data[1] = -data[1] * denum;
        fout.data[2] = data[0] * denum;
        ...
        if (opt_determinant != NULL) *opt_determinant = ExOp::mkMagscaled(fout.data[0]);
        fout.data[0] = ExOp::mkInverse(data[0]);
        if (is_definite != NULL) *is_definite = ExOp::isNegative(fout.data[0]) ? 2 : 1;
    }
return(fout);}
...
LFHTEMP const Trianglix<C, 0u>& Trianglix<C, 0u>::show(FILE* f, int level)const{
...
LFHTEMP C Trianglix<C, 0u>::log_determinant(short *definite_class, bool verbose) const{
    Magscaled<C> fout = this->determinant(definite_class, verbose);
return ExOp::mkLog(fout.value) + fout.exponent;}
LFHTEMP Magscaled<C> Trianglix<C, 0u>::determinant(short *definite_class, bool verbose) const{ Magscaled<C> fout;
    Trianglix<C, 0u> buff(*this);
    ...
    unsigned int i,j,k;
      for(j=t_size-1;j>1;j--){
            k = (j * (j+1))/2;
            normbuf[j-2] = ExOp::pnorm(buff.data[k]);hv[k] = ExOp::mkTrju(buff.data[k]);
            for(i=1;i<j-1;i++) {normbuf[j-2] += ExOp::pnorm(buff.data[k+i]); hv[k+i] = ExOp::mkTrju(buff.data[k+i]);}
            hv[k+i] = ExOp::mkTrju(buff.data[k+i]);
            hv[k+i] *= 1.0 - sqrt(1.0 + normbuf[j-2] / ExOp::pnorm(hv[k+i]));
            normbuf[j-2] += ExOp::pnorm(hv[k+i]);ExOp::toZero(hv[k+j]); normbuf[j-2] *=0.5f;
            buff.HouseHolderMultiply(hv+k, normbuf[j-2], j+1, buf,true);
        }
        ...
        Magscaled<C> magbuf; // contains -determinents!
        int pf;
        if (verbose) {printf("tridiago starts with %e\n", buff.data[0]);}
        if (t_size & 1){
            fout = ExOp::mkMagscaled(buff.data[0]);
            magbuf.value = ExCo<C>::mkOne();
            magbuf.exponent = 0.0;
            pf = ExOp::isNegative(buff.data[0]) ? 2 : 1;
            buff.data[0] = ExOp::mkInverse(buff.data[0]);
            buf[t_size] = buff.data[0];
        }else{
            pf = ExOp::isNegative(buff.data[0]) ? 2 : 1;
            magbuf = ExOp::mkMagscaled(buff.data[0]);
            fout = ExOp::mkMagscaled(buff.data[0] * buff.data[2] - ExOp::pnorm(buff.data[1]));
            pf &= ExOp::isNegative(fout.value) ? 0 : 3;
            buff.data[0] = ExOp::mkInverse(buff.data[0]);
            buf[t_size] = buff.data[0] * buff.data[1];
            buff.data[2] = ExOp::mkInverse(buff.data[2] - ExOp::mkTrju(buff.data[1]) * buf[t_size]);
            ExOp::toZero(buff.data[1]);
            buf[t_size+1] = buff.data[2];
            if (verbose) {printf("%e %e\t:\t", buff.data[1], buff.data[2]);fout.show();}
        }
        if (verbose) fout.show();
        for(j=2-(t_size & 1);j<t_size;j+=2){
            k = (j*(j+3))/2;
            buf[t_size+j-1] *= buff.data[k-1];
            magbuf *= ExOp::mkMagscaled(-ExOp::pnorm(buff.data[k-1]));
            magbuf += fout * ExOp::mkMagscaled(buff.data[k]);
            buff.data[k] = ExOp::mkInverse(buff.data[k] - ExOp::mkTrju(buff.data[k-1]) * buf[t_size+j-1]);
            buf[t_size+j] = buff.data[k];
            ExOp::toZero(buff.data[k-1]);
            //printf("part det %e => Ldet %e\n", magbuf.value * exp(magbuf.exponent), magbuf.exponent + log(fabs(magbuf.value)),pf);
			if (verbose) {printf("%e %e\t:\t", buff.data[k-1], buff.data[k]);magbuf.show();}

            k = ((j+1)*(j+4))/2;
            buf[t_size+j] *= buff.data[k-1];
            fout *= ExOp::mkMagscaled(-ExOp::pnorm(buff.data[k-1]));
            fout += magbuf * ExOp::mkMagscaled(buff.data[k]);
            buff.data[k] = ExOp::mkInverse(buff.data[k] - ExOp::mkTrju(buff.data[k-1]) * buf[t_size+j]);
            buf[t_size+j+1] = buff.data[k];
            ExOp::toZero(buff.data[k-1]);
            if (j & 1) pf &= (ExOp::isNegative(magbuf.value) ? 0 : 3)&(ExOp::isNegative(fout.value) ? 2 : 1);
            else pf &= (ExOp::isNegative(magbuf.value) ? 2 : 1)&(ExOp::isNegative(fout.value) ? 0 : 3);
            //printf("part det %e => Ldet %e\n", opt_determinant->value * exp(opt_determinant->exponent), opt_determinant->exponent + log(fabs(opt_determinant->value)),pf);
			if (verbose) {printf("%e %e\t:\t", buff.data[k-1], buff.data[k]);fout.show();}
        }
        if (definite_class != NULL) *definite_class = (short)pf;

        delete[](hv);
        delete[](buf);
        delete[](normbuf);
    }else if (t_size == 3){ // 5.932993
        C tmp = data[2] * data[5] - ExOp::pnorm(data[4]);
        fout = ExOp::mkMagscaled(data[0] * tmp + ExOp::mkTrju(data[1]) * (ExOp::mkTrju(data[4])* data[3] - data[1]*data[5]) + ExOp::mkTrju(data[3])* (data[1]*data[4] - data[2]*data[3]));
        if (definite_class != NULL) {
            *definite_class = (short)((ExOp::isNegative(data[5]) ? 2 : 1)&(ExOp::isNegative(tmp) ? 0 : 3)&(ExOp::isNegative(fout.value) ? 2 : 1));
        }
    }else if (t_size == 2){
        fout = ExOp::mkMagscaled(data[0] * data[2] - ExOp::pnorm(data[1]));
        if (definite_class != NULL) *definite_class = ExOp::isNegative(fout.value) ? 0 : ExOp::isNegative(data[0]) ? 2 : 1;
    }  else if (t_size == 1){
        fout = ExOp::mkMagscaled(data[0]);
        if (definite_class != NULL) *definite_class = ExOp::isNegative(fout.value) ? 2 : 1;
    }
return(fout);}
...
/*
    ...
    if (getSize() >2){
       ...
       return deter[0] + 300.0f * log(2.0f) * safe_mag;
        ...
        fout = ExOp::mkMagscaled(data[0]);
        if (definite_class != NULL) *definite_class = ExOp::isNegative(data[0]) ? 2 : 1;
	}else fout = ExOp::mkMagscaled(ExCo<C>::mkOne());
return fout;}*/
...
//				printf("da pre scale : %e\n", normbuf[j-2]);
			...
			hv[k+i] = ExOp::mkTrju(fout.data[k+i]);
			tmptmp = 1.0 -sqrt(1.0 + normbuf[j-2] / ExOp::pnorm(hv[k+i]));
...
LFHTEMP template<class RCL> void Trianglix<C,SIZE>::rdMatrix(const arma::Mat<RCL> &where){
    int i,j,k;
    if (where.nrow() < SIZE) LFH_exit("input matrix is too small");
...
return fout;}
LFHTEMP const Trianglix<C, SIZE>& Trianglix<C, SIZE>::show(FILE* f, int level)const{
...
template<class F, typename std::enable_if< Exlisten_functor< F, void (F::*)()>::ans, bool>::type > void ThreadBase::insert(F* functor){
	{std::lock_guard<std::mutex> lock(mut);
        todolist.push_front(std::bind(&F::operator(), functor));
    }
	condvar.notify_one();
}
...
template<class F> void ThreadBase::ThreadArrayScope::submit(F & func){
    {
        std::lock_guard<std::mutex> lock(tb.mut);
        #ifdef STDBINDFILTER
        for(int i=nbthreads-1;i> 0; i--) tb.todolist.push_front(std::bind(&F::operator(), std::ref(func), i));
        #endif // STDBINDFILTER
    }
    tb.condvar.notify_all();
    func(0);
    {
        std::unique_lock<std::mutex> lock(tb.mut);
        tb.main_condvar.wait(lock,[this]{return ((tb.nbactivethr==0)&&(tb.todolist.empty())) ||(!tb.running); });
    }
    if (tb.async_progress_maintain != 0xFFFFFFFF){
        tb.progb.finish();
        tb.async_progress_maintain = 0xFFFFFFFF;
    }
    tb.flushMsgs();
}
...
template<class F, typename... ARGS> void ThreadBase::submit_ThenWait(F & func, ARGS... args){
   /* {
    ...
    func(args...);
    {
        ...
        if ((nbactivethr!=0)||(!todolist.empty())) main_condvar.wait(lock,[this]{return ((nbactivethr==0)&&(todolist.empty())) ||(!running); });
    }
...
template<class I> bool AtomicIterator<I>::produce(AtomicIterator<I>::ELEMENT &fout, KEYELEMENT &key){
	std::lock_guard<std::mutex> lock(foreach.mut);
	switch(flag){
	case 0:
		flag = 1;
		prog =0; // that missing one is added uppon checking queue is empty!
		if (ite) {fout = *ite; key = ite(); return true;}
		flag = 2;
	break; case 1:
		//foreach.progb.update(++prog, prog_max);
		if (ite++) {fout = *ite; key = ite(); return true;}
		flag = 2;
	}
return false;}
...
template<class I> template<class TASK> auto AtomicIterator<I>::run()->decltype(TASK()()){
	flag = 0;
	//printf("Hallo\n");
	TASK* dascopes = new TASK[foreach.nbthreads+1];
	prog_max = ite.getSize();
    {
        std::lock_guard<std::mutex> lock(foreach.mut);
        //foreach.progb.start("Parallel run");
        if (this == NULL) printf("WTFWTF2222!!!!!!\n");
        for(int i=foreach.nbthreads-1;i> 0; i--) foreach.todolist.push_front([this,task = dascopes + i](){if (this == NULL) printf("WTFWTF!!!\n"); (*task)(*this);});
    }
    foreach.condvar.notify_all();
    (*dascopes)(*this);
    {
        std::unique_lock<std::mutex> lock(foreach.mut);
        foreach.main_condvar.wait(lock,[this]{return ((foreach.nbactivethr==0)&&(foreach.todolist.empty())) ||(!foreach.running); });
    }
    for(int i =foreach.nbthreads-1;i> 0; i--) dascopes[0] <<= dascopes[i];
    decltype(TASK()()) fout = dascopes[0]();
    delete[](dascopes);
	{
        //std::unique_lock<std::mutex> lock(foreach.endmut);
		//foreach.progb.finish();
    }
return fout;}/*
template<class I> template<class TASK, class A> auto AtomicIterator<I>::run(A& a)->decltype(TASK()()){
	flag = 0;
	TASK* dascopes = new TASK[foreach.nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(foreach.mut);
        for(int i=foreach.nbthreads-1;i> 0; i--) foreach.todolist.push_front([=,task = dascopes + i, a=&a](){(*task)(*this,*a);});
    }
    foreach.condvar.notify_all();
    (*dascopes)(*this,a);
    {
        std::unique_lock<std::mutex> lock(foreach.endmut);
        foreach.endcondvar.wait(lock,[this]{return ((foreach.nbactivethr==0)&&(foreach.todolist.empty())) ||(!foreach.running); });
    }
    for(int i =foreach.nbthreads-1;i> 0; i--) dascopes[0] <<= dascopes[i];
    decltype(TASK()()) fout = dascopes[0]();
    delete[](dascopes);
return fout;}
template<class I> template<class TASK, class A, class B> auto AtomicIterator<I>::run(A& a, B& b)->decltype(TASK()()){
	flag = 0;
	TASK* dascopes = new TASK[foreach.nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(foreach.mut);
        for(int i=foreach.nbthreads-1;i> 0; i--) foreach.todolist.push_front([=,task = dascopes + i, a=&a, b=&b](){(*task)(*this,*a,*b);});
    }
    foreach.condvar.notify_all();
    (*dascopes)(*this,a,b);
    {
    ...
    delete[](dascopes);
return fout;}*/
template<class I> template<class TASK, class A> auto AtomicIterator<I>::run(A& a)->decltype(TASK()(declval<A&>())){
	flag = 0;
	TASK* dascopes = new TASK[foreach.nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(foreach.mut);
        if (this == NULL) printf("WTFWTF2222!!!!!!\n");
        for(int i=foreach.nbthreads-1;i> 0; i--) foreach.todolist.push_front([this,task = dascopes + i, a=&a](){if (this == NULL) printf("WTFWTF!!!\n"); (*task)(*this,*a);});
    }
    foreach.condvar.notify_all();
    (*dascopes)(*this,a);
    {
        std::unique_lock<std::mutex> lock(foreach.mut);
	    //printf("Lets wait for %i %i\n", foreach.todolist.size(), foreach.nbactivethr);
        foreach.main_condvar.wait(lock,[this]{
		//						printf("%i%c%i\n", foreach.nbactivethr, ((foreach.nbactivethr==0)&&(foreach.todolist.empty())) ||(!foreach.running) ? 'Y' : 'N', foreach.todolist.size());
								return ((foreach.nbactivethr==0)&&(foreach.todolist.empty())) ||(!foreach.running); });
    }
    //printf("Lets merge\n");
    for(int i =foreach.nbthreads-1;i> 0; i--) dascopes[0] <<= dascopes[i];
    decltype(TASK()(declval<A&>())) fout = dascopes[0](a);
    //printf("And we are done\n");
    delete[](dascopes);
return fout;}
template<class I> template<class TASK, class A, class B> auto AtomicIterator<I>::run(A& a, B& b)->decltype(TASK()(declval<A&>(),declval<B&>())){
	flag = 0;
	TASK* dascopes = new TASK[foreach.nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(foreach.mut);
        for(int i=foreach.nbthreads-1;i> 0; i--) foreach.todolist.push_front([this,task = dascopes + i, a=&a, b=&b](){(*task)(*this,*a,*b);});
    }
    foreach.condvar.notify_all();
    (*dascopes)(*this,a,b);
    {
    ...
    for(int i =foreach.nbthreads-1;i> 0; i--) dascopes[0] <<= dascopes[i];
    auto fout = dascopes[0](a,b);
    delete[](dascopes);
return fout;}



template<class TASK, class A> int ThreadBase::execute(A& a){ // dascopes = new TASK[nbthreads+1];
	TASK* dascopes = new TASK[nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(mut);
        for(int i=nbthreads-1;i> 0; i--) todolist.push_front([task = dascopes + i, a=&a](){(*task)(*a);});
    }
    condvar.notify_all();
    (*dascopes)(a);
    {
    ...
    for(int i =nbthreads-1;i> 0; i--) dascopes[0] <<= dascopes[i];
    int fout = dascopes[0]();
    delete[](dascopes);
return fout;}

/*
template<class TASK, class A, class B> int ThreadBase::execute(A& a,B b){
	TASK* dascopes = new TASK[nbthreads+1];
    {
        std::lock_guard<std::mutex> lock(mut);
        for(int i=nbthreads-1;i> 0; i--) todolist.push_front(std::bind(&TASK::operator(), dascopes + i, a, b));
    }
    condvar.notify_all();
    (*dascopes)(a, b);
    {
...
template<class C> void ThreadBase::show(const C& obj){
	std::unique_lock<std::mutex> lck(accessFinalMutex());
    ExOp::show(obj,stdout,0);
return;}
...
LFHTEMP Dictionary<C>& Dictionary<C>::toMemfree(){
	HiddenClasses::DictionaryBase::toMemfree();
	data.toMemfree();
return *this;}


LFHTEMP auto Dictionary<C>::addEntry(const std::string str) -> decltype(data.push_back()){
    HiddenClasses::DictionaryBase::addEntry_routine(str.c_str(), str.length());
return data.push_back();}
LFHTEMP auto Dictionary<C>::addEntry(const char* str) -> decltype(data.push_back()){
    HiddenClasses::DictionaryBase::addEntry_routine(str, strlen(str));
return data.push_back();}

LFHTEMP uint32_t Dictionary<C>::find(const char* what) const{ // TODOTODO

return 0;}


LFHTEMP void Dictionary<C>::show(FILE*f, int level) const{
	fprintf(f,"dictionary with %i entries\n", entries.size());
	for(uint32_t i=0;i<  entries.size();i++){
		fprintf(f,"%s\t", entries[i]); data.showElem(i, f, level + 1);
	}
