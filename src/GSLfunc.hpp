...
double log_1plusx_mx_e(double x){
	if(x <= -1.0) LFH_exit("invalid Domain for log_1plusx_mx_e\n");
...
double gammastar(const double x){
	if(x <= 0.0) LFH_exit("invalid Domain for gammastar\n");
	else if(x < 0.5) {
		double lg = lngamma(x);
...
double gamma_inc_P_series(const double a, const double x){
	const int nmax = 5000;
...
double gamma_inc_F_CF(const double a, const double x){
	const int    nmax  =  5000;
...
int gsl_sf_psi_e(const double x, gsl_sf_result * result) {
	...
	if(x == 0.0 || x == -1.0 || x == -2.0) {
		printf("Triggered a domain error, x was an negative integer: %e\n", x);
		DOMAIN_ERROR(result);
	}
	else if(y >= 2.0) {
		...
		if(x < 0.0) {
			...
			if(fabs(s) < 2.0*GSL_SQRT_DBL_MIN) {
				printf("Triggered a domain error, x is too close to a negative integer: %e\n", x);
				DOMAIN_ERROR(result);
			}
			else {
...
static double psi_x(const double x) {
	const double y = fabs(x);
	if(x == 0.0 || x == -1.0 || x == -2.0) {
		return nan("NAN");
	}else if(y >= 2.0) {
		...
		if(x < 0.0) {
			const double s = sin(M_PI*x);
			const double c = cos(M_PI*x);
			if(fabs(s) < 2.0*GSL_SQRT_DBL_MIN) return nan("NAN");
			else return log(y) - 0.5/x + result_c.val - M_PI * c/s;
		}else return log(y) - 0.5/x + result_c.val;
	}else { /* -2 < x < 2 */
		gsl_sf_result result_c;
		if(x < 0.0) { /* x = -2 + v */
			if(x < -1.0){
				...
				const double t3 = 1.0/v;
				cheb_eval_e(&psi_cs, 2.0*v-1.0, &result_c);
				return -(t1 + t2 + t3) + result_c.val;
			}else{ /* x = -1 + v */
				const double v  = x + 1.0;
				const double t1 = 1.0/x;
				const double t2 = 1.0/v;
				cheb_eval_e(&psi_cs, 2.0*v-1.0, &result_c);
				return -(t1 + t2) + result_c.val;
			}
		}else if(x < 1.0) { /* x = v */
			const double t1 = 1.0/x;
			cheb_eval_e(&psi_cs, 2.0*x-1.0, &result_c);
			return -t1 + result_c.val;
		}else{ /* x = 1 + v */
			const double v = x - 1.0;
			cheb_eval_e(&psi_cs, 2.0*v-1.0, &result_c);
			return result_c.val;
...
static int psi_n_xg0(const int n, const double x, gsl_sf_result * result){
	if (n == 0) return gsl_sf_psi_e(x, result);
...
double incgamma_frac(const double a, const double x) {
	if(a < 0.0 || x < 0.0) LFH_exit("invalid Domain for incgamma_frac\n");
		...
		}else return gamma_inc_Q_large_x(a, x);
	}else if(x > a - sqrt(a)) {
		/* Continued fraction again. The convergence
		 * is a little slower here, but that is fine.
		 * We have to trade that off against the slow
		 ...
		 */
		return gamma_inc_Q_CF(a, x);
	} else return 1.0f - gamma_inc_P_series(a, x);
return 0.0;}
...
double incgamma_log_frac(const double a, const double x) {
	if(a < 0.0 || x < 0.0) LFH_exit("invalid Domain for incgamma_log_frac\n");
		...
		}else return log_gamma_inc_Q_large_x(a, x);
	}else if(x > a - sqrt(a)) {
		/* Continued fraction again. The convergence
		 * is a little slower here, but that is fine.
		 * We have to trade that off against the slow
		 ...
		 */
		return log_gamma_inc_Q_CF(a, x);
	}else return gslfh_sf_log_1plusx(-gamma_inc_P_series(a, x));
return 0.0;}

/* LFH log transformed */
double incgamma_1minus_frac(const double a, const double x) {
	if(a < 0.0 || x < 0.0) LFH_exit("invalid Domain for incgamma_log_1minus_frac\n");
	else if(x == 0.0) return(0.0);
		...
		}else return 1.0f - gamma_inc_Q_large_x(a, x);
	}else if(x > a - sqrt(a)) {
		/* Continued fraction again. The convergence
		 * is a little slower here, but that is fine.
		 * We have to trade that off against the slow
		 ...
		 */
		return 1.0f - gamma_inc_Q_CF(a, x);
	}else return gamma_inc_P_series(a, x);
return(0.0);}
...
double incgamma_log_1minus_frac(const double a, const double x) {
	if(a < 0.0 || x < 0.0) LFH_exit("invalid Domain for incgamma_log_1minus_frac\n");
		...
		}else return gslfh_sf_log_1plusx(-gamma_inc_Q_large_x(a, x));
	}else if(x > a - sqrt(a)) {
		/* Continued fraction again. The convergence
		 * is a little slower here, but that is fine.
		 * We have to trade that off against the slow
		 ...
		 */
		return gslfh_sf_log_1plusx(-gamma_inc_Q_CF(a, x));
	}else return log_gamma_inc_P_series(a, x);
return 0.0;}
...
double gsl_sf_beta(const double x, const double y){ gsl_sf_result result;
...
#ifndef M_LOG_SQRT_2PI
#define M_LOG_SQRT_2PI 0.9189385332046727417803297364056176398613974736377834128171515404
#endif

#ifndef M_SQRT_2PI
#define M_SQRT_2PI 2.5066282746310005024157652848110452530069867406099383166299235763
#endif
...
static double inv_cornish_fisher (double z, double nu){
  double a = 1.0 / (nu - 0.5);
  double b = 48.0 / (a * a);
  double cf1 = z * (3.0 + z * z);
  double cf2 = z * (945.0 + z * z * (360.0 + z * z * (63.0 + z * z * 4)));
  double y = z - cf1 / b + cf2 / (10.0 * b * b);
...
double gsl_ran_tdist_pdf (const double x, const double nu){
  ...
  p = ((exp (lg2 - lg1) / sqrt (M_PI * nu))* pow ((1.0 + x * x / nu), -(nu + 1) / 2));
return p;}
...
double gsl_cdf_tdist_Pinv (const double P, const double nu){
  ...
  // really? gotos ?
  {
    ...
    if (dP == 0.0 || n++ > 32) goto end;
    {
      ...
      if (fabs (step) > 1e-10 * fabs (x)) goto start;
    }
