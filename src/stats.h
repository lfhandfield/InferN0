...
class Cumulant{
	Cumulant& fixFromCentralMomments_routine(); // assumes central moments are there...
	public:
	Tuple<double> data; // should sumulant be pre divided by x! ????:q

	operator const Tuple<double>&() const{return data;}
	operator Tuple<double>&(){return data;}

	Cumulant& setSize(int s, bool initialize = true){ data.setSize(s); if (initialize) data.toZero();return *this;}
	Cumulant& setNormal(double mu, double var);

	Cumulant& setGeometric(double p, int order = 16){return this->setNegativeBinomial(1.0, p, order);}
	Cumulant& setGeometriclLogitP(double logit_p, int order = 8){return this->setNegativeBinomialLogitP(1.0, logit_p, order);}
	Cumulant& setNegativeBinomial(double r, double p, int order = 16);
	Cumulant& setHypergeometric(double r, double p, int order = 8);
	Cumulant& setNegativeBinomialLogitP(double r, double logit_p, int order = 8);

	Cumulant& setExponential(double mu, int order = 16){return this->setGamma(1.0, mu, order);}
	Cumulant& setChiSquare(double free, int order = 16){return this->setGamma(free * 0.5, 2.0, order);}
	Cumulant& setGamma(double r, double theta, int order = 16);
	Cumulant& setFromEmpirical(const myHashmap<double, double> &probabilitymass, int order = 16);

	Cumulant& setMannWhitney(int n, int m, int order = 16);

	Tuple<double, 4> getStdMoments()const;


	Cumulant& toDeviation(double observation);
	Cumulant operator+(double offset)const;
	Cumulant& operator+=(double offset);
	Cumulant operator-() const; //negative
	Cumulant operator*(double scale) const;
	Cumulant& operator*=(double scale);
	Cumulant operator+(const Cumulant& other)const;
	Cumulant& operator+=(const Cumulant& other);

	double sampleCumulant()const {return this->fromQuantile((0.5 + rand()) / (1.0 + RAND_MAX));}
	double fromQuantile(double quantile) const; // approximation, TDDO use iterated step for fixed point method for more accuracy!
	double fromLogQuantile(double logquantile) const; // approximation, TDDO use iterated step for fixed point method for more accuracy!
	double evalDensity(double x) const { return exp(this->evalLogLikelihood(x));}
	double evalLogLikelihood(double x) const;
	double evalQuantile(double x) const{ return exp(this->evalLogQuantile(x,false));}

	double evalLogLikelihoodSaddle(double x, bool neg=false)const;
	double evalLogQuantileSaddle(double x, bool neg=false)const;
	double evalLogQuantileEdgeworth(double x, bool neg=false) const;
	double evalLogQuantile(double x, bool neg=false) const;
	double evalLogQuantileOld(double x, bool neg=false) const;
};
////library(InferN0); source("Rcode.R"); showCum(2, type='l', fake = 14)
...
public:
   ...
   //     template<int F> void initCounts( SparseMatrix<double, 0u> &rowhid, SparseMatrix<double, 0u> &colhid);
    ...
    void run2D_EM_v5(const SparseMatrix<C>& in, const Vector<uint32_t> &excl_list, const Vector<uint16_t> &cellbatch, uint32_t nbstep = 200u, bool use_heavy_prior=true);
...
public:
    static const bool IsComplex = false;
    static const bool NeedsAddLink = false;
    std::function<double(const C&)> classes[nbstates];
    double* unknown_scope;
    double relerr;
    double antioss;
    Classifier(): unknown_scope(NULL){}
    void operator()(Tuple<double, nbstates>& _out, const C& _in )const;
    std::function<Tuple<double, nbstates>(const C&)> bindLogLikelihood(const C& _in){using std::placeholders::_1; Tuple<double, nbstates> fout; std::bind(&Classifier::operator(),this,fout,_1); return fout;}
    void setUnknownProbability(double); // used for the Maximization updates only, the marginal is outputed the Expectation
...
template<unsigned int SIZE = 0u>
class NormalDistribution{
public:
    static const bool NeedsAddLink = false;

    Tuple<double, SIZE> mean;
    Trianglix<double, SIZE> precision;
    double LL_term;
	std::tuple<double, Tuple<double,SIZE>, Trianglix<double,SIZE> >* stat_Scope;

    class EMScope{
    public:
        double cnt, cnt2;
        Tuple<double, SIZE> sum;
        Trianglix<double,SIZE> outer_sum;
        EMScope& setSize(unsigned int size) {sum.setSize(size);outer_sum.setSize(size);return *this;}
        EMScope& init(){cnt2 = cnt = 0; sum.toZero(); outer_sum.toZero();return *this;}
        void regist(const Tuple<double, SIZE> &instance, const double prob = 1.0f);
        double finit(NormalDistribution<SIZE>& );
    };
    NormalDistribution::EMScope mkEMscope(){NormalDistribution::EMScope fout; fout.setSize(mean.getSize()); return fout;}

    NormalDistribution(): stat_Scope(NULL){}
    NormalDistribution(const NormalDistribution<SIZE> &inp): mean(inp.mean), precision(inp.precision), LL_term(inp.LL_term){}
    const NormalDistribution<SIZE>& operator=(const NormalDistribution<SIZE>& other){ mean = other.mean; precision = other.precision; LL_term = other.LL_term; return *this;}
    ~NormalDistribution() {}

//	NormalDistribution(const NormalDistribution<SIZE>&);
//	const NormalDistribution<SIZE>& operator=(const NormalDistribution<SIZE>&);


    //template<unsigned int nbout> NormalDistribution<nbout> makeSubGaussian(const Tuple<unsigned int, nbout> & ) const;

    NormalDistribution<SIZE>& setSizes(unsigned int s);

    void setMean(const Tuple<double, SIZE>& _mean){mean = _mean;}
    void setVariance(const Tuple<double, SIZE>& _var);
    void setCovariance(const Trianglix<double, SIZE>& _cov);

    double getLikelihood(const Tuple<double, SIZE> &in) const{return exp(LL_term + precision.Xformed_inner_product(in - mean) * -0.5);}
    double LL(const Tuple<double, SIZE> &in)const{return LL_term + precision.Xformed_inner_product(in - mean) * -0.5;}

    std::function<double(const Tuple<double, SIZE>&)> bindLogLikelihood()const {using std::placeholders::_1; return std::bind(&NormalDistribution::LL,this,_1);}
    void show(FILE* o= stdout, int level = 0) const;
    template<unsigned int out_dims> NormalDistribution<out_dims> operator*(const TMatrix<double, SIZE, out_dims> &) const;
    double getCorrelationCoef(unsigned int d1, unsigned int d2) const;

    Trianglix<double, SIZE> getVariance(){return precision.mkInverse();}
    ...
    const NormalDistribution& operator*=(const Tuple<double,SIZE>& scale);
};
/*
class NormalDistributionV : public ProbabilityDistribution< double* >{
public:
    ...
    NormalDistributionV(): nbsizes(0),stat_Scope(NULL){}
    NormalDistributionV(const NormalDistributionV &inp): nbsizes(inp.nbsizes),  mean(new double[inp.nbsizes]), eigenvec(inp.eigenvec), normfactor(inp.normfactor) ,stat_Scope(NULL) {}

    ~NormalDistributionV() {delete[](stat_Scope);}



//	NormalDistribution(const NormalDistribution<nbchannels>&);
//	const NormalDistribution<nbchannels>& operator=(const NormalDistribution<nbchannels>&);
    const NormalDistributionV& operator=(const NormalDistributionV& other);

    NormalDistributionV& setSizes(unsigned int s);
    ...
    double LL(const Tuple<double>&)const;
    std::function<double(const Tuple<double>&)> bindLogLikelihood()const {using std::placeholders::_1; return std::bind(&NormalDistributionV::LL,this,_1);}
    ...
    double EMfinit();
    ProbabilityDistribution< double* >* EMnext(double alpha) const {return new NormalDistributionV(*this);}
...
*/
class FoldedNormalDistribution : public ProbabilityDistribution< double >{
public:
    double mean;
    double i_std; // = 1 / (sqrt2)std
    FoldedNormalDistribution();
    FoldedNormalDistribution(double mean, double std);
    ...
    double EMfinit();
    FoldedNormalDistribution EMnext_exec(double alpha) const;
    ProbabilityDistribution< double >* EMnext(double alpha) const{ return new FoldedNormalDistribution( this->EMnext_exec(alpha)  ); }
    void EMclear();
    void show(FILE* o = stdout) const;
};
...
//		double scale[TEMPLATE_TRIANGLE_NUMBER<2, NBPARAM >::ans +1]; // lower triangular root matrix, and a signal level variable n*(n+1)/2 + 1
		...
		NormalDistribution<NBINPUT> noise;
        ...
        NormalDistribution<NBINPUT> marginal(const Tuple<double, NBPARAM> &par) const{ NormalDistribution<NBINPUT> fout(noise); fout.mean += getSignalMean(par); return(fout);}
	...
	class BetaDistribution{
		double a,b, ilogdenum;
	public:
		double embuffer[3];
	BetaDistribution& EMinit(){ExOp::toZero(embuffer);return *this;}
	void EMregist(double val, double w =1.0);
	double EMfinit();

	double getLL(double val) {return ilogdenum + (a -1.0) * log(val) + (b -1.0) * log(1.0 - val);}
	BetaDistribution& setParams(double _a,double _b){ a = _a; b = _b; ilogdenum = lngamma(a + b) - lngamma(a) - lngamma(b); return *this;}
	};
...
LFHTEMP NormalDistribution<nbchannels>& NormalDistribution<nbchannels>::setSizes(unsigned int s){
    if (s == mean.getSize()) return *this;
    mean.setSize(s); // mean and var eigevals
    /*unsigned int coor[2];
    coor[0] =s;
    coor[1] =s;
...
/*
LFHTEMP template<unsigned int SIZEO> NormalDistribution<SIZEO> NormalDistribution<nbchannels>::makeSubGaussian(const Tuple<unsigned int, SIZEO> & channels) const{NormalDistribution<SIZEO> fout;
	unsigned int j;
	TMatrix<double, nbchannels, nbchannels> varmat = ihvar.inverse();
	TMatrix<double, SIZEO, SIZEO> var_new;

	fout.mean.setSize(channels.getSize());

	if (auto i = channels.getIterator())) do{
		fout.mean[i.ite] = mean[*i];
		for(j=0;j<i;j++) var_new.data[i + j * SIZEO] = var_new.data[j + i * SIZEO];
		for(j=i;j<SIZEO;j++) var_new.data[i + j * SIZEO] = varmat.data[channels[i] + channels[j] * nbchannels];
	}while(i++);
	...
	TMatrix<double, SIZEO, SIZEO > funt(dainv);

	var_new = funt.mkTranspose();

	for(i=0;i<SIZEO;i++) for(j=0;j<SIZEO;j++) funt.data[i+j*SIZEO] *= eigen[i];


	funt *= var_new;

	fout.normfactor = eigen[0];for(i=1;i<SIZEO;i++) fout.normfactor *= eigen[i];
...
/*
LFHTEMP void NormalDistribution<nbchannels>::EMAlphainit(double alpha){
...
LFHTEMP void NormalDistribution<nbchannels>::EMregist_priorcount(const NormalDistribution<nbchannels> &instance){
	for(unsigned int i= 1 + (((nbchannels + 3) * nbchannels) / 2);i != 0xFFFFFFFF;i--) stat_Scope[i] += instance.stat_Scope[i];
}

LFHTEMP void NormalDistribution<nbchannels>::EMregist_priorcount_var_only(const NormalDistribution<nbchannels> &instance){
	unsigned int i,j,k;
	if (instance.stat_Scope == NULL) LFH_exit(123);
	if (stat_Scope == NULL) LFH_exit(123);
...
LFHTEMP void NormalDistribution<nbchannels>::setWeight(const double &w){
...
LFHTEMP void NormalDistribution<nbchannels>::setMean_of_statistic_to_zero(){
	unsigned int i,j,k;
...
/*
	 ...
	 LFHTEMP double NormalDistribution<nbchannels>::EMfinit(){
	 int i,j,k;
	...
	*/
LFHTEMP void NormalDistribution<nbchannels>::EMScope::regist( const Tuple<double, nbchannels> &instance, const double prob){
    if ((prob <= 0.0f)||(!ExOp::isValid(prob))) return;
    cnt += prob;
    cnt2 += prob * prob;
    sum += instance * prob;
    outer_sum.addOuterProduct(instance, prob);
}
LFHTEMP double NormalDistribution<nbchannels>::EMScope::finit(NormalDistribution<nbchannels>& target){
    int i,j,k;
    TMatrix<double, nbchannels, nbchannels> nihvar;
	double num = (cnt2 - cnt*cnt) /2;
    double tmp = 1.0f / cnt;
    target.mean = sum / cnt;

    outer_sum.addOuterProduct(target.mean, cnt);

    Magscaled<double> det;
    short is_definite;
    target.precision = (outer_sum /= num).mkInverse(&det, &is_definite);

    if (is_definite != 1) {
        printf("not positive def!\n");
    }

   target.LL_term = log( 2.0 * M_PI) * (-0.5 * nbchannels) + log(det.value) + det.exponent;
return( cnt * (target.LL_term - 0.5 * nbchannels));}



/*
LFHTEMP double NormalDistribution<nbchannels>::LL(const Tuple<double, nbchannels> &in) const{
...
LFHTEMP void NormalDistribution<nbchannels>::setVariance(const Tuple<double, nbchannels>& _var){
    int i,j;
    normfactor = 1.0f;
    for(i=0;i<nbchannels;i++){
...
LFHTEMP void NormalDistribution<nbchannels>::setCovariance(const Trianglix<double, nbchannels>& _cov){
    uint32_t i,j,k;
    normfactor = 1.0f;
...
LFHTEMP double NormalDistribution<nbchannels>::getCorrelationCoef(unsigned int d1, unsigned int d2) const{
    TMatrix<double, nbchannels, nbchannels> varmat = ihvar.inverse();
    varmat *= -0.5f;
return( varmat.data[d1+ d2*nbchannels] / sqrt(varmat.data[d1*(1+nbchannels)]*varmat.data[d2*(1+nbchannels)] ) );}
LFHTEMP const NormalDistribution<nbchannels>&  NormalDistribution<nbchannels>::operator*=(const Tuple<double,nbchannels>  &scale){
    unsigned int i,j;
...
LFHTEMP void NormalDistribution<nbchannels>::show(FILE* o, int level) const{
    printf("mean="); ExOp::show(mean,stdout,0);
    printf("LL term=%e\tPrecision Matrix=\n",LL_term);
    precision.show();
}

LFHTEMP void NormalDistribution<nbchannels>::save(FILE* f) const{
    fwrite(&LL_term,sizeof(double),1,f);
    mean.save(f);
    precision.save(f);
}
LFHTEMP void NormalDistribution<nbchannels>::load(FILE* f, unsigned int size){
    fread(&LL_term,sizeof(double),1,f);
    mean.load(f);
    precision.load(f);
}
...
LFHTEMP	void ClassifierV<C>::EMaccept(bool do_accept){
    ...
    int nbstates = classes.size();
    if (!do_accept){
        if (EMsclasses == NULL) {LFH_ALIVE; LFH_exit(1);} // should always accept first!
...
LFHTEMP	void ClassifierV<C>::setPrior(double* npr){
    ...
    for(nbstates--;nbstates != 0xFFFFFFFF;nbstates--)  prior_freq[nbstates] = npr[nbstates];
    if (isseq){
